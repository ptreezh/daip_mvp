...
class TaskDecompositionIntegrator:
    ...
    async def decompose_and_execute_task(self, user_request: str) -> AsyncGenerator[str, None]:
        """分解并执行任务，返回字符串事件流"""
        yield f"[bold magenta]🧩 开始处理复杂任务:[/bold magenta]\\n'{user_request[:100]}{'...' if len(user_request) > 100 else ''}'"
        ...
        try:
            ...
            execution_results = await self.executor.execute_decomposed_tasks(tasks, user_request)
            ...
            yield f"[bold green]✅ 任务执行完成: {completed_count}/{total_count} 个任务成功执行 ({success_rate:.1f}%)[/bold green]"
    ...
    def _generate_task_list_display(self, tasks: List[DecomposedTask]) -> str:
        ...
        for i, task in enumerate(tasks, 1):
            ...
            display += f"{i}. {status_icon} **{task.title}** ({status_text})\\n"
    ...
    def _decompose_by_rules(self, user_request: str) -> List[DecomposedTask]:
        ...
        for title, description in steps:
            task = DecomposedTask(
                ...
                status=TaskStatus.PENDING,
                priority=4 if title in ["执行过程", "实现执行"] else 3
            )
            tasks.append(task)
...
class SequentialTaskExecutor:
    ...
    async def _execute_single_task(self, task: DecomposedTask, original_request: str) -> str:
        """执行单个任务"""
        if self.model_provider:
            prompt = f"""请执行以下子任务：
...
请专注于完成当前子任务，并提供具体的结果或答案。"""
    ...
    async def _synthesize_final_result(self, execution_results: Dict[str, Any]) -> str:
        ...
        if self.model_provider:
            ...
            completed_results = [tr for tr in task_results if tr.get("status") == "completed"]
...
请提供一个完整、连贯、结构化的最终回答。"""
        ...
        for tr in completed_results:
            ...
            if result_text:
                summary += f"- {tr['title']}: {result_text[:100]}...\\n"
