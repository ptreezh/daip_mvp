...
class FormatDetector:
    ...
    def get_format_from_content(self, content: str, max_bytes: int = 1024) -> Optional[str]:
        """Detect format from file content."""
        content_sample = content[:max_bytes].lower()
        
        # Simple heuristics for content-based detection
        if content_sample.startswith('<'):
            if any(tag in content_sample for tag in ['<html>', '<head>', '<body>', '<!doctype', '<!DOCTYPE']):
                return 'html'
            elif '<?xml' in content_sample or any('<' + tag in content_sample for tag in ['xml', 'document', 'note']):
                return 'xml'
        elif content_sample.startswith(('{', '[')):
            return 'json'
        elif any(marker in content_sample for marker in ['# ', '## ', '### ', '- ', '* ', '> ']):
            return 'markdown'
        elif any(marker in content_sample for marker in ['[', ']']) and any('title:' in content_sample or 'author:' in content_sample):
            # Could be ini, toml, or similar config file with title/author metadata
            return 'yaml'
        elif any(marker in content_sample for marker in ['import ', 'def ', 'class ', 'from ', 'async def']):
            return 'python'
        else:
