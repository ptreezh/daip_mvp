...
class IntentRecognitionResult(BaseModel):
    ...
    matched_pattern: Optional[str] = Field(default=None, description="Pattern that matched the intent")
    extracted_parameters: Dict[str, Any] = Field(default_factory=dict, description="Parameters extracted from the input")
    matched_text: Optional[str] = Field(default=None, description="Text that matched the intent pattern")
...
class DebateHistoryIntent(BaseModel):
    ...
    session_id: Optional[str] = Field(default=None, description="Session ID for specific history view")
...
class DocumentConversionIntent(BaseModel):
    ...
    action: str = Field(default="convert", description="Action type: 'convert', 'format', 'transform'")
    source_format: Optional[str] = Field(default=None, description="Source document format")
    target_format: Optional[str] = Field(default=None, description="Target document format")
...
class WikiManagementIntent(BaseModel):
    ...
    action: str = Field(default="list", description="Action type: 'create', 'list', 'view', 'export', 'search'")
    page_title: Optional[str] = Field(default=None, description="Title for wiki page operations")
    search_query: Optional[str] = Field(default=None, description="Query for wiki search")
...
class PaperDownloadIntent(BaseModel):
    ...
    action: str = Field(default="download", description="Action type: 'download', 'search', 'list'")
    query: Optional[str] = Field(default=None, description="Search query for papers")
    source: Optional[str] = Field(default="arxiv", description="Paper source: arxiv, pubmed, etc.")
...
class SessionManagementIntent(BaseModel):
    ...
    action: str = Field(default="list", description="Action type: 'list', 'view', 'clear', 'search'")
    session_id: Optional[str] = Field(default=None, description="Session ID for specific operations")
...
class RoleManagementIntent(BaseModel):
    ...
    action: str = Field(default="list", description="Action type: 'list', 'view', 'create', 'edit'")
...
class ModelManagementIntent(BaseModel):
    ...
    action: str = Field(default="list", description="Action type: 'list', 'view', 'switch', 'info'")
