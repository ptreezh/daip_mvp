...
class IntelligentRoleSelector:
    """Intelligently selects roles for debate topics based on relevance and debate dynamics."""
    
    def __init__(self):
        # Domain keyword mappings
        self.domain_keywords = {
            "technology": ["ai", "software", "hardware", "internet", "digital", "computer", "algorithm", "automation"],
            "ethics": ["moral", "ethical", "right", "wrong", "should", "ought", "justice", "fair", "implications"],
            "politics": ["government", "policy", "law", "regulation", "election", "democracy", "reform"],
            "economics": ["money", "market", "economy", "business", "cost", "profit", "trade", "economic", "financial", "automation"],
            "science": ["research", "study", "experiment", "evidence", "data", "scientific", "genetic", "engineering"],
            "social": ["society", "culture", "community", "people", "relationship", "social", "reform"],
            ...
            "education": ["school", "learning", "teach", "student", "education", "knowledge"],
            "philosophy": ["meaning", "existence", "consciousness", "reality", "truth", "wisdom"]
        }
        
        # Debate type patterns
        self.debate_type_patterns = {
            "technical": ["how", "implement", "technical", "feasible", "architecture", "system"],
            "ethical": ["should", "moral", "right", "wrong", "ethical", "just", "fair"],
            "social": ["society", "community", "cultural", "social", "people", "relationship"],
            "political": ["government", "policy", "law", "regulation", "political", "democratic"],
            "economic": ["economic", "financial", "cost", "benefit", "market", "business"]
        }
    ...
    def analyze_topic(self, topic: str) -> TopicAnalysis:
        ...
        if any(word in topic_lower for word in ["regulation", "policy", "implementation", "governance"]):
    ...
    def calculate_relevance_score(self, topic_analysis: TopicAnalysis,
                                role_features: Dict[str, any]) -> float:
        ...
        if topic_type == "technical" and "analytical" in personality_traits:
            score += 0.2
        elif topic_type == "ethical" and any(trait in personality_traits
                                         for trait in ["analytical", "conservative", "progressive"]):
            score += 0.2
        elif topic_type == "social" and any(trait in personality_traits
                                         for trait in ["optimistic", "pessimistic"]):
            score += 0.2
        elif topic_type == "political" and any(trait in personality_traits
                                           for trait in ["conservative", "progressive"]):
            score += 0.2
        elif topic_type == "economic" and "analytical" in personality_traits:
            score += 0.2
        
        # Persona relevance (10% weight)
        persona_relevance = self._calculate_persona_relevance(topic_analysis, role_features)
        score += persona_relevance * 0.1
        
        return min(score, 1.0)
    
    def _calculate_persona_relevance(self, topic_analysis: TopicAnalysis,
                                   role_features: Dict[str, any]) -> float:
    ...
    def calculate_conflict_potential(self, role1_features: Dict[str, any],
                                    role2_features: Dict[str, any]) -> float:
    ...
    def suggest_roles(self, topic: str, available_roles: List[Role], 
                     num_suggestions: int = 3) -> List[RoleSuggestion]:
        """Suggest roles for a debate topic."""
        topic_analysis = self.analyze_topic(topic)
        role_features_list = [(role, self.extract_role_features(role)) for role in available_roles]
        ...
        for role, features in role_features_list:
            ...
            # Generate reasoning
            reasoning = self._generate_reasoning(topic_analysis, features, relevance_score)
        ...
        for i, suggestion in enumerate(top_suggestions):
            other_features = [s.role_features for s in top_suggestions if s != suggestion]
            if other_features:
                max_conflict = max(
                    self.calculate_conflict_potential(
                        suggestion.role_features,
                        other_role
    ...
    def _generate_reasoning(self, topic_analysis: TopicAnalysis,
                          role_features: Dict[str, any], score: float) -> str:
        """Generate human-readable reasoning for role suggestion."""
        reasons = []

        # Domain expertise
        common_domains = set(topic_analysis.domains).intersection(set(role_features.get("expertise_domains", [])))
        if common_domains:
        ...
        if topic_analysis.debate_type != "general":
            relevant_traits = []
            personality_traits = role_features.get("personality_traits", [])
            if topic_analysis.debate_type == "technical" and "analytical" in personality_traits:
                relevant_traits.append("analytical thinking")
            elif topic_analysis.debate_type == "ethical" and "analytical" in personality_traits:
    ...
    def auto_select_roles(self, topic: str, available_roles: List[Role], 
                         num_roles: int = 2) -> List[Role]:
