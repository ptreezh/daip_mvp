...
class ModelSwitcher:
    ...
    def get_model_suggestions(self, current_task: str) -> List[ModelInfo]:
        ...
        if any(keyword in current_task.lower() for keyword in ["chat", "talk", "conversation"]):
            chat_models = [m for m in available_models if m.model_type == ModelType.CHAT]
            suggestions.extend(chat_models[:3])

        # For completion tasks
        elif any(keyword in current_task.lower() for keyword in ["complete", "finish", "generate"]):
            completion_models = [m for m in available_models if m.model_type == ModelType.COMPLETION]
