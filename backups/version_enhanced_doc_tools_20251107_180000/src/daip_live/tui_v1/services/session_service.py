...
class SessionServiceAdapter(BaseServiceAdapter):
    ...
    async def get_session(self, session_id: str) -> Optional[Dict]:
        ...
        try:
            if self.service and hasattr(self.service, 'get_session'):
                ...
                self.emit_event('session_retrieved', {'session_id': session_id, 'session': session})
                return session
            else:
                # Mock data for testing/fallback
                mock_session = {"id": session_id, "status": "active"}
                self.emit_event('session_retrieved', {'session_id': session_id, 'session': mock_session})
                return mock_session
        except Exception as e:
            logger.error(f"Error getting session {session_id}: {e}")
            self.emit_event('session_error', {'error': str(e), 'session_id': session_id})
            return None
    ...
    async def update_session_name(self, session_id: str, new_name: str) -> bool:
        ...
        try:
            ...
            if success:
                self.emit_event('session_updated', {
