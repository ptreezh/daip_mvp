...
try:
    ...
    async def test_collaboration():
        ...
        # 结束会话
        result = await collaborator.end_collaboration()  # 使用正确的函数名
...
try:
    ...
    # 测试混合识别能力
    hybrid_tests = [
        ("创建维基 混合测试", "create_wiki", "混合识别-维基"),
        ("多模型辩论 混合测试", "start_debate", "混合识别-辩论"),
        ("帮我处理混合内容", "execute_skill", "混合识别-技能")
    ]
    
    hybrid_success = 0
    for test_input, expected_intent, description in hybrid_tests:
    ...
    print(f"   混合识别准确率: {hybrid_success}/{len(hybrid_tests)} ({hybrid_success/len(hybrid_tests)*100:.1f}%)")
    
    hybrid_success_flag = hybrid_success >= 2  # 至少2个通过
    
except ImportError:
    print("   ⚠️  HybridIntentRecognizer 未实现")
    hybrid_success_flag = False
except Exception as e:
    print(f"   ❌ 混合意图识别器错误: {e}")
    import traceback
    traceback.print_exc()
...
print(f"  混合意图识别: {'✅ 通过' if hybrid_success_flag else '❌ 失败'}")

print(f"\\n🎯 总体验证结果: {'✅ 全部通过' if overall_pass else '⚠️  部分通过'}")

if overall_pass:
    print("\\n🎉 恭喜！DAIP-LIVE系统所有功能已成功实现并通过验证！")
    print("   - 多角色维基协作编辑功能完整 (支持多模型协同创作)")
    print("   - 论文搜索下载连续流程完整 (搜索→扩展→下载)") 
    print("   - 意图识别准确率达标 (≥80%)")
    print("   - 澄清机制工作正常")
    print("   - 混合意图识别器运行正常")
    print("\\n🚀 系统现在可以部署使用！")
