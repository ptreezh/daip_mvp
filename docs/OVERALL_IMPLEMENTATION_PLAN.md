...
*   **子阶段 2.7: Role Recommender Service 实现 (已完成)**
    *   **原则**: 恢复并增强旧有代码中的自动化角色推荐功能，以遵守“技术遗产全覆盖原则”。
    *   **说明**: 根据项目手册 (`manu.html`) 的描述，此功能已作为核心体验的一部分完全实现，用于在辩论开始前进行交互式角色推荐。
    *   **原子计划 2.7.1**: 创建 `RoleRecommenderService`，使用向量数据库（ChromaDB）管理角色嵌入。(已完成)
    *   **原子计划 2.7.2**: 实现 `build_index` 方法，在启动时为所有角色创建并存储向量嵌入。(已完成)
    *   **原子计划 2.7.3**: 实现 `recommend_roles` 方法，根据输入主题的语义相似度返回最相关的角色。(已完成)
...
*   **子阶段 3.5: Synthesis Engine 实现 (已完成)**
    *   **原子计划 3.5.1**: 创建新文件 `src/core_services/synthesis_engine.py`。(已完成)
    *   **原子计划 3.5.2**: 在 `synthesis_engine.py` 中，实现 `synthesize_opinions` 方法，该方法接收多个观点文本。(已完成)
    *   **原子计划 3.5.3**: 实现调用 `LLM Interface` 生成综合性总结的逻辑。(已完成)
...
*   **子阶段 4.1: 辩论与共识协议 (已完成)**
    *   **说明**: 随着新的 TUI 和前后端解耦的辩论引擎的实现，辩论的核心流程（包括轮次控制和调度）已经完成。
...
*   **子阶段 5.1: CLI 客户端开发 (已完成)**
    *   **说明**: 根据项目手册 (`manu.html`)，已基于 `prompt-toolkit` 实现了一个功能完备、可交互的 TUI 客户端，该客户端为“辩论”场景深度优化。
    *   **架构决策 (新增)**: 为避免单一客户端的复杂性膨胀，未来的新工作流（如敏捷项目管理）将考虑开发独立的、专用的前端应用程序，而不是强行集成到现有的辩论 TUI 中。这确保了每个客户端都能提供针对其特定场景的最佳用户体验。
    *   **原子计划 5.1.1**: 开发用于发起和管理辩论的 CLI 命令。(已完成)
    *   **原子计划 5.1.2**: 实现对 SSE 实时事件流的处理和显示。(已完成，通过 TUI 的实时滚动更新实现)
    *   **原子计划 5.1.3**: 开发用于任务管理的 CLI 命令。(**状态变更**: 未来计划 -> 将作为独立的客户端应用实现)
*   **子阶段 5.2: 端到端集成测试 (未来计划)**
