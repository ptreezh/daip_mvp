...
-   [x] **GREEN**: **T-FMW-06**: 编写一个失败的测试 `test_navigation_to_submenu_on_valid_input`。该测试模拟用户输入 "5"，并使用 `mock.patch` 断言一个名为 `start_role_management` 的占位符函数被准确调用了一次。
...
-   [x] **GREEN**: **T-FMW-08**: 编写一个失败的测试 `test_error_message_on_non_numeric_input`，模拟用户输入 "abc"，捕获stdout并断言输出了清晰的错误消息。
...
-   [x] **GREEN**: **T-FMW-10**: 编写一个失败的测试 `test_error_message_on_out_of_range_input`，模拟用户输入 "99"，并断言输出了清晰的错误消息。
-   [x] **GREEN**: **T-FMW-11**: 在 `if/elif` 逻辑中添加一个 `else` 块，以处理范围之外的数字输入，让 `test_error_message_on_out_of_range_input` 测试通过。
-   [x] **GREEN**: **T-FMW-12**: (可选) 重构主循环的输入处理逻辑，可能将其提取到一个单独的 `handle_main_menu_input` 函数中，以保持 `main` 函数的整洁。确保所有测试仍然通过。
