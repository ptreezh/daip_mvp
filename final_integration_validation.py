...
# 评估是否达到目标
intent_target = accuracy >= 90
clarif_target = clarif_accuracy >= 85
overall_success = intent_target and clarif_target and collaboration_success

print(f"\\n🎯 整体验收标准:")
print(f"  意图识别 ≥90%: {'✅ 达到' if intent_target else '❌ 未达到'} ({accuracy:.1f}%)")
print(f"  澄清机制 ≥85%: {'✅ 达到' if clarif_target else '❌ 未达到'} ({clarif_accuracy:.1f}%)")
