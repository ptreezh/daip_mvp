"""
修复EnhancedIntentRecognizer中的技能参数提取函数
"""
import sys
sys.path.insert(0, './src')

def update_skill_params_extractor():
    print("修复技能参数提取函数...")
    ...
    # 找到extract_skill_params函数的开始和结束位置
    start_idx = -1
    end_idx = -1
    
    for i, line in enumerate(lines):
        if "def _extract_skill_params(self, text: str, match: re.Match)" in line:
            start_idx = i
        elif start_idx != -1 and line.strip().startswith("return") and "}" in line:
            # 查找函数结束位置
            for j in range(i, len(lines)):
                if lines[j].strip() and not lines[j].startswith(" ") and not lines[j].startswith("\t"):
                    # 非空行但不缩进，说明是下一函数或块开始了
                    end_idx = j
                    break
            if end_idx == -1:  # 如果没找到缩进变化点，尝试查找函数定义
                end_idx = len(lines)
            break
    
    if start_idx == -1:
        print("未找到 _extract_skill_params 函数定义")
        return
    
    # 重写函数部分
    new_function = '''    def _extract_skill_params(self, text: str, match: re.Match) -> Dict[str, Any]:
        """提取技能参数"""
        # 首先尝试从文本中提取技能相关参数
        skill_patterns = [
            # 专门的技能请求 - 明确的关键词后跟内容
        ...
        elif any(keyword in text.lower() for keyword in ["技能", "skill"]):
    ...
    # 写回文件
    with open("D:/DAIP/refactdoc/src/daip_live/agent_engine/enhanced_intent_recognizer.py", "w", encoding="utf-8") as f:
        f.writelines(updated_lines)
    
    print("✅ _extract_skill_params 函数已更新")
...
if __name__ == "__main__":
    success = update_skill_params_extractor()
