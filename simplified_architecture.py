...
class SimpleIntentRecognizer:
    ...
    async def recognize_intent(self, text: str) -> IntentRecognitionResult:
        ...
        action_words = ["分析", "设计", "创建", "开发", "实现", "制定", "研究", "评估", "比较", "总结"]
        action_count = sum(1 for word in action_words if word in text_lower)

        is_complex_task = task_flow_match or (word_count > 10 and action_count >= 2)

        if is_complex_task:
            # 需要进一步判断是否需要任务分解
            needs_decomposition = word_count > 15 or action_count >= 3
            return IntentRecognitionResult(
                mode=InteractionMode.TASK_FLOW,
                confidence=0.8 if needs_decomposition else 0.7,
                original_request=text,
...
async def test_simplified_architecture():
    ...
    for request, expected_mode in test_cases:
        ...
        if expected_mode == InteractionMode.THOUGHTFUL_CYCLE:
            actual_mode = InteractionMode.THOUGHTFUL_CYCLE if any("深度思考" in r for r in results) else actual_mode
        elif expected_mode == InteractionMode.TASK_FLOW:
            actual_mode = InteractionMode.TASK_FLOW if any("任务" in r for r in results) else actual_mode
        else:  # SIMPLE_CHAT
            actual_mode = InteractionMode.SIMPLE_CHAT if any("聊天" in r for r in results) else actual_mode
        
        success = actual_mode and actual_mode == expected_mode
        status = "✅" if success else "❌"
        print(f"   预期: {expected_mode.value} -> 实际: {actual_mode.value if actual_mode else 'UNKNOWN'} {status}")
