...
class AppState:
    ...
    def __init__(self):
        ...
        self._universal_context_service = UniversalContextService(
            token_service=self.token_management_service, memory_service=self._memory_service
        )
        ...
        self._intent_analysis_service = BasicIntentAnalysisService(
            user_profile_service=self._user_profile_service, llm_interface=self.llm_interface
        )

        self._personal_context_service = BasicPersonalContextService(
            user_profile_service=self._user_profile_service, memory_service=self._memory_service
        )
        ...
        self._interaction_manager = InteractionManager(client=ollama_client, model=default_llm_config.model)
    ...
    def _initialize_schema_library(self):
        ...
        class MockSchemaLibrary:
            ...
            def _extract_tags_from_schema(self, content):
                return []
    ...
    def upsert_role_to_vector_db(self, role: dict):
        ...
        self.role_collection.upsert(ids=[doc_id], embeddings=[embedding], documents=[document_content])
    ...
    def search_roles_by_vector(self, query: str, top_k: int = 5) -> list[dict[str, Any]]:
        ...
        results = self.role_collection.query(query_embeddings=[query_embedding], n_results=top_k)
...
"""
    ...
    def virtual_team_service(self):
        ...
        if "virtual_team_service" not in self._service_instances:
            try:
                ...
                logger.warning(
                    f"VirtualTeamService initialization failed: {e}. Continuing without virtual team functionality."
                )
    ...
    def fact_validation_service(self):
        ...
        if "fact_validation_service" not in self._service_instances:
            try:
                FactValidationService = _import_service(
                    "src.core_services.fact_validation_service", "FactValidationService"
                )
    ...
    def fact_extraction_service(self):
        ...
        if "fact_extraction_service" not in self._service_instances:
            try:
                FactExtractionService = _import_service(
                    "src.core_services.fact_extraction_service", "FactExtractionService"
                )
    ...
    def intent_analysis_service(self):
        ...
        if "intent_analysis_service" not in self._service_instances:
            try:
                BasicIntentAnalysisService = _import_service(
                    "src.core_services.intent_analysis_service", "BasicIntentAnalysisService"
                )
    ...
    def personal_context_service(self):
        ...
        if "personal_context_service" not in self._service_instances:
            try:
                BasicPersonalContextService = _import_service(
                    "src.core_services.personal_context_service", "BasicPersonalContextService"
                )
    ...
    def prompt_optimization_service(self):
        ...
        if "prompt_optimization_service" not in self._service_instances:
            try:
                BasicPromptOptimizationService = _import_service(
                    "src.core_services.prompt_optimization_service", "BasicPromptOptimizationService"
                )
    ...
    def universal_context_service(self):
        ...
        if "universal_context_service" not in self._service_instances:
            try:
                UniversalContextService = _import_service(
                    "src.core_services.universal_context_service", "UniversalContextService"
                )
    ...
    def session_management_service(self):
        ...
        if "session_management_service" not in self._service_instances:
            try:
                SessionManagementService = _import_service(
                    "src.core_services.session_management_service", "SessionManagementService"
                )
    ...
    def multi_agent_collaboration_system(self):
        ...
        if "multi_agent_collaboration_system" not in self._service_instances:
            try:
                MultiAgentCollaborationSystem = _import_service(
                    "src.core_services.multi_agent_collaboration_system", "MultiAgentCollaborationSystem"
                )
