...
class EnhancedChatExecutor:
    ...
    async def chat_run(
        self, initial_goal: str, step_executor: Any
        ...
        yield ThoughtEvent(
            content=f"Session {session.session_id} started for goal: {initial_goal}"
        )

        # Process the initial goal
        async for event in self._process_chat_turn(
            initial_goal, step_executor, session
        ):
            yield event
        ...
        try:
            while True:
                ...
                session.history.append(
                    DialogueTurn(participant_id="user", content=user_input)
                )

                async for event in self._process_chat_turn(
                    user_input, step_executor, session
                ):
                    yield event
            ...
            yield ThoughtEvent(
                content=f"Session {session.session_id} saved with status {AgentState.COMPLETED.name}"  # noqa: E501
            )

    async def _process_chat_turn(
        self, turn_description: str, step_executor: Any, session: Any
    ) -> AsyncGenerator[AgentEvent, None]:
        """
        处理单个聊天回合
        ...
        if intent:
            # 显示意图识别结果
            yield ThoughtEvent(
                content=f"识别到意图: {intent.description} (置信度: {intent.confidence:.2f})"  # noqa: E501
            )

            # 根据意图类型处理（识别器枚举：QUESTION/CHAT/WORKFLOW + intent.name 细分）
            if intent.intent_type == IntentType.QUESTION:
                # 问题需要置信度循环
                async for event in self._handle_question_with_confidence_loop(
                    intent, step_executor, session
                ):
                    yield event
                ...
                # 工作流类型，转换为相应命令
                modified_task = self._convert_intent_to_command(
                    intent, turn_description
                )
                yield ThoughtEvent(content=f"转换为工作流命令: {modified_task}")
                current_task = TodoItem(
                    id=0, description=modified_task, status="pending", priority=1
                )
                async for event in step_executor.execute_step(current_task, session):
                    yield event
        else:
    ...
    async def _handle_question_with_confidence_loop(
        self, intent: Any, step_executor: Any, session: Any
    ) -> AsyncGenerator[AgentEvent, None]:
        """
        处理需要置信度循环的问题
        ...
        yield ThoughtEvent(
            content=f"问题识别，当前置信度: {current_confidence:.2f}，开始置信度循环..."
        )

        for round_num in range(self.max_reflection_rounds):
            if current_confidence >= self.confidence_threshold:
                yield ThoughtEvent(
                    content=f"置信度达到阈值 ({current_confidence:.2f} >= {self.confidence_threshold})，生成最终回答"  # noqa: E501
                )
                ...
                # 执行推理
                response, confidence = await self._execute_reasoning_with_confidence(
                    prompt
                )
                ...
                yield ThoughtEvent(
                    content=f"第{round_num + 1}轮反思，当前置信度: {current_confidence:.2f} < {self.confidence_threshold}"  # noqa: E501
                )
        ...
        yield ThoughtEvent(content="达到最大反思轮数，使用当前最佳答案")
        prompt = self._construct_confidence_prompt(
            question, session, self.max_reflection_rounds
        )
        response, _ = await self._execute_reasoning_with_confidence(prompt)
        yield FinalResponseEvent(content=response)

    def _construct_confidence_prompt(
        self, question: str, session: Any, round_num: int
    ) -> str:
        """构建置信度循环的提示词"""
        # 获取历史对话
        history_text = "\n".join(
...
"""
    ...
    def _construct_reflection_prompt(
        ...
        previous_text = "\n".join(
            [f"回答{i + 1}: {resp}" for i, resp in enumerate(previous_responses)]
        )
...
"""
    ...
    async def _execute_reasoning_with_confidence(
        self, prompt: str
    ) -> tuple[str, float]:
        """执行推理并返回置信度评估"""
        try:
    ...
    def _calculate_confidence(self, response: str) -> float:
        ...
        # 基于包含关键信息指标
        if any(
            keyword in response for keyword in ["因为", "所以", "首先", "其次", "最后"]
        ):
            confidence += 0.1
        ...
        if any(
            keyword in response
            for keyword in ["根据", "基于", "分析", "研究表明", "数据显示"]
        ):
            confidence += 0.1
    ...
    def _save_to_long_term_memory(self, content: str) -> None:
        ...
        try:
            ...
            # 写入文件
            with open(
                self.memory_service.long_term_memory_file, "w", encoding="utf-8"
            ) as f:
                f.write(new_memory)
    ...
    def organize_knowledge(self, session: Any) -> None:
        ...
        try:
            ...
            if knowledge_points:
                organized_knowledge = "\n".join(
                    [f"- {point}" for point in knowledge_points]
                )
