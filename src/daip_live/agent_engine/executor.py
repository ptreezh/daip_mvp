...
from daip_live.permission.permission_manager import PermissionManager  # 新增权限管理器导入
...
class AgentExecutor:
    def __init__(
        ...
        self.chat_executor = ChatExecutor(session_manager, memory_service, user_input_queue)
    ...
    async def _execute_workflow_element_events(self, element: Any) -> AsyncGenerator[AgentEvent, None]:
        ...
        async for event in self.workflow_executor._execute_workflow_element_events(element, {}, {}):
            yield event
    ...
    async def _execute_task_element_events(self, element: Any) -> AsyncGenerator[AgentEvent, None]:
        """执行任务元素并产生事件"""
        # 这个方法现在在WorkflowExecutor中实现
        async for event in self.workflow_executor._execute_task_element_events(element, {}):
            yield event
    ...
    async def _execute_condition_element_events(self, element: Any) -> AsyncGenerator[AgentEvent, None]:
        """执行条件元素并产生事件"""
        # 这个方法现在在WorkflowExecutor中实现
        async for event in self.workflow_executor._execute_condition_element_events(element):
            yield event
    ...
    async def _execute_loop_element_events(self, element: Any) -> AsyncGenerator[AgentEvent, None]:
        """执行循环元素并产生事件"""
        # 这个方法现在在WorkflowExecutor中实现
        async for event in self.workflow_executor._execute_loop_element_events(element, {}):
            yield event
    ...
    async def _execute_subworkflow_element_events(self, element: Any) -> AsyncGenerator[AgentEvent, None]:
        """执行子工作流元素并产生事件"""
        # 这个方法现在在WorkflowExecutor中实现
        async for event in self.workflow_executor._execute_subworkflow_element_events(element):
            yield event
    ...
    async def _execute_tool_with_permission_check(
        ...
        return await self.step_executor._execute_tool_with_permission_check(tool_name, args, session_context)
