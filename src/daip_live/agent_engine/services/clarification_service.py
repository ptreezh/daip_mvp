...
from daip_live.agent_engine.models.clarification_models import (
    ClarificationType, ClarificationOption, ClarificationRequest, ContextualIntentResult
)
...
class ClarificationService:
    ...
    def check_missing_keywords(self, intent_name: str, parameters: Dict[str, Any]) -> Optional[ClarificationRequest]:
    ...
    def check_missing_parameters(self, intent_name: str, parameters: Dict[str, Any]) -> Optional[ClarificationRequest]:
    ...
    def check_ambiguous_intent(self, original_input: str, intent_confidence: float, 
                              possible_intents: List[str]) -> Optional[ClarificationRequest]:
        """Check if intent is ambiguous and requires user clarification."""
        
        # If we have multiple possible intents with similar confidence, ask for clarification
        if len(possible_intents) > 1 and intent_confidence < 0.8:  # Low confidence with multiple options
            options = []
    ...
    def create_clarification_event(self, session_id: str, clarification_request: ClarificationRequest) -> ClarificationRequestEvent:
    ...
    def process_user_clarification(self, session_id: str, user_response: str, 
                                   pending_clarification: ClarificationRequest) -> Dict[str, Any]:
        ...
        if pending_clarification.type == ClarificationType.MISSING_KEYWORDS:
            ...
            result_params[pending_clarification.required_parameters[0]] = user_response.strip()
