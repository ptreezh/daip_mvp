...
class SessionManager:
    ...
    async def create_session_event(self, session_id: str, status: AgentState) -> ThoughtEvent:
        """
        创建会话事件
        ...
        Returns:
            ThoughtEvent: 会话事件
        """
        return ThoughtEvent(content=f"Session {session_id} saved with status {status.name}")