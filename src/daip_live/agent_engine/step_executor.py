...
class StepExecutor:
    ...
    async def execute_step(self, current_task: TodoItem, session: "Session") -> AsyncGenerator[AgentEvent, None]:
        """
        执行单个步骤
        ...
        while not step_completed and iteration_count < max_iterations:
            ...
            if state == AgentState.THINKING:
                ...
                session.history.append(DialogueTurn(participant_id="agent", content=self.llm_response))

                # Send model metrics event
                self._request_count += 1
                yield ModelMetricsEvent(latency=latency, request_count=self._request_count)
                ...
                try:
                    ...
                    yield ToolOutputEvent(tool_name=tool_name, status="success", output=tool_result)
                    ...
                    if isinstance(e, ToolPermissionRequest):
                        ...
                        self.last_tool_result = f"Permission request for tool '{tool_name}' requires user interaction."
