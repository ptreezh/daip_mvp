...
class WorkflowExecutor:
    ...
    async def execute_workflow(
        ...
        if not workflow_definition or not workflow_definition.elements:
            ...
            yield ThoughtEvent(
                content="WORKFLOW_EXECUTION_FAILED: Empty workflow definition"
            )
            return
        ...
        if workflow_definition.logging:
            yield ThoughtEvent(
                content=f"Starting workflow: {workflow_definition.name} v{workflow_definition.version}"  # noqa: E501
            )
        ...
        while current_element_id and state != AgentState.FAILED:
            ...
            if not element:
                ...
                yield ThoughtEvent(
                    content=f"WORKFLOW_EXECUTION_FAILED: Element {current_element_id} not found"  # noqa: E501
                )
            ...
            if element.logging:
                yield ThoughtEvent(
                    content=f"Executing element: {element.name} ({element.type.value})"
                )
            ...
            # 根据元素类型执行相应逻辑
            # 首先执行元素并收集所有事件
            async for event in self._execute_workflow_element_events(
                element, element_outputs, loop_counters
            ):
                yield event
    ...
    async def _execute_workflow_element_events(
        ...
        if element.type == WorkflowElementType.TASK:
            async for event in self._execute_task_element_events(
                element, element_outputs
            ):
                yield event
            ...
            async for event in self._execute_loop_element_events(
                element, loop_counters
            ):
                yield event
    ...
    def _get_next_element_id(
        self, element: Any, loop_counters: dict[str, int]
    ) -> Optional[str]:
        """
        获取下一个元素ID
    ...
    async def _execute_task_element_events(
        self, element: Any, element_outputs: dict[str, Any]
    ) -> AsyncGenerator[AgentEvent, None]:
        """
        执行任务元素并产生事件
    ...
    async def _execute_loop_element_events(
        ...
        yield ThoughtEvent(
            content=f"Executed loop element: {element.name} (iteration {loop_counters[element.id]})"  # noqa: E501
        )
    ...
    async def _execute_subworkflow_element_events(
        ...
        if element.workflow_ref:
            ...
            if element.logging:
                yield ThoughtEvent(
                    content=f"Subworkflow {element.workflow_ref} execution completed"
                )
