...
def create(
    ...
    content: str = typer.Option("", "--content", "-c", help="Initial content for the page"),
    tags: Optional[str] = typer.Option(None, "--tags", "-t", help="Comma-separated tags for the page")
):
    ...
    async def _create_wiki_page():
        ...
        async with perf_monitor.measure_command("wiki_create") as metrics:
            ...
            try:
                ...
                try:
                    ...
                    role_model_manager = RoleModelManager(roles_dir_path=str(roles_path))
                    ...
                    print(f"Warning: Could not initialize full wiki manager: {e}")
                ...
                # Process content to handle escaped characters that may have been passed from CLI
                content_to_use = content
                if content_to_use:
                    # On Windows command line, when user types \\n, it becomes literal \\n in the parameter
                    # We need to convert these literal escape sequences to actual characters
                ...
                page = wiki_manager.create_page(title=title, content=content_to_use, tags=tag_list)

                console.print(f"[green]✅ Wiki page '{page.title}' created successfully![/green]")
...
@app.command()
def list(
    limit: int = typer.Option(20, "--limit", "-l", help="Maximum number of pages to show"),
    ...
    async def _list_wiki_pages():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_list") as metrics:
            if not json_output:
            ...
            try:
                ...
                if not wiki_path.exists():
                    if json_output:
                        ...
                        console.print("[dim]Create a page with 'daip wiki create' first[/dim]")
                    return
...
def _display_wiki_pages(pages: List, total_count: int):
...
def search(
    ...
    by_content: bool = typer.Option(True, "--content", "-c", help="Search in content (default)"),
    by_tag: bool = typer.Option(False, "--tag", "-t", help="Search by tag instead of content"),
    ...
    async def _search_wiki_pages():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_search") as metrics:
            if not json_output:
                console.print(f"[bold blue]🔍 Searching wiki pages for: {query}[/bold blue]")
            ...
            try:
                ...
                if json_output:
                    ...
                    _display_search_results(query, results[:limit], len(results), by_tag)
...
def _display_search_results(query: str, results: List, total_count: int, by_tag: bool):
    """Display search results in formatted table"""

    if not results:
        console.print("[yellow]⚠️  No results found for your query[/yellow]")
        console.print(f"[dim]Search: '{query}' ({'by tag' if by_tag else 'by content'})[/dim]")
        return

    table = Table(title=f"Search Results for '{query}' ({'by tag' if by_tag else 'by content'})")
    table.add_column("Title", style="cyan", no_wrap=True)
...
def show(
    ...
    async def _show_wiki_page():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_show") as metrics:
            if not json_output:
            ...
            try:
                ...
                if not page:
                    if json_output:
                        console.print(json.dumps({"error": f"Page '{title}' not found", "page_exists": False}))
                    else:
...
def _display_wiki_page(page):
    ...
    info_lines = [
        ...
        f"[bold]Size:[/bold] {page.get_word_count()} words, ~{page.get_reading_time()} min read"
    ]
...
def delete(
    ...
    confirm: bool = typer.Option(True, "--confirm/--no-confirm", "-y/-n", help="Confirm deletion")
):
    """Delete a wiki page"""
    ...
    async def _delete_wiki_page():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_delete") as metrics:
            console.print(f"[bold red]🗑️  Deleting wiki page: {title}[/bold red]")

            if confirm:
                confirm_input = input(f"Are you sure you want to delete '{title}'? This cannot be undone. (y/N): ")
            ...
            try:
                ...
                if success:
                    console.print(f"[green]✅ Wiki page '{title}' deleted successfully![/green]")
                else:
                    console.print(f"[red]❌ Page '{title}' not found[/red]")
...
def stats(
    ...
    async def _show_wiki_stats():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_stats") as metrics:
            if not json_output:
...
def _display_wiki_stats(stats):
    ...
    content_lines = [
        ...
        f"[bold]Last Updated:[/bold] {stats.last_updated.strftime('%Y-%m-%d %H:%M:%S')}"
    ]

    # Add top tags if available
    if stats.most_used_tags:
        top_tags = ", ".join([f"{tag}({count})" for tag, count in stats.most_used_tags[:5]])
    ...
    panel = Panel(
        content,
        title="[bold green]Wiki Statistics[/bold green]",
        border_style="green"
    )
...
def export(
    ...
    async def _export_wiki_pages():
        perf_monitor = PerformanceMonitor()
        async with perf_monitor.measure_command("wiki_export") as metrics:
            console.print(f"[bold blue]📤 Exporting wiki pages to: {output_dir}[/bold blue]")
            ...
            try:
                ...
                console.print(f"[green]✅ Wiki pages exported successfully![/green]")
                console.print(f"[dim]Format: {format}, Output: {output_path.absolute()}[/dim]")
