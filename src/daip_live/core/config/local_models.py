...
LOCAL_MODEL_CONFIGS = {
    ...
    "mock-llm": LocalModelConfig(
        model_name="mock-llm", temperature=0.5, max_tokens=500
    ),
    "local-gpt": LocalModelConfig(
        model_name="local-gpt", temperature=0.8, max_tokens=2000
    ),
}
...
def is_local_model(model_name: str) -> bool:
    """检查是否为本地模型"""
    return (
        model_name in LOCAL_MODEL_CONFIGS
        or model_name.startswith("test-")
        or model_name.startswith("mock-")
    )
