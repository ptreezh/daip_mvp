...
class Session(BaseModel):
    ...
    session_type: Literal[
        "debate", "chat", "workflow", "compression", "auto_compression"
    ]
    ...
    status: AgentState = (
        AgentState.INIT
    )  # Represents the state of the session orchestration
...
class PermissionRequestEvent(BaseModel):
    ...
    timeout_seconds: float = Field(
        default=30.0, description="Timeout for user response"
    )
    risk_level: Literal["low", "medium", "high"] = Field(
        default="medium", description="Risk level of the requested operation"
    )
    description: Optional[str] = Field(
        default=None, description="Human-readable description of the operation"
    )
...
class PermissionResult(BaseModel):
    ...
    response_time_seconds: float = Field(
        default=0.0, description="Time taken to process the permission request"
    )
...
class PermissionInteraction(BaseModel):
    ...
    timeout_seconds: float = Field(
        default=30.0, description="Timeout for this permission interaction"
    )
    user_choice: Optional[str] = None
    error_count: int = Field(
        default=0, description="Number of errors encountered during processing"
    )
    ...
    circuit_breaker_open: bool = Field(
        default=False, description="Whether circuit breaker is open for this request"
    )
    is_duplicate: bool = Field(
        default=False, description="Whether this is a duplicate request"
    )
    ...
    def update_response(self, response: PermissionResponse) -> None:
        ...
        if self.state != PermissionState.PENDING:
            raise ValueError(
                f"Cannot update response for non-pending interaction (current state: {self.state})"  # noqa: E501
            )
    ...
    def mark_as_remembered(self) -> None:
        ...
        if self.response not in [PermissionResponse.ALWAYS, PermissionResponse.NEVER]:
            raise ValueError(
                "Can only mark as remembered if response is ALWAYS or NEVER"
            )
        self.state = PermissionState.REMEMBERED
        self.last_updated = datetime.now(timezone.utc)
    ...
    def is_stale(
        self, current_time: Optional[datetime] = None, stale_threshold: float = 300.0
    ) -> bool:
...
class LLMProviderConfig(BaseModel):
    ...
    default_model: str = Field(
        ..., description="The default model for generation tasks."
    )
    embedding_model: str = Field(
        ..., description="The model used for creating embeddings."
    )
...
class KnowledgeBaseConfig(BaseModel):
    ...
    directory: str = Field(
        ..., description="Path to the root directory for knowledge documents."
    )
    embedding_dimension: int = Field(
        default=768,
        description="Dimension of embedding vectors for the knowledge base.",
    )
...
class RoleManagerConfig(BaseModel):
    ...
    roles_dir: str = Field(
        ..., description="Path to the directory containing role definitions."
    )
...
class WikiConfig(BaseModel):
    ...
    pages_directory: str = Field(
        ..., description="Path to the directory for wiki pages storage."
    )
