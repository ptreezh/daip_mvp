...
from daip_live.doc.models.document_models import PaperMetadata, PaperDownloadResult, PaperSource
...
class PaperDownloader:
    ...
    async def download_paper_by_topic(self, topic: str, source: PaperSource = PaperSource.ARXIV) -> PaperDownloadResult:
    ...
    async def _smart_download(self, topic: str, start_time: float) -> PaperDownloadResult:
    ...
    def _is_arxiv_id(self, topic: str) -> bool:
        ...
        arxiv_pattern = r'(?:arxiv:)?(?:[a-z-]+/\d{4}\.\d{4,5}(v\d+)?)|(?:\d{4}\.\d{4,5}(v\d+)?)'
        return bool(re.match(arxiv_pattern, topic.strip(), re.IGNORECASE))
    
    async def _download_from_arxiv(self, query: str, start_time: float) -> PaperDownloadResult:
    ...
    async def _download_from_arxiv_by_id(self, arxiv_id: str, start_time: float) -> PaperDownloadResult:
    ...
    async def _download_from_pubmed(self, query: str, start_time: float) -> PaperDownloadResult:
        ...
        return PaperDownloadResult(
            ...
            warnings=["PubMed download functionality is planned but not yet implemented"]
        )
    
    async def _download_from_web(self, query: str, start_time: float) -> PaperDownloadResult:
    ...
    async def search_papers(self, query: str, source: PaperSource = PaperSource.ARXIV, max_results: int = 5) -> List[PaperMetadata]:
...
class EnhancedDebatePaperManager:
    ...
    async def download_paper_for_debate(self, topic: str, roles: List[str]) -> PaperDownloadResult:
