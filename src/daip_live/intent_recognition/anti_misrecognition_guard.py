...
class AntiMisrecognitionGuard:
    ...
    def apply_antimisrecognition_protection(
        self, intent_result: Union[Intent, ContextualIntent], context: dict[str, Any]
        ...
        if self._is_misrecognition_risk(intent_result, context):
            self.logger.info(
                f"Misrecognition risk detected for intent: {intent_result.name}"
            )
            # 应用置信度惩罚
    ...
    def _is_misrecognition_risk(
        ...
        non_academic_context = any(
            keyword in current_topic for keyword in non_academic_keywords
        )

        # 检查意图历史是否也支持非学术语境
        non_academic_intents = ["chat", "question", "personal_assistant", "greeting"]
        non_academic_history = any(
            intent in non_academic_intents for intent in intent_history[-3:]
        )
        ...
        if (
            hasattr(intent_result, "parameters")
            and "question" in str(intent_result.parameters).lower()
        ):
            question_indicators = ["找不到", "为什么", "为啥", "如何", "怎么", "怎样"]
            question_in_params = any(
                indicator in str(intent_result.parameters)
    ...
    def _apply_protection_penalty(
        self, intent_result: Union[Intent, ContextualIntent]
    ) -> Union[Intent, ContextualIntent]:
        """
        ...
        if (
            not intent_result
            or not hasattr(intent_result, "name")
            or not hasattr(intent_result, "confidence")
        ):
            return intent_result
        ...
        if intent_result.name in paper_related_intents:
            ...
            adjusted_conf = max(
                original_conf + self.paper_intent_penalty, 0.1
            )  # 最低置信度0.1
            intent_result.confidence = adjusted_conf

            self.logger.info(
                f"Applied penalty to paper intent '{intent_result.name}': {original_conf:.3f} -> {adjusted_conf:.3f}"  # noqa: E501
            )
        ...
        if intent_result.name in chat_related_intents:
            original_conf = getattr(intent_result, "confidence", 0.0)
            adjusted_conf = min(
                original_conf + self.chat_intent_boost, 1.0
            )  # 最高置信度1.0
            intent_result.confidence = adjusted_conf

            self.logger.info(
                f"Applied boost to chat intent '{intent_result.name}': {original_conf:.3f} -> {adjusted_conf:.3f}"  # noqa: E501
            )

        # 检查是否需要将论文意图转换为聊天意图
        if (
            intent_result.name in paper_related_intents
            and intent_result.confidence < 0.4  # 置信度较低
            and self._likely_chat_context(intent_result)
        ):
            # 将意图转换为聊天意图
            intent_result.name = "chat"
            intent_result.intent_type = IntentType.CHAT
            intent_result.confidence = max(
                intent_result.confidence + 0.2, 0.5
            )  # 提升到中等置信度
            intent_result.tool_name = None

            self.logger.info(
                "Converted paper intent to chat intent due to low confidence and chat context"  # noqa: E501
            )

        return intent_result

    def _likely_chat_context(
        self, intent_result: Union[Intent, ContextualIntent]
    ) -> bool:
        """
        判断是否可能是聊天上下文
    ...
    def protect_intent_sequence(
        self, intent_results: list, context: dict[str, Any]
    ) -> list:
        """
        对一系列意图识别结果应用防误识别保护
        ...
        for intent_result in intent_results:
            protected_result = self.apply_antimisrecognition_protection(
    ...
    def validate_protection_rules(self, test_cases: list) -> dict[str, Any]:
        ...
        for test_case in test_cases:
            ...
            mock_intent = Intent(
                ...
                if "chat" in actual_intent
                else IntentType.WORKFLOW,
            )

            protected_intent = self.apply_antimisrecognition_protection(
                mock_intent, context
            )
        ...
        results["accuracy"] = results["correctly_protected"] / max(
    ...
    def check_core_use_case(self, text: str) -> bool:
        ...
        protected_intent = self.apply_antimisrecognition_protection(
            mock_paper_intent, context
        )

        # 检查是否成功将论文意图转换为其他意图
        is_protected = (
            protected_intent.name != "download_paper"
            or protected_intent.confidence < 0.5
        )

        self.logger.info(
            f"Core use case check for '{text}': {'PROTECTED' if is_protected else 'NOT PROTECTED'}"  # noqa: E501
        )
        return is_protected
