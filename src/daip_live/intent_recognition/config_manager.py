...
class ConfigManager:
    ...
    def load_config(self) -> EnhancedIntentSystemConfig:
        ...
        if not os.path.exists(self.config_path):
            self.logger.info(f"Config file {self.config_path} not found, using default config")
    ...
    def update_multiple_configs(self, config_updates: Dict[str, Any]) -> Dict[str, bool]:
        """
        批量更新多个配置项
...
class DynamicConfigurableIntentSystemMixin:
    ...
    def _update_component_configs(self):
        ...
        self.misrecognition_protection_enabled = self.config.misrecognition_protection_enabled
        self.semantic_disambiguation_enabled = self.config.semantic_disambiguation_enabled
...
def example_usage():
    ...
    print(f"Current response time threshold: {config.response_time_threshold}")
    ...
    if success:
        ...
        print(f"New response time threshold: {new_config.response_time_threshold}")
        ...
        if diff:
            ...
            for key, values in diff.items():
                print(f"  {key}: {values['default']} -> {values['current']}")
    else:
