...
class ContextInjector:
    ...
    def inject_context_to_padatious_query(self, text: str, session_id: str = "default") -> str:
        """
        ...
        if injection_time > 0.02:  # 如果注入时间超过20ms，记录警告
            self.logger.warning(f"Context injection took {injection_time:.3f}s for session {session_id}")
        
        self.logger.debug(f"Context injected for session {session_id}: '{text}' -> '{injected_text}'")
    ...
    def _inject_context(self, text: str, context: Dict[str, Any]) -> str:
        ...
        for param_name, param_value in list(parameters.items())[-3:]:  # 只使用最近3个参数
    ...
    def format_context_for_padatious(self, context: Dict[str, Any]) -> str:
        ...
        intent_history = context.get('intent_history', [])
        if intent_history:
            formatted_parts.append(f"HISTORY: {' -> '.join(intent_history[-5:])}")  # 最近5个意图

        return " | ".join(formatted_parts)

    def inject_context_selectively(self, text: str, session_id: str, 
                                 injection_types: List[str] = None) -> str:
        """
        选择性地注入特定类型的上下文
    ...
    def get_context_summary(self, session_id: str) -> str:
        ...
        summary_parts = [
            ...
            f"Intent History: {', '.join(context.get('intent_history', [])[-3:]) or 'N/A'}",
    ...
    def validate_injection_performance(self, test_queries: List[str], session_id: str = "test") -> Dict[str, Any]:
        """
        验证注入性能是否符合要求（≤20ms）
        ...
        if results['total_queries'] > 0:
            ...
            results['performance_ok'] = results['within_threshold'] / results['total_queries'] >= 0.95  # 95%的查询应在阈值内

        results['min_time'] = results['min_time'] if results['min_time'] != float('inf') else 0
