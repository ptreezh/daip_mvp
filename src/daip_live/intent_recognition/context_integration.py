...
class ContextAwareEnhancedRecognizer:
    ...
    def recognize_intent_with_context(self, text: str, session_id: str = "default") -> Optional[Intent]:
        """
        使用上下文感知的意图识别
    ...
    def _handle_wiki_continuation(self, text: str, session_id: str, context: Dict[str, Any]) -> Optional[Intent]:
        """处理Wiki创建任务的延续"""
        # 获取当前任务的缺失参数
                ...
                if hasattr(task, 'get_missing_params'):
                    ...
                    if missing_params:
                        ...
                        if hasattr(self.context_manager, 'add_task_parameter'):
                            self.context_manager.add_task_parameter(session_id, first_missing, text)
                        ...
                        if hasattr(task, 'is_complete') and task.is_complete():
                            ...
                            print(f"✅ Wiki任务参数已完整: {task.filled_params}")
    ...
    def _handle_paper_continuation(self, text: str, session_id: str, context: Dict[str, Any]) -> Optional[Intent]:
        """处理论文下载任务的延续"""
        # 获取当前任务的缺失参数
        if hasattr(self.context_manager, 'get_session_state'):
            session_state = self.context_manager.get_session_state(session_id)
            if session_state and hasattr(session_state, 'current_task'):
                ...
                if hasattr(task, 'get_missing_params'):
                    ...
                    if missing_params:
                        ...
                        if hasattr(task, 'is_complete') and task.is_complete():
                            intent.task_completed = True
                            print(f"✅ 论文任务参数已完整: {task.filled_params}")
    ...
    def start_task_session(self, session_id: str, task_type: str, required_params: list = None, initial_params: dict = None):
        """
        开始一个任务会话
        ...
        # 使用上下文管理器启动任务
        if hasattr(self.context_manager, 'set_context') and hasattr(self.context_manager, 'add_task_parameter'):
            context_data = {
                "task_type": task_type,
        ...
        print(f"🎯 为会话 {session_id} 开始 {task_type} 任务")
