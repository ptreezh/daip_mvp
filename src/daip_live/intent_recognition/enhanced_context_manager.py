...
try:
    ...
    from .contextual_intent_recognizer import (  # noqa: F401
        ContextualIntent,
        ConversationTurn,
    )
...
class ConversationContext:
    ...
    def is_expired(self) -> bool:
        """检查会话是否过期"""
        return datetime.now() - self.last_accessed > timedelta(
...
class EnhancedContextManager:
    ...
    def update_conversation_context(self, session_id: str, intent: ContextualIntent):
        ...
        if not context:
            # 创建新的上下文
            topic = intent.filled_slots.get("topic", "") or intent.inferred_params.get(
                "topic", ""
            )
            context = self.create_conversation_context(
                session_id, intent.intent.name if intent.intent else "", topic
            )
        ...
        if intent.intent:
            ...
            if (
                not context.intent_history
                or context.intent_history[-1] != intent.intent.name
            ):
        ...
        for param_name, param_value in intent.inferred_params.items():
            context.set_parameter(
                param_name, param_value, ParameterSource.INFERENCE, confidence=0.8
            )

        # 同步到基础管理器
        self._sync_to_base_manager(session_id, intent)

    def add_parameter_with_source(
        self,
        session_id: str,
        param_name: str,
        param_value: Any,
        ...
        conversation_context.set_parameter(
            param_name, param_value, source, confidence, context
        )
    ...
    def get_parameter_with_metadata(
        self, session_id: str, param_name: str
    ) -> Optional[ParameterMetadata]:
        """获取带元数据的参数"""
        context = self.get_conversation_context(session_id)
        if context:
    ...
    def inherit_parameters(
        self, session_id: str, target_intent: str, required_params: list[str]
    ) -> dict[str, Any]:
        """从上下文继承参数"""
        inherited = {}
        context = self.get_conversation_context(session_id)
                ...
                if param in rules:
                    ...
                    if "from_intent" in inheritance_rule:
                        ...
                        if source_intent in context.intent_history:
                            # 查找历史上下文中的参数
                            inherited_value = self._find_historical_parameter(
                                session_id, source_intent, param
                            )
                            if inherited_value is not None:
                                ...
                                context.set_parameter(
                                    param,
                                    inherited_value,
                                    ParameterSource.CONTEXT_INHERIT,
                                )
                                continue
                    ...
                    if "from_param" in inheritance_rule:
                        ...
                        if source_meta:
                            inherited_value = self._transform_parameter(
                                source_meta.value, param, inheritance_rule
                            )
                            if inherited_value is not None:
                                inherited[param] = inherited_value
                                context.set_parameter(
                    ...
                    if "default" in inheritance_rule:
                        ...
                        context.set_parameter(
                            param, default_value, ParameterSource.DEFAULT
                        )

        return inherited

    def _find_historical_parameter(
        self, session_id: str, intent_name: str, param_name: str
    ) -> Optional[Any]:
        """查找历史参数"""
        context = self.get_conversation_context(session_id)
        if not context:
    ...
    def _transform_parameter(
        self, source_value: Any, target_param: str, rule: dict[str, Any]
    ) -> Any:
        """参数转换"""
    ...
    def _sync_to_base_manager(self, session_id: str, intent: ContextualIntent):
        ...
        if intent.intent and intent.intent.name:
            ...
            # 添加所有已填充的参数
            for param_name, param_value in intent.filled_slots.items():
                self.base_manager.add_task_parameter(
    ...
    def is_in_task(self, session_id: str) -> bool:
        ...
        context = self.get_conversation_context(session_id)
        if context:
            return base_check or (
                context.current_intent != "" and context.status == SessionStatus.ACTIVE
            )
    ...
    def add_task_parameter(
        ...
        """添加任务参数（兼容原接口）"""
        # 添加到增强上下文
        success = self.add_parameter_with_source(
            session_id, param_name, param_value, ParameterSource.USER_INPUT
        )

        # 添加到基础管理器
        if self.base_manager.is_in_task(session_id):
            return self.base_manager.add_task_parameter(
                session_id, param_name, param_value
            )
    ...
    def _initialize_inheritance_rules(self) -> dict[str, dict[str, dict[str, Any]]]:
        ...
        return {
            "start_debate": {
                "topic": {"from_param": "title", "transform": "identity"},
                "rounds": {"default": 3, "transform": "int"},
            },
            "create_wiki": {
                "title": {
            ...
            "download_paper": {
                ...
                "paper_id": {"from_intent": "search_papers", "transform": "identity"},
            },
            "execute_skill": {
                "content": {
                    "from_intent": "search_papers",
    ...
    def export_context(self, session_id: str) -> Optional[dict[str, Any]]:
        ...
        if context:
            return {
                ...
                "intent_history": context.intent_history,
                "parameters": {
                    name: asdict(meta) for name, meta in context.parameters.items()
                },
                "status": context.status.value,
        ...
        try:
            ...
            context = ConversationContext(
                ...
                last_accessed=datetime.fromisoformat(
                    context_data.get("last_accessed", datetime.now().isoformat())
                ),
