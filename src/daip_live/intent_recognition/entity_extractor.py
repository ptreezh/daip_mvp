...
class EntityExtractor:
    ...
    def extract_entities_from_context(self, session_id: str) -> List[Entity]:
        ...
        if self.context_manager:
            try:
                ...
                if conversation_context:
                    # 从参数中提取实体
                    for param_name, param_value in conversation_context.get_filled_parameters().items():
                    ...
                    for param_name, meta in conversation_context.parameters.items():
                        entities.append(Entity(
                            ...
                            entity_type=meta.source.value if hasattr(meta.source, 'value') else 'parameter'
                        ))
            except Exception as e:
                self.logger.warning(f"Failed to extract entities from EnhancedContextManager: {e}")

        # 从会话历史中提取实体
        if self.session_manager:
            try:
                session_history_entities = self._extract_entities_from_session_history(session_id)
                entities.extend(session_history_entities)
            except Exception as e:
                self.logger.warning(f"Failed to extract entities from session history: {e}")
        ...
        if self.context_manager:
            try:
                ...
                self.logger.warning(f"Failed to extract entities from base context: {e}")
    ...
    def _extract_entities_from_session_history(self, session_id: str) -> List[Entity]:
        ...
        if hasattr(self.session_manager, 'get_session_context'):
            try:
                ...
                self.logger.warning(f"Failed to extract entities from universal session context: {e}")
        ...
        try:
            ...
            self.logger.warning(f"Failed to extract entities from traditional session: {e}")
    ...
    def _extract_entities_from_text(self, text: str) -> List[Entity]:
        ...
        academic_keywords = ['算法', '模型', '系统', '方法', '技术', '理论', '应用', '研究', '分析', '设计', '实现']
    ...
    def _infer_entity_type(self, param_name: str, param_value: Any) -> str:
        ...
        if any(keyword in name_lower for keyword in ['paper', 'id', 'arxiv']):
            ...
            return 'number'
        elif any(keyword in name_lower for keyword in ['content', 'text', 'description']):
            return 'content'
    ...
    def extract_entities_by_type(self, session_id: str, entity_type: str) -> List[Entity]:
        """
        按类型提取实体
    ...
    def find_most_recent_entity(self, session_id: str, entity_type: str) -> Optional[Entity]:
        """
        查找最近的指定类型实体
