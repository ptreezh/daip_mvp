...
class IntentRecognitionErrorHandler:
    ...
    def _log_error(self, error_info: ErrorInfo):
        """记录错误信息"""
        error_msg = (
            f"Error in {error_info.component}: {error_info.error_type} - {error_info.error_message}"
        )
        ...
        if error_info.traceback_info:
            self.logger.debug(f"Full traceback for {error_info.component} error:\n{error_info.traceback_info}")
    ...
    def _attempt_recovery(self, error_info: ErrorInfo, fallback_result: Any) -> Any:
        ...
        recovery_msg = f"Recovered from {error_info.error_type} in {error_info.component}, using fallback"
        if error_info.session_id:
...
def error_handler_decorator(error_handler: IntentRecognitionErrorHandler, 
                          component_name: str,
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                if args and hasattr(args[0], '__class__') and hasattr(args[0], '__dict__'):
                    # 如果是类方法，可能在其他参数中有输入
                    for arg in args[1:]:
                        if isinstance(arg, str) and len(arg) < 200:  # 假设字符串参数是输入
                            original_input = arg
                            break
...
class IntentRecognitionLogger:
    ...
    def __init__(self, logger_name: str = "intent_recognition", log_level: int = logging.INFO):
        self.logger = logging.getLogger(logger_name)
    ...
    def log_intent_recognition_start(self, user_input: str, session_id: str):
        """记录意图识别开始"""
        self.logger.debug(f"Starting intent recognition for session {session_id}: '{user_input}'")
    
    def log_intent_recognition_result(self, intent_name: str, confidence: float, session_id: str, processing_time: float = None):
        """记录意图识别结果"""
        time_info = f" (time: {processing_time*1000:.2f}ms)" if processing_time else ""
        self.logger.debug(f"Intent recognition result for session {session_id}: {intent_name} (confidence: {confidence:.3f}){time_info}")
    ...
    def log_performance_warning(self, operation: str, duration: float, threshold: float, session_id: str = None):
        """记录性能警告"""
        session_info = f" (session {session_id})" if session_id else ""
        self.logger.warning(f"Performance warning: {operation} took {duration*1000:.2f}ms, exceeding threshold of {threshold*1000:.2f}ms{session_info}")
...
def setup_intent_recognition_logging(log_file: Optional[str] = None, 
...
def example_usage():
    ...
    # 使用装饰器的示例函数
    @error_handler_decorator(error_handler, "example_component", fallback_result="fallback")
    ...
    result = example_function("session1", "normal input")
    print(f"Result: {result}")
    
    # 测试错误情况
    print("\nTesting error handling:")
    result = example_function("session2", "input with error")
