...
class IntentFuser:
    ...
    def fuse_intents(self, padatious_result: Optional[PadatiousResult],
    ...
    def _adjust_confidence_with_context(self, intent: Union[Intent, ContextualIntent],
        ...
        for signal_name, signal_weight in context_signals.items():
            ...
            elif signal_name.startswith('entity_') and signal_name.endswith('_relevance'):
                # 实体相关性高，增加置信度
    ...
    def _perform_intent_fusion(self, padatious_result: PadatiousResult,
        ...
        fused_parameters = self._fuse_parameters(
            ...
            getattr(original_result, 'parameters', {})
        )

        # 根据上下文选择主要意图类型
        final_intent_type = self._select_intent_type(padatious_result.name, original_result.intent_type, context)

        # 根据置信度和上下文选择最终意图名称
        final_intent_name = self._select_intent_name(
            padatious_result.name,
        ...
        if isinstance(original_result, ContextualIntent):
            ...
            if not isinstance(final_intent, ContextualIntent):
                ...
                final_intent = ContextualIntent(
                    ...
                    clarification_needed=getattr(original_result, 'clarification_needed', False),
                    clarification_message=getattr(original_result, 'clarification_message', ''),
    ...
    def _fuse_parameters(self, padatious_params: Dict[str, str],
    ...
    def _select_intent_type(self, padatious_intent: str,
                           original_intent_type: IntentType,
                           context: Dict[str, Any]) -> IntentType:
        """
        根据上下文选择意图类型
        """
        # 默认使用原有意图类型
        ...
        # 但如果上下文表明需要更改为其他类型
        if padatious_intent in ['general_chat', 'question_ask'] and 'chat' in original_intent_type.name.lower():
            selected_type = IntentType.CHAT
        elif padatious_intent in ['search_request', 'paper_download'] and 'workflow' not in original_intent_type.name.lower():
            selected_type = IntentType.WORKFLOW
        elif padatious_intent == 'question_ask' and 'question' not in original_intent_type.name.lower():
            selected_type = IntentType.QUESTION

        return selected_type

    def _select_intent_name(self, padatious_intent: str,
                           original_intent: str,
        ...
        if padatious_intent in ['paper_download', 'search_request'] and non_academic_context:
            return original_intent

        # 如果Padatious置信度明显更高，选择Padatious意图
        if padatious_intent != original_intent:
            # 检查是否是误识别
            is_likely_misrecognition = self._is_likely_misrecognition(padatious_intent, context)
            if is_likely_misrecognition:
                return original_intent
            else:
                # 在置信度显著更高时选择Padatious意图
                padatious_conf = context.get('padatious_result', {}).get('confidence', 0)
    ...
    def _is_likely_misrecognition(self, intent_name: str, context: Dict[str, Any]) -> bool:
        """
    ...
    def _map_intent_type(self, padatious_intent: str) -> IntentType:
        ...
        elif padatious_intent in ['search_request', 'paper_download', 'debate_start', 'wiki_create', 'skill_execute']:
