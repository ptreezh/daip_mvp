...
from daip_live.intent_recognition.models.intent_result import (
    IntentRecognitionResult, IntentType, DebateHistoryIntent, DocumentConversionIntent,
    WikiManagementIntent, PaperDownloadIntent, SessionManagementIntent,
    RoleManagementIntent, ModelManagementIntent
)
from daip_live.intent_recognition.patterns import (
    DEBATE_HISTORY_PATTERNS, DOCUMENT_CONVERSION_PATTERNS, WIKI_MANAGEMENT_PATTERNS,
    PAPER_DOWNLOAD_PATTERNS, SESSION_MANAGEMENT_PATTERNS, ROLE_MANAGEMENT_PATTERNS,
...
class IntentParser:
            ...
            for action, pattern_list in patterns_by_action.items():
                for pattern in pattern_list:
                    ...
                    if matches:
                        ...
                        confidence = self._calculate_confidence(pattern, user_input_lower, matches)
    ...
    def _calculate_confidence(self, pattern: str, input_text: str, match_obj) -> float:
        ...
        # Adjust based on pattern complexity (more specific patterns get higher confidence)
    ...
    def create_intent_object(self, result: IntentRecognitionResult) -> Optional[Any]:
        ...
        if result.intent_type == IntentType.DEBATE_HISTORY:
            ...
            if "session_id" in result.extracted_parameters:
                ...
                action = "search"
            elif result.matched_pattern in DEBATE_HISTORY_PATTERNS["show_specific_debate"]:
                action = "view"
            else:
                action = "list"
            ...
            if result.matched_pattern in SESSION_MANAGEMENT_PATTERNS["view_specific_session"]:
                action = "view"
            elif result.matched_pattern in SESSION_MANAGEMENT_PATTERNS["clear_sessions"]:
                action = "clear"
            else:
                action = "list"
            ...
            if result.matched_pattern in ROLE_MANAGEMENT_PATTERNS.get("view_role", []) or result.matched_pattern in ROLE_MANAGEMENT_PATTERNS.get("list_roles", []):
                action = "view" if result.matched_pattern in ROLE_MANAGEMENT_PATTERNS.get("view_role", []) else "list"
            else:
                action = "list"
                
            return RoleManagementIntent(
                action=action,
                role_name=result.extracted_parameters.get("identifier")  # Using identifier from extract_parameters
            )
        
        elif result.intent_type == IntentType.MODEL_MANAGEMENT:
            if result.matched_pattern in MODEL_MANAGEMENT_PATTERNS.get("switch_model", []):
                action = "switch"
            else:
                action = "list"
            ...
            return ModelManagementIntent(
                action=action,
                model_name=result.extracted_parameters.get("identifier")  # Using identifier from extract_parameters
            )
...
class EnhancedIntentParser(IntentParser):
    ...
    def recognize_intent_with_context(self, user_input: str) -> List[IntentRecognitionResult]:
    ...
    def _context_suggests_intent(self, intent_type: IntentType) -> bool:
        ...
        if intent_type == IntentType.DEBATE_HISTORY:
            return any("debate" in topic.lower() or "history" in topic.lower() for topic in context_clues)
        elif intent_type == IntentType.WIKI_MANAGEMENT:
            return any("wiki" in topic.lower() or "knowledge" in topic.lower() for topic in context_clues)
        elif intent_type == IntentType.PAPER_DOWNLOAD:
            return any("paper" in topic.lower() or "research" in topic.lower() or "article" in topic.lower() for topic in context_clues)
        elif intent_type == IntentType.SESSION_MANAGEMENT:
            return any("session" in topic.lower() or "history" in topic.lower() for topic in context_clues)
