...
class PadatiousEnhancedIntentRecognizer:
    ...
    def __init__(
        ...
        if not PADATIOUS_AVAILABLE:
            raise ImportError(
                "Padatious is not installed. Please install it with 'pip install padatious'"  # noqa: E501
            )
    ...
    def _add_intent(self, name: str, examples: list):
        ...
        for example in examples:
            try:
                ...
                self.logger.warning(
                    f"Could not add intent {name} with example '{example}': {e}"
                )
    ...
    def _recognize_with_padatious(
        self, text: str, context: dict[str, Any]
    ) -> Optional[PadatiousResult]:
        """
        使用Padatious进行语义意图识别
        ...
        try:
            ...
            if context and self.context_integrator:
                enriched_text = self.context_integrator.inject_context_to_query(
                    text, context
                )
    ...
    def _convert_padatious_result(
        self, padatious_result, original_text: str, context: dict[str, Any]
    ) -> PadatiousResult:
        """
        将Padatious结果转换为系统标准格式
    ...
    def _adjust_result_with_context(
        self, result: Optional[Union[Intent, ContextualIntent]], context: dict[str, Any]
    ) -> Optional[Union[Intent, ContextualIntent]]:
        """使用上下文调整意图结果"""
    ...
    def _convert_padatious_to_system_intent(
        ...
        if padatious_result.name == "chat":
            ...
            content = parameters.get(
                "content", parameters.get("task", padatious_result.source_text)
            )
            parameters["content"] = content

        # 确定意图类型
        intent_type = IntentType.CHAT
        if padatious_result.name in [
            "search_papers",
            "download_paper",
            "start_debate",
            "create_wiki",
    ...
    def _perform_intent_fusion(
        ...
        if fused_confidence > 0.6:  # 使用融合后置信度作为判断标准
            # 基于上下文选择更好的意图
            if self._should_prefer_padatious(
                padatious_result, original_result, context
            ):
                ...
                final_intent = Intent(
                    ...
                    tool_name=getattr(
                        original_result,
                        "tool_name",
                        self._get_tool_for_intent(padatious_result.name),
                    ),
                    description=getattr(
                        original_result,
                        "description",
                        f"Fused intent: {padatious_result.name}",
                    ),
                    intent_type=getattr(
                        original_result, "intent_type", IntentType.CHAT
                    ),
    ...
    def _should_prefer_padatious(
        ...
        if (
            padatious_result.name == "download_paper"
            or original_result.name == "download_paper"
        ):
            # 检查上下文话题
        ...
        context_signals = context.get("context_signals", {})
        if any(
            "entity" in signal and signal.endswith("_relevance")
