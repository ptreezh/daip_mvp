...
class RoleIntelligenceSelector:
    ...
    def analyze_topic_for_roles(self, topic: str, max_roles: int = 4) -> list[str]:
        ...
        # 根据主题类型获取基础角色列表
        base_roles = self.topic_role_mapping.get(
            topic_type, ["domain_expert", "researcher", "editor"]
        )
    ...
    def get_role_recommendation_reason(
        self, topic: str, selected_roles: list[str]
    ) -> dict[str, str]:
        """获取角色推荐的理由
    ...
    def enhance_role_selection_with_context(
        self, topic: str, context: Optional[dict[str, Any]] = None, max_roles: int = 4
    ) -> list[str]:
        """基于上下文增强角色选择
