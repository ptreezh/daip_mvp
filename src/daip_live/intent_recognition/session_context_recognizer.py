...
class SessionContextAwareRecognizer:
    ...
    def analyze_session_context(self, current_input: str) -> Dict[str, Any]:
        ...
        # 检测Wiki编辑上下文
        wiki_related = any('wiki' in str(item.get('intent_result', '')).lower() 
        ...
        # 检测持续的讨论话题
        continuity_pattern = re.compile('|'.join([
            r'继续', r'接着', r'然后', r'往下', r'继续说', r'接着说', 
            r'more', r'continue', r'next', r'further'
        ]), re.IGNORECASE)
    ...
    def recognize_intent_with_context(self, user_input: str) -> Any:
        ...
        if self.current_context.get('maintain_wiki_context'):
            ...
            if base_intent and hasattr(base_intent, 'confidence'):
                # 检查是否已经有Wiki意图
                is_wiki_intent = any('wiki' in str(getattr(intent, 'name', '')).lower() 
                                    for intent in ([base_intent] if hasattr(base_intent, 'name') 
                                                  else base_intent if isinstance(base_intent, list) else [base_intent]))
                
                if not is_wiki_intent:
                    # 检查输入中是否包含Wiki相关词汇
                    wiki_indicators = ['词条', '编辑', 'wiki', '页面', '知识', '文档', '条目']
