...
class TestEnhancedIntentSystemEndToEnd(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.system = IntegratedIntentSystem(
            enable_context_aware=True, enable_debug=True, enable_enhanced_features=True
        )
    ...
    def test_complete_wiki_creation_flow(self):
        ...
        # 2. 系统应能识别后续的相关操作
        intent2 = self.system.recognize_intent(
            "添加内容 人工智能起源于1950年代", session_id
        )
    ...
    def test_complete_debate_start_flow(self):
        ...
        # 1. 用户启动辩论
        intent1 = self.system.recognize_intent(
            "开始辩论 人工智能的伦理问题", session_id
        )
    ...
    def test_misrecognition_protection_end_to_end(self):
        ...
        self.assertNotIn(
            ...
            f"核心测试用例失败：输入 '你好啊，为啥找不到roles' 被错误地识别为 {intent.name}",  # noqa: E501
        )
        ...
        self.assertGreaterEqual(
            self.system.recognition_stats["misrecognition_protection_hits"], 0
        )
    ...
    def test_multi_intent_handling(self):
        ...
        # 提供一个可能触发多个意图的复杂输入
        intent = self.system.recognize_intent(
            "帮我创建关于机器学习的维基页面，同时搜索相关论文", session_id
        )
    ...
    def test_parameter_inference_and_completion(self):
        ...
        self.system.recognize_intent("开始辩论", session_id)
        # 系统应识别缺少主题参数

        # 提供缺失参数
        self.system.recognize_intent("辩论主题是自动驾驶", session_id)
        # 系统应能将此作为对前一个请求的参数补充
    ...
    def test_system_statistics_and_monitoring(self):
        ...
        # 验证统计信息被正确更新
        self.assertGreaterEqual(
            final_stats["total_requests"], initial_stats["total_requests"] + 5
        )
    ...
    def test_session_lifecycle_management(self):
        ...
        # 检查缺失参数
        self.system.get_missing_parameters(session_id)
        # 根据具体任务，可能会有其他必需参数
