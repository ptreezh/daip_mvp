...
{original_query}_技术"""
            ...
            print(f"[PAPER EXPANSION] {original_query} -> {keywords[:5]}...")
            return keywords[:10]  # 限制为最多10个关键词
            
        except Exception as e:
            print(f"[PAPER EXPANSION] 关键词扩展失败: {e}")
            # 备选方案：简单的关键词扩展
            return [original_query]
    
    async def search_papers_multiple_sources(self, keywords: List[str], max_results: int = 5) -> List[PaperInfo]:
        """
        在多个源上搜索论文
        """
        all_papers = []
        ...
        for keyword in keywords[:3]:  # 只搜索前3个关键词，避免搜索太多
            print(f"[PAPER SEARCH] 搜索关键词: '{keyword}'...")
        ...
        print(f"[PAPER SEARCH] 总计找到 {len(all_papers)} 篇论文")
        return all_papers
    
    async def _simulate_arxiv_search(self, query: str, max_results: int = 5) -> List[PaperInfo]:
        ...
        for i in range(min(max_results, 3)):  # 最多模拟3篇论文
            paper = PaperInfo(
                ...
                authors=[f"Author_{random.choice(['A','B','C'])}", f"Author_{random.choice(['D','E','F'])}"],
                abstract=f"这篇论文探讨了{query}的多个方面，包括技术细节、应用案例和未来发展方向。研究显示{query}在现代技术发展中具有重要作用。",
                url=f"https://arxiv.org/abs/{random.randint(1000, 9999)}.{random.randint(1000, 9999)}",
                source="arxiv",
                published_date=f"{random.randint(2020, 2024)}-{random.randint(1, 12):02d}-{random.randint(1, 28):02d}",
    ...
    async def select_papers_for_download(self, search_results: List[PaperInfo]) -> List[PaperInfo]:
        """
        选择要下载的论文
        基于相关性排序和用户选择
        """
        ...
        if self.search_history:
            ...
            search_results.sort(key=lambda p: calculate_relevance(p, original_query), reverse=True)
        
        # 返回排名靠前的论文
        selected = search_results[:min(3, len(search_results))]  # 选择前3篇
        print(f"[PAPER SELECTION] 从 {len(search_results)} 篇中选择了 {len(selected)} 篇进行下载")
        
        return selected
    
    async def download_selected_papers(self, papers: List[PaperInfo]) -> List[Dict[str, Any]]:
        """
        下载选中的论文
        """
        download_results = []
        
        print(f"[PAPER DOWNLOAD] 开始下载 {len(papers)} 篇论文...")
        
        for i, paper in enumerate(papers):
            print(f"  [{i+1}/{len(papers)}] 下载: {paper.title[:50]}...")
    ...
    async def search_and_download_pipeline(self, query: str) -> Dict[str, Any]:
        ...
        从关键词扩展到最终下载的完整连续流程
        """
        print(f"[PIPELINE] 🔍 开始执行论文搜索下载流水线: '{query}'")
        
        start_time = datetime.now()
        
        # 步骤1: 扩展关键词
        print("[PIPELINE] 步骤1: 扩展搜索关键词...")
        expanded_keywords = await self.expand_search_keywords_with_llm(query)
        print(f"  扩展关键词: {expanded_keywords[:5]}...")  # 只显示前5个
        
        # 步骤2: 多源搜索
        print("[PIPELINE] 步骤2: 执行多源搜索...")
        search_results = await self.search_papers_multiple_sources(expanded_keywords, max_results=5)
        ...
        print(f"[PIPELINE] 🎉 流水线完成: 成功下载 {success_count}/{total_count} 篇论文，耗时 {duration:.2f}秒")
        
        return result


async def integrate_with_intent_recognizer(recognizer, paper_system=None, model_provider=None):
    """
    将论文搜索下载系统集成到意图识别器
    """
    if not paper_system:
    ...
    print("[INTEGRATION] 高级论文搜索下载系统已集成到意图识别器")
    return paper_system


if __name__ == "__main__":
    print("="*80)
    print("📄 高级论文搜索下载连续流程系统")
    print("="*80)
    
    print("\\n系统组件:")
    print("  - AdvancedPaperSearchDownloadSystem: 核心搜索下载引擎")
    print("  - 关键词扩展: 基于大模型的查询扩展")
    print("  - 多源搜索: 在多个学术源上搜索论文") 
    print("  - 智能选择: 基于相关性选择论文")
    print("  - 批量下载: 并行下载多篇论文")
    print("  - 完整流水线: 从搜索到下载的端到端流程")
