        ...
        try:
            ...
            if isinstance(llm_result, dict) and "intent_name" in llm_result:
                ...
                llm_intent = Intent(
                    ...
                    intent_type=IntentType.WORKFLOW if "work" in llm_result.get("intent_name", "").lower()
                                else IntentType.QUESTION,
                    requires_clarification=llm_result.get("requires_clarification", False)
                )

                print(f"[HYBRID RECOGNIZER] LLM分析: '{text}' -> {llm_intent.name} (置信度: {llm_intent.confidence})")
            ...
            print(f"[HYBRID RECOGNIZER] LLM分析失败: {e}")
            import traceback
            traceback.print_exc()
...
class EnhancedWikiCollaborationEngine:
    ...
    def __init__(self, model_provider=None):
        ...
        self.hybrid_recognizer = HybridIntentRecognizer(llm_model_provider=model_provider)
        self.paper_system = AdvancedPaperSearchDownloadSystem(model_provider=model_provider)
    ...
    async def process_collaborative_wiki_request(self, user_input: str):
        ...
        if intent and "wiki" in intent.name.lower():
            print(f"[WIKI COLLAB PROCESS] 检测到维基协作意图: {intent.name}")
            ...
            if not title:
                # 如果没有明确标题，尝试从用户输入中提取
                title = user_input.replace("创建维基", "").replace("写个维基", "").replace("新建维基", "").strip()
            ...
            print(f"[WIKI COLLAB PROCESS] 协作编辑完成，收到 {len(contributions)} 个贡献")
    ...
    async def process_paper_search_download_request(self, user_input: str):
        ...
        if intent and ("download" in intent.name.lower() or "search" in intent.name.lower() or "paper" in intent.name.lower()):
            print(f"[PAPER PIPELINE] 检测到论文处理意图: {intent.name}")
            
            # 获取查询参数
            search_query = intent.parameters.get("search_query", intent.parameters.get("query", ""))
            if not search_query:
                # 尝试从用户输入中提取
                search_query = user_input.replace("下载论文", "").replace("搜索论文", "").replace("查找论文", "").strip()
            ...
            # 执行搜索-下载流水线
            pipeline_result = await self.paper_system.search_and_download_pipeline(search_query)
...
if __name__ == "__main__":
    print("="*90)
    print("🧠 混合意图识别器与多角色协作系统")
    print("="*90)
    
    print("\\n系统组件:")
    print("  1. HybridIntentRecognizer: 结合规则匹配和大模型分析")
    print("  2. MultiRoleWikiCollaborator: 多角色AI协作编辑器")
    print("  3. AdvancedPaperSearchDownloadSystem: 高级论文搜索下载系统")
    print("  4. EnhancedWikiCollaborationEngine: 集成引擎")
