...
- {original_query}_应用"""
            ...
            print(f"[PAPER SEARCH] 关键词扩展: '{original_query}' -> {[k for k in keywords[:5]]}")
...
async def integrate_paper_search_download_functionality(intent_recognizer):
    ...
    # 创建论文搜索下载系统实例
    paper_system = AdvancedPaperSearchDownloadSystem(model_provider=intent_recognizer.model_provider if hasattr(intent_recognizer, 'model_provider') else None)
