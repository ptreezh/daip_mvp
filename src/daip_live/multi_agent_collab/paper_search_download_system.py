...
- {original_query}_应用"""
            ...
            for line in lines:
                ...
                if line.strip().startswith("- "):
                    ...
                    if keyword and keyword not in original_query and keyword not in keywords:
                        keywords.append(keyword)
                elif "扩展:" not in line and len(line.strip()) > 1 and line.strip() not in keywords:
                    # 简单关键词提取
                    clean_line = line.strip().strip("-").strip("*").strip()
                    if clean_line and len(clean_line) > 1 and clean_line not in keywords and clean_line != original_query:
            ...
            print(f"[PAPER SEARCH] 关键词扩展: '{original_query}' -> {keywords[:5]}...")
            return keywords[:10]  # 限制为最多10个关键词，避免搜索过多
            
        except Exception as e:
            print(f"[PAPER SEARCH] 关键词扩展失败: {e}")
            # 备选方案：简单关键词扩展
            return [original_query, original_query + " 研究", original_query + " 应用"]
    
    async def search_papers_multiple_sources(self, keywords: List[str]) -> List[PaperInfo]:
        """
        在多个源上搜索论文
        返回匹配的论文列表
        """
        ...
        for keyword in keywords[:3]:  # 只处理前3个关键词以避免搜索过多
            print(f"[PAPER SEARCH] 在arXiv搜索: '{keyword}'")
        ...
        print(f"[PAPER SEARCH] 搜索完成，找到 {len(all_papers)} 篇相关论文")
        return all_papers
    ...
    async def _simulate_arxiv_search(self, query: str) -> List[PaperInfo]:
        ...
        mock_papers = [
            PaperInfo(
                ...
                abstract=f"这篇论文研究了 {query} 的各个方面，包括方法学、应用场景和未来发展方向。",
    ...
    async def generate_download_instructions(self, papers: List[PaperInfo]) -> List[Dict[str, Any]]:
        """
        为搜索结果生成下载指令
        """
        ...
        print(f"[PAPER DOWNLOAD] 生成了 {len(download_instructions)} 个下载指令")
        return download_instructions
    
    async def execute_download_batch(self, download_instructions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        批量执行下载指令
        """
        download_results = []
        
        print(f"[PAPER DOWNLOAD] 开始批量下载 {len(download_instructions)} 篇论文...")
        
        for i, instruction in enumerate(download_instructions):
            print(f"  [{i+1}/{len(download_instructions)}] 下载: {instruction['title'][:50]}...")
            ...
            try:
                ...
                print(f"     ✅ 下载成功: {instruction['save_path']}")
        ...
        print(f"[PAPER DOWNLOAD] 批量下载完成: {len([r for r in download_results if r['success']])}/{len(download_results)} 成功")
    ...
    async def search_and_download_pipeline(self, query: str) -> Dict[str, Any]:
        ...
        # 步骤1: 使用大模型扩展关键词
        print("[PIPELINE] Step 1: 使用大模型扩展搜索关键词...")
        expanded_keywords = await self.expand_search_keywords_with_llm(query)
        
        # 步骤2: 多源搜索论文
        print("[PIPELINE] Step 2: 在多个源上搜索论文...")
        search_results = await self.search_papers_multiple_sources(expanded_keywords)
        
        # 步骤3: 生成下载指令
        print("[PIPELINE] Step 3: 生成论文下载指令...")
        download_instructions = await self.generate_download_instructions(search_results)
        
        # 步骤4: 执行批量下载
        print("[PIPELINE] Step 4: 执行批量下载...")
        ...
        print(f"[PIPELINE] 🎉 流水线完成: 成功下载 {success_count}/{total_count} 篇论文")
        return result
...
async def integrate_paper_search_download_functionality(intent_recognizer):
    ...
    将高级论文搜索下载功能集成到意图识别器
    """
    print("🔧 集成高级论文搜索下载功能到意图识别器")
    
    # 创建论文搜索下载系统实例
    paper_system = AdvancedPaperSearchDownloadSystem(model_provider=getattr(intent_recognizer, 'model_provider', None))
...
if __name__ == "__main__":
    ...
    print("\\n系统功能:")
    print("  1. 使用大模型分析和扩展搜索关键词")
    print("  2. 多源搜索获取相关论文") 
    print("  3. 生成批量下载指令")
    print("  4. 执行下载并返回结果")
    print("  5. 提供完整流水线视图")
    
    print("\\n✅ 系统已准备就绪，支持完整的搜索-下载连续流程!")