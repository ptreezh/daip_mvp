...
请编辑这个部分的内容。""",  # noqa: E501
        ...
        for participant in participants:
            if participant in self.role_prompts:
                ...
                self.participants[participant] = {
                    ...
                    "prompt_template": "请为{topic}主题提供有价值的贡献。当前内容: {current_content}",  # noqa: E501
    ...
    async def generate_content_with_role(
        ...
        if self.model_provider:
            ...
            # 模拟模型响应用于测试
            return f"[模拟内容] {role_name} 贡献了关于 '{self.title}' 的内容到 '{section}' 部分"  # noqa: E501
    ...
    async def run_collaborative_editing_round(self, sections_to_edit: list[str] = None):
        ...
        if sections_to_edit is None:
            sections_to_edit = (
                list(self.content.keys()) if self.content else ["overview"]
            )
        ...
        for section in sections_to_edit:
            for role_name in self.participants.keys():
                # 生成角色特定的内容
                new_content = await self.generate_content_with_role(
                    role_name, section, self.content.get(section, "")
                )
    ...
    async def end_collaboration(self) -> dict[str, Any]:
        ...
        result = {
            ...
            "final_content_length": sum(
                len(content) for content in self.content.values()
            ),
            "ended_at": datetime.now(),
            "summary": f"协作编辑会话完成，共产生{len(self.contribution_history)}个贡献",  # noqa: E501
        }
    ...
    "explanation": "简短解释为什么这样判断"
        ...
        try:
            if self.model_provider:
                ...
                if any(
                    keyword in lower_input for keyword in ["辩论", "讨论", "争", "辩"]
                ):
                    return {
                        "intent_name": "start_debate",
                        "parameters": {
                            "topic": user_input.replace("辩论", "")
                            .replace("讨论", "")
                            .strip()
                            or ""
                        },
                        "requires_clarification": not user_input.replace("辩论", "")
                        .replace("讨论", "")
                        .strip(),
                        "confidence": 0.85,
                    ...
                    keyword in lower_input for keyword in ["创建", "写", "新建", "编辑"]
                ):
                    if any(
                        keyword in lower_input
                        for keyword in ["维基", "词条", "百科", "wiki"]
                    ):
                        return {
                            "intent_name": "create_wiki",
                            "parameters": {
                                "title": user_input.replace("创建", "")
                                .replace("写", "")
                                .replace("维基", "")
                                .replace("词条", "")
                                .replace("百科", "")
                            ...
                            "requires_clarification": not user_input.replace("创建", "")
                            .replace("写", "")
                            .replace("维基", "")
                            .replace("词条", "")
                            .replace("百科", "")
                    ...
                    keyword in lower_input for keyword in ["搜索", "查找", "找", "检索"]
                ):
                    if any(
                        keyword in lower_input
                        for keyword in ["论文", "文献", "arxiv", "学术"]
                    ):
                        return {
                            ...
                            "requires_clarification": not user_input.replace("搜索", "")
                            .replace("查找", "")
                            .replace("论文", "")
                            .replace("文献", "")
                            .replace("学术", "")
                    ...
                    if any(
                        keyword in lower_input
                        for keyword in ["论文", "文献", "arxiv", "article"]
                    ):
                        # 检查是否包含ID模式
                        import re
                        ...
                        if id_match:
                            ...
                            return {
                                "intent_name": "download_paper",
                                "parameters": {
                                    "search_query": user_input.replace("下载", "")
                                    .replace("获取", "")
                                    .replace("论文", "")
                                    .replace("文献", "")
                                    .strip()
                                },
                                "requires_clarification": not user_input.replace(
                                    "下载", ""
                                )
                                .replace("获取", "")
                                .replace("论文", "")
    ...
    def _simulate_llm_analysis(self, user_input: str) -> dict[str, Any]:
        ...
        if any(keyword in lower_input for keyword in complex_task_keywords):
            ...
            if any(keyword in lower_input for keyword in ["分析", "评估", "比较"]):
                ...
                keyword in lower_input for keyword in ["写", "创建", "生成", "设计"]
            ):
                if any(
                    keyword in lower_input
                    for keyword in ["维基", "词条", "百科", "wiki", "文档"]
                ):
                    return {
                        "intent_name": "create_wiki",
                        "parameters": {
        ...
        if any(keyword in lower_input for keyword in academic_keywords) and any(
            keyword in lower_input for keyword in search_keywords
        ):
            # 提取搜索查询
            query = user_input
        ...
        return {
            "intent_name": "question",
            "parameters": {"query": user_input, "original_request": user_input},
            "requires_clarification": False,
            "confidence": 0.6,
