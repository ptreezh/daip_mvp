...
class WikiRulesEngine:
    ...
    def check_factual_accuracy(self, content: str) -> tuple[bool, str]:
        ...
        accuracy_issues = [
            concern for concern in self.accuracy_concerns if concern in content_lower
        ]
    ...
    def detect_vandalism(self, content: str) -> tuple[bool, str]:
        ...
        # 检查破坏性模式
        vandalism_matches = [
            pattern for pattern in self.vandalism_patterns if pattern in content_lower
        ]
        ...
        meaningful_chars = sum(
            1 for c in content if c.isalnum() or c in "，。！？：；、''（）《》【】"
        )
    ...
    def validate_references(self, content: str) -> tuple[bool, str]:
        ...
        # 简单检查是否包含引用格式
        has_ref_pattern = any(
            char in content for char in ["[", "]", "参考", "引自", "来源", "据"]
        )
...
def integrate_rules_engine():
    """将规则引擎集成到Wiki协作会话中"""
    from daip_live.multi_agent_collab.wiki_collaboration_session import (
        WikiCollaborationSession,
    )

    # 添加规则检查方法到现有类
    def submit_contribution_with_validation(
        self,
        contributor: str,
        section: str,
        content: str,
        ...
        # 即使有规则问题也允许提交，但记录问题
        contribution = self._submit_contribution_original(
            contributor, section, content, contribution_type
        )
    ...
    if not hasattr(WikiCollaborationSession, "_submit_contribution_original"):
        WikiCollaborationSession._submit_contribution_original = (
            WikiCollaborationSession.submit_contribution
        )
        WikiCollaborationSession.submit_contribution = (
            submit_contribution_with_validation
        )
