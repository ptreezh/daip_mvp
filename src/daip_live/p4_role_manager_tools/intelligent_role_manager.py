"""Intelligent role management system with dynamic role creation and model availability checking."""  # noqa: E501
...
class IntelligentRoleManager:
    """Enhanced role manager that intelligently creates and manages roles based on topics."""  # noqa: E501

    def __init__(
        self, roles_dir: str = "roles", model_provider: Optional[LiteLLMProvider] = None
    ):
    ...
    def suggest_roles_for_topic(
        self, topic: str, available_roles: list[EnhancedRole], num_suggestions: int = 3
    ) -> list[EnhancedRole]:
        """Suggest the best roles for a given topic."""
        suggestions = self.role_selector.suggest_roles(
            topic, available_roles, num_suggestions
        )
        return [suggestion.role for suggestion in suggestions]

    def auto_select_roles(
        self, topic: str, available_roles: list[EnhancedRole], num_roles: int = 2
    ) -> list[EnhancedRole]:
    ...
    def _generate_role_name(self, topic: str, position: str) -> str:
        ...
        significant_words = [
            w
            for w in words
            if len(w) > 3 and w not in ["the", "and", "for", "with", "are", "you"]
        ]
    ...
    def _generate_role_name_from_persona(self, persona: str) -> str:
        ...
        significant_words = [
            w
            for w in words
            if len(w) > 4 and w not in ["this", "that", "with", "have", "from"]
        ]
        if significant_words:
    ...
    def _generate_role_persona(
        self, topic: str, position: str, topic_analysis: dict
    ) -> str:
        """Generate a persona for the role based on topic and position."""
        # Define base templates for different positions
        templates = {
            "supporting": f"You are a strong advocate for the topic: '{topic}'. Your role is to present arguments in favor of this position, highlighting benefits, opportunities, and positive outcomes. Focus on constructive solutions and optimistic perspectives.",  # noqa: E501
            "opposing": f"You are a critical thinker analyzing the topic: '{topic}'. Your role is to present counter-arguments, identify risks, challenges, and potential negative consequences. Focus on critical analysis and constructive skepticism.",  # noqa: E501
            "neutral": f"You are an impartial observer of the topic: '{topic}'. Your role is to provide balanced perspective, consider multiple viewpoints, and facilitate understanding of different positions. Focus on objective analysis.",  # noqa: E501
            "moderator": f"You are a debate moderator discussing the topic: '{topic}'. Your role is to facilitate fair discussion, ensure all viewpoints are heard, and guide participants toward deeper understanding. Focus on process and balanced inquiry.",  # noqa: E501
        }

        base_persona = templates.get(
            position,
            f"You are an expert discussing: '{topic}'. Provide thoughtful insights on this subject.",  # noqa: E501
        )
    ...
    def save_role_to_file(
        self, role: EnhancedRole, file_path: Optional[str] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if file_path.exists():
                logger.warning(
                    f"Role file already exists: {file_path}, will be overwritten"
                )
    ...
    def load_role_from_file(self, file_name: str) -> Optional[EnhancedRole]:
        ...
        try:
            ...
            if not isinstance(role_data, dict):
                logger.error(
                    f"Role file {file_path} does not contain valid YAML dictionary"
                )
                return None
            ...
            for config_data in model_configs_data:
                try:
                    ...
                    logger.error(
                        f"Error creating RoleModelConfig from data {config_data}: {e}"
                    )
            ...
            if not model_configs:
                logger.warning(
                    f"No valid model configs found for role {file_name}, using default"
                )
            ...
            role_name = role_data.get(
                "name", file_name
            )  # Use name from file or default to file_name
            role = EnhancedRole(
                name=role_name,
            ...
            if debate_config_data:
                try:
                    role.debate_model_config = RoleModelConfig(
                        ...
                        frequency_penalty=debate_config_data.get(
                            "frequency_penalty", 0.0
                        ),
                        presence_penalty=debate_config_data.get(
                    ...
                    logger.error(
                        f"Error creating debate model config from data {debate_config_data}: {e}"  # noqa: E501
                    )
        ...
        if not available_models:
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.info(
                        f"Found {len(available_models)} local models via Ollama"
                    )
                else:
                    logger.warning("Ollama command returned non-zero exit code")
            except FileNotFoundError:
                logger.warning(
                    "Ollama not found. Please install Ollama to use local models."
                )
            except subprocess.TimeoutExpired:
                logger.warning(
                    "Ollama command timed out. Please check if Ollama is running."
                )
            except Exception as e:
    ...
    async def update_role_models(self, role: EnhancedRole) -> EnhancedRole:
        ...
        try:
            ...
            if not available_models:
                logger.warning(
                    f"No models available for role {role.name}, keeping original model configurations"  # noqa: E501
                )
                return role
            ...
            for config in role.model_configs:
                if config.model_name in available_models:
                    ...
                    logger.info(
                        f"Model {config.model_name} not available for role {role.name}, finding replacement"  # noqa: E501
                    )
                    ...
                    if replacement:
                        ...
                        logger.info(
                            f"Replaced model {config.model_name} with {replacement} for role {role.name}"  # noqa: E501
                        )
                    else:
                        logger.warning(
                            f"Could not find replacement for model {config.model_name} for role {role.name}"  # noqa: E501
                        )
                        # Keep the original config as fallback if no replacement is found  # noqa: E501
                        updated_configs.append(config)
            ...
            if role.debate_model_config:
                if role.debate_model_config.model_name in available_models:
                    ...
                    logger.debug(
                        f"Debate model {role.debate_model_config.model_name} is available for role {role.name}"  # noqa: E501
                    )
                else:
                    logger.info(
                        f"Debate model {role.debate_model_config.model_name} not available for role {role.name}, finding replacement"  # noqa: E501
                    )
                    replacement = self._find_best_model_replacement(
                        role.debate_model_config, available_models
                    )
                    if replacement:
                        ...
                        logger.info(
                            f"Replaced debate model {role.debate_model_config.model_name} with {replacement} for role {role.name}"  # noqa: E501
                        )
                    else:
                        logger.warning(
                            f"Could not find replacement for debate model {role.debate_model_config.model_name} for role {role.name}"  # noqa: E501
                        )
    ...
    def _find_best_model_replacement(
        self, original_config: RoleModelConfig, available_models: list[str]
    ) -> Optional[str]:
        ...
        model_name = (
            original_parts[-1] if len(original_parts) > 1 else original_parts[0]
        )
    ...
    async def create_and_save_role_for_topic(
        self, topic: str, position: str = "supporting"
