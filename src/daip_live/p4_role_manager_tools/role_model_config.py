...
class RoleModelConfig(BaseModel):
    ...
    model_name: str = Field(
        ..., description="Name of the model (e.g., 'gpt-4', 'claude-3')"
    )
    provider: str = Field(
    ...
    is_primary: bool = Field(
        True, description="Whether this is the primary model for the role"
    )
...
class EnhancedRole(Role):
    ...
    debate_model_config: Optional[RoleModelConfig] = Field(
        None,
        description="Specific model configuration for debates (if different from primary)",  # noqa: E501
    )
...
class RoleModelMapping(BaseModel):
    ...
    role_name: str
    role_model_config: RoleModelConfig = Field(
        ..., description="Model configuration for this role"
    )
    priority: int = Field(
        1, description="Priority for model selection (higher = preferred)"
    )

    @classmethod
    def from_role(
        cls, role: EnhancedRole, use_debate_config: bool = True
    ) -> "RoleModelMapping":
        ...
        return cls(role_name=role.name, role_model_config=model_config, priority=1)
