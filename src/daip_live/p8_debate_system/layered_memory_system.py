...
class LayeredMemorySystem:
    ...
    # 角色个人记忆 - 每个角色独立的记忆
    role_personal_memories: dict[str, dict[str, list[dict[str, Any]]]] = field(
    ...
    def add_shared_fact(
        self, round_num: int, fact: str, source: str, confidence: float
    ):
        """添加共享事实"""
    ...
    def update_role_memory(
        self, role_name: str, content: str, round_num: int, memory_type: str
    ):
        """更新角色记忆"""
    ...
    def get_role_context(self, role_name: str, current_round: int) -> str:
        ...
        relevant_facts = [
            f for f in self.shared_factual_history if f["round"] < current_round
        ]
        if relevant_facts:
            for fact in relevant_facts[-5:]:  # 最近5个事实
                context_parts.append(
                    f"  - {fact['fact']} (Round {fact['round']}, Confidence: {fact['confidence']})"  # noqa: E501
                )
        else:
        ...
        if role_name in self.role_personal_memories:
            ...
            # 此处需兼容单复数两种 key
            arguments = self.role_personal_memories[role_name].get(
                "arguments"
        ...
        context_parts.append("\nRound Summaries:")
        relevant_rounds = [
            r for r in range(1, current_round) if r in self.round_summaries
        ]
        if relevant_rounds:
    ...
    def get_compressed_context(
        self, role_name: str, current_round: int, max_rounds: int = 3
    ) -> str:
        """获取压缩上下文（用于防止上下文过长）"""
        context_parts = []
        ...
        recent_facts = [
            f
            for f in self.shared_factual_history
            if f["round"] >= current_round - max_rounds
        ]
        if recent_facts:
        ...
        if role_name in self.role_personal_memories:
            ...
            recent_args = [
                arg for arg in arguments if arg["round"] >= current_round - max_rounds
            ]
            if recent_args:
                context_parts.append("\nRecent Arguments:")
        ...
        recent_rounds = [
            r
            for r in range(max(1, current_round - max_rounds + 1), current_round)
            if r in self.round_summaries
        ]
        if recent_rounds:
                ...
                for i, fact1 in enumerate(facts):
                    for fact2 in facts[i + 1 :]:
                        if self._are_contradictory(fact1["fact"], fact2["fact"]):
                            conflicts.append(
                                f"Contradictory facts about '{word}': '{fact1['fact']}' vs '{fact2['fact']}'"  # noqa: E501
                            )
    ...
    def cleanup_old_memories(self, max_rounds_to_keep: int, current_round: int):
        ...
        self.shared_factual_history = [
            f for f in self.shared_factual_history if f["round"] >= cutoff_round
        ]
        ...
        for role_name in self.role_personal_memories:
            for memory_type in self.role_personal_memories[role_name]:
                self.role_personal_memories[role_name][memory_type] = [
                    m
                    for m in self.role_personal_memories[role_name][memory_type]
        ...
        for role_name in self.stance_evolution:
            self.stance_evolution[role_name] = [
                s
                for s in self.stance_evolution[role_name]
    ...
    def analyze_stance_consistency(self, role_name: str) -> dict[str, Any]:
        ...
        for i in range(1, len(stances)):
            if self._are_stances_different(
                stances[i - 1]["stance"], stances[i]["stance"]
            ):
    ...
    def get_memory_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_stance_entries": sum(
                len(stances) for stances in self.stance_evolution.values()
            ),
    ...
    def __str__(self) -> str:
        ...
        return f"LayeredMemorySystem(facts={stats['total_shared_facts']}, roles={stats['total_roles']}, rounds={stats['total_round_summaries']})"  # noqa: E501

    def __repr__(self) -> str:
        """详细字符串表示"""
        return (
            f"LayeredMemorySystem("
