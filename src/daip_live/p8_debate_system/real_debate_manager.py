...
from daip_live.core.models import (
    AgentEvent, DebateStartEvent, DebateRoundStartEvent,
    DebateTurnStartEvent, DebateTurnCompleteEvent, DebateCompleteEvent,
    DialogueTurn, Role, Session
)
...
class RealDebateManager(IDebateManager):
    ...
    def _build_debate_context(self, role: str) -> str:
        ...
        if self.debate_state['current_round'] > 0:
            context_parts.append(f"当前轮次：第{self.debate_state['current_round']}轮（共{self.debate_state['rounds']}轮）")

        # 添加历史对话
        if self.debate_state['turn_history']:
            recent_turns = self.debate_state['turn_history'][-5:]  # 最近5个回合
            for turn in recent_turns:
                context_parts.append(f"第{turn['round']}轮 - {turn['role']}：{turn['prompt'][:100]}...（{turn['response'][:100]}）")
    ...
    def _build_debate_prompt(self, role: str, context: str) -> str:
        ...
        # 获取当前轮次的所有发言
        current_round_turns = [
            turn for turn in self.debate_state['turn_history']
        ...
        opponent_turns = [
            turn for turn in current_round_turns
            if turn['role'] == opponent_role
        ]
    ...
    async def _evaluate_role_performances(self) -> Dict[str, str]:
        ...
        for role in self.debate_state['participants']:
            ...
            if not role_turns:
                ...
                performances[role] = f"表现：{performance}（平均响应时间：{avg_response_time:.2f}秒）"
...
"""
    ...
    def get_debate_model_summary(self, roles: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_models': len(set([assignments.values()])),
    ...
    def _get_model_for_role(self, role: str) -> str:
        ...
        if role == 'pro_arguer':
            ...
            if len(available_models) > 1:
                return available_models[1] if available_models[0] != self.model_provider.get_default_model() else self.model_provider.get_default_model()
