...
class PermissionManager:
    ...
    def __init__(self, user_input_queue: asyncio.Queue, tui_interface):
        ...
        self.permission_config = ToolPermissionConfig(default="ask", tools={})
    ...
    async def check_permission(
        ...
        request = PermissionRequestEvent(
            tool_name=tool_name, args=args, timeout_seconds=timeout or 30.0
        )
        ...
        try:
            ...
            if cached_response:
                logger.info(
                    f"Using cached permission for {tool_name}: {cached_response}"
                )
                return self._create_permission_result(
                    cached_response, request, cached=True
                )
            ...
            if permission_status == "allow":
                ...
                # 未知状态，默认拒绝（安全优先）
                logger.warning(
                    f"Unknown permission status: {permission_status}, defaulting to deny"  # noqa: E501
                )
                return self._create_permission_result(PermissionResponse.DENY, request)
            ...
            # 错误时默认拒绝（安全优先）
            return self._create_permission_result(
                PermissionResponse.DENY, request, error=str(e)
            )
    ...
    def get_permission_status(self, tool_name: str) -> str:
        ...
        """
        return self.permission_config.tools.get(
            tool_name, self.permission_config.default
        )
    ...
    def set_permission_rule(self, tool_name: str, permission: str) -> None:
        ...
        if permission not in ["allow", "deny", "ask"]:
            raise ValueError(
                f"Invalid permission: {permission}. Must be allow/deny/ask"
            )
