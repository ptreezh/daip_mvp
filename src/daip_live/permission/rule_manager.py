...
class PermissionRuleManager(PermissionRuleManagerInterface):
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            # 配置加载失败，使用默认配置
            print(f"警告: 无法加载权限配置文件 {self._config_file}: {e}")
            self._default_permission = "ask"
    ...
    def _save_config(self) -> None:
        ...
        try:
            ...
            print(f"错误: 无法保存权限配置文件 {self._config_file}: {e}")
    ...
    def set_tool_permission(self, tool_name: str, permission: str) -> None:
        ...
        if permission not in ["allow", "deny", "ask"]:
            raise ValueError(f"无效的权限值: {permission}. 必须是 'allow', 'deny', 或 'ask' 之一.")
...
class PermissionRuleManagerFactory:
    ...
    @staticmethod
    def create_permission_rule_manager(config_file: Optional[str] = None) -> PermissionRuleManagerInterface:
        """
        创建权限规则管理器实例
