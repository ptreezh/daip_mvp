...
class PermissionTUIInterface:
    ...
    async def get_user_response(self, timeout: float = 30.0) -> PermissionResponse:
        ...
        try:
            ...
            logger.warning(
                f"Permission request timed out for {self.current_request.tool_name}"
            )
    ...
    async def _collect_user_response_with_timeout(
        self, timeout: float
    ) -> PermissionResponse:
        """带超时的用户响应收集"""
        ...
        while True:
            try:
                ...
                user_input = await asyncio.wait_for(
                    self.user_input_queue.get(), timeout=wait_timeout
                )
    ...
    async def _render_permission_interface(
        self, request: PermissionRequestEvent
    ) -> None:
        """渲染权限请求界面"""
        content = self._render_permission_interface_content(request)
        await self._render_to_screen(content)

    def _render_permission_interface_content(
        self, request: PermissionRequestEvent
    ) -> str:
        """渲染权限请求界面内容"""
        lines = []
        ...
        # 风险警告
        warning = self._get_risk_warning(
    ...
    def _get_risk_warning(
        self, tool_name: str, args: dict[str, Any], risk_level: str
    ) -> str:
        """获取风险警告信息"""
        warnings = []
        ...
        if risk_level == "medium":
            warnings.append(
                f"{self.theme.warning_prefix} This tool will access system resources. Please review carefully."  # noqa: E501
            )
        elif risk_level == "high":
            warnings.append(
                f"{self.theme.warning_prefix} WARNING: This tool performs sensitive operations that may affect system security."  # noqa: E501
            )
            warnings.append(
                f"{self.theme.warning_prefix} Please ensure you understand the implications before proceeding."  # noqa: E501
            )
        ...
        if "read" in tool_name.lower() and "path" in args:
            ...
            if path and len(path) > 0:
                warnings.append(
                    f"{self.theme.warning_prefix} This tool will access file system resources at: {path}"  # noqa: E501
                )
                warnings.append(
                    f"{self.theme.warning_prefix} Ensure the path is safe and you have appropriate permissions."  # noqa: E501
                )
            ...
            if path and len(path) > 0:
                warnings.append(
                    f"{self.theme.warning_prefix} This tool will write to file system at: {path}"  # noqa: E501
                )
                warnings.append(
                    f"{self.theme.warning_prefix} Ensure the destination is safe and you have write permissions."  # noqa: E501
                )

        elif "execute" in tool_name.lower() or "command" in tool_name.lower():
            warnings.append(
                f"{self.theme.warning_prefix} This tool will execute system commands"
            )
            warnings.append(
                f"{self.theme.warning_prefix} Commands may have system-wide effects. Proceed with caution."  # noqa: E501
            )
...
{self.theme.warning_prefix} Are you sure? [Y/N]: """  # noqa: E501
...
"""
    ...
    async def _render_simplified_interface(
        self, request: PermissionRequestEvent
    ) -> None:
        """渲染简化界面（降级方案）"""
