...
class UserResponseCollector:
    ...
    def __init__(
        ...
        config: Optional[ResponseCollectorConfig] = None,
    ):
    ...
    async def collect_response(
        self, request: PermissionRequestEvent, timeout: Optional[float] = None
    ) -> PermissionResponse:
        """
        收集用户对权限请求的响应
        ...
        self.current_interaction = PermissionInteraction(
            tool_name=request.tool_name, args=request.args, timeout_seconds=timeout
        )
        ...
        try:
            ...
            logger.warning(
                "Response collection timed out for tool: %s", request.tool_name
            )
    ...
    async def _collect_response_with_retry(
        self, request: PermissionRequestEvent, timeout: float
    ) -> PermissionResponse:
        """带重试机制的响应收集"""
            ...
            while timeout > 0:
                try:
                    user_input = await asyncio.wait_for(
                        self.user_input_queue.get(), timeout=poll_timeout
                    )
                    return user_input
                except asyncio.TimeoutError:
                    # 检查是否被取消
    ...
    def _validate_and_parse_input(
        ...
        if self.config.security_validation_enabled:
            if not self._validate_security(cleaned_input):
                logger.warning(
                    "Security validation failed for input: %s", cleaned_input
                )
                return None
    ...
    def _parse_input(
        self, user_input: str, original_input: Optional[str] = None
    ) -> Optional[PermissionResponse]:
        """解析用户输入"""
        if not user_input:
            # 处理只有空白字符的情况（特殊测试用例）
    ...
    async def _handle_confirmation(self, response: PermissionResponse) -> bool:
        ...
        try:
            confirmation_input = await asyncio.wait_for(
                self.user_input_queue.get(), timeout=self.config.confirmation_timeout
            )
...
class ResponseProcessor:
    ...
    def process_response(
        self, response: PermissionResponse, request: PermissionRequestEvent
    ) -> PermissionResult:
        """
        处理用户响应并生成权限结果
        ...
        result = PermissionResult(
            ...
            remembered=response
            in [PermissionResponse.ALWAYS, PermissionResponse.NEVER],
    ...
    def _generate_reason(
        ...
        """生成处理原因"""
        reasons = {
            PermissionResponse.GRANT: f"Permission granted by user for {request.tool_name}",  # noqa: E501
            PermissionResponse.DENY: f"Permission denied by user for {request.tool_name}",  # noqa: E501
            PermissionResponse.ALWAYS: f"Permission always granted by user for {request.tool_name}",  # noqa: E501
            PermissionResponse.NEVER: f"Permission never granted by user for {request.tool_name}",  # noqa: E501
            PermissionResponse.CANCEL: f"Permission request cancelled by user for {request.tool_name}",  # noqa: E501
        }
