...
class LLMModelConfig:
    ...
    def validate(self) -> List[str]:
        ...
        if self.provider not in [LLMProvider.LOCAL, LLMProvider.OLLAMA] and not self.api_key:
...
class LLMService:
    ...
    async def generate_text(self, request: LLMRequest) -> LLMResponse:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                response = await self._call_provider(request.model_config.provider, request)
                if response.success:
                    return response

                # 如果是速率限制错误，尝试重试
                if response.error_code == "rate_limit" and attempt < self.config.max_retries:
    ...
    async def generate_text_stream(self, request: LLMRequest) -> AsyncGenerator[LLMResponse, None]:
        """流式生成文本"""
        if not request.model_config and self.config.default_model:
            request.model_config = self.config.default_model
        ...
        try:
            async for response in self._call_provider_stream(request.model_config.provider, request):
                yield response
    ...
    async def _call_openai_stream(self, request: LLMRequest) -> AsyncGenerator[LLMResponse, None]:
        """调用OpenAI流式提供商（兼容性方法）"""
    ...
    async def _call_fallback_provider(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            response = await self._call_provider(self.config.fallback_model.provider, request)
            return response
        finally:
