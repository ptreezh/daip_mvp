...
class ClaudeSkillsManager:
    ...
    async def download_skill_from_github(self, repo_url: str, skill_name: str = None) -> bool:
        """从GitHub仓库下载技能"""
        try:
            print(f"📥 正在从GitHub下载技能: {repo_url}")
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not skill_dirs:
                    print("❌ 未找到包含 manifest.json 的技能目录")
                    return False
        ...
        try:
            ...
            try:
                with zipfile.ZipFile(temp_path, 'r') as zip_ref:
                    ...
                    if not has_manifest:
                        print(f"❌ 下载的ZIP不包含有效的技能清单: {url}")
                        return False
                    
                    # 提取到技能目录
                    zip_ref.extractall(self.skills_directory)
                    print(f"✅ 技能已解压到: {self.skills_directory}")
                    return True
            ...
            if skill_dir.is_dir():
                ...
                if manifest_file.exists():  # 只需要manifest.json
                    try:
                        ...
                        print(f"❌ 加载技能清单失败 {skill_dir.name}: {e}")
    ...
    def sync_official_skills(self) -> bool:
        ...
        for repo_url in official_repos:
            try:
                ...
                print(f"🔄 同步官方技能仓库: {repo_url}")
            except Exception as e:
                print(f"❌ 同步仓库失败 {repo_url}: {e}")
                success = False
...
async def sync_claude_skills_from_official_repo() -> bool:
    ...
    official_repos = [
        ...
        ("https://github.com/anthropics/claude-computer-use-tools", "computer-use-tools")
    ]
    
    success = True
    for repo_url, repo_name in official_repos:
        print(f"🔄 正在同步 {repo_name} 技能仓库...")
        try:
            result = await manager.download_skill_from_github(repo_url)
            if not result:
                print(f"⚠️  {repo_name} 仓库同步失败，尝试备用仓库")
                # 可以在这里添加备用仓库
                success = False
        except Exception as e:
            print(f"❌ 同步 {repo_name} 仓库时出错: {e}")
            success = False
