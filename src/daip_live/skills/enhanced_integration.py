...
class GitHubSkillDownloader:
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(api_url) as response:
                        if response.status == 200:
                            ...
                            # 查找包含技能定义的目录
                            skill_dirs = [
                                item for item in contents if item.get("type") == "dir"
                            ]
                            ...
                            for skill_dir in skill_dirs:
                                ...
                                async with session.get(manifest_url) as manifest_resp:
                                    if manifest_resp.status == 200:
                                        # 下载整个技能目录
                                        await self._download_skill_directory(
                                            session,
                                            username,
                                            repo_name,
                                            skill_name,
                            ...
                            raise ModelError(
                                f"GitHub API request failed with status {response.status}: {error_data}"  # noqa: E501
                            )
    ...
    async def _download_skill_directory(
        self, session, username: str, repo_name: str, skill_name: str, skill_path: Path
    ):
        ...
        # 首先检查根目录的文件
        contents_url = (
            f"https://api.github.com/repos/{username}/{repo_name}/contents/{skill_name}"
        )
            ...
            if resp.status == 200:
                ...
                for file_info in files:
                    if file_info["type"] == "file":
                        ...
                        # 递归下载子目录
                        await self._download_skill_subdirectory(
                            session,
                            username,
                            repo_name,
                            skill_name,
    ...
    async def _download_skill_subdirectory(
        self,
        session,
        username: str,
        repo_name: str,
                ...
                for file_info in files:
                    if file_info["type"] == "file":
                        ...
                        await self._download_skill_subdirectory(
                            ...
                            username,
                            repo_name,
                            f"{parent_skill_name}/{sub_dir_name}",
                            file_info["name"],
                            sub_path,
                        )
            else:
...
class ContextLimitHandler:
    ...
    def _estimate_tokens(self, text: str) -> int:
        ...
        symbols = (
            len(text)
            - chinese_chars
            - len("".join(re.findall(r"\b[a-zA-Z]+\b", text)))
            - len("".join(re.findall(r"\b\d+\b", text)))
        )
        ...
        for chunk in chunks:
            if self._estimate_tokens(chunk) > self.max_tokens_per_chunk:
                ...
                for sent in sentences:
                    if (
                        sent.strip()
                        and self._estimate_tokens(temp_chunk + sent)
...
class JSONSchemaValidator:
    ...
    def _basic_validation(
        ...
        for req_field in required:
            if (
                req_field not in params
                or params[req_field] is None
                or params[req_field] == ""
            ):
        ...
        for field, field_def in properties.items():
            if field in params and params[field] is not None:
                ...
                if field_type == "string" and not isinstance(params[field], str):
                    errors.append(
                        f"参数 {field} 应该是字符串类型，当前类型: {type(params[field])}"  # noqa: E501
                    )
                elif field_type in ["number", "integer"] and not isinstance(
                    params[field], (int, float)
                ):
                    errors.append(
                        f"参数 {field} 应该是数字类型，当前类型: {type(params[field])}"
                    )
                elif field_type == "boolean" and not isinstance(params[field], bool):
                    errors.append(
                        f"参数 {field} 应该是布尔类型，当前类型: {type(params[field])}"
                    )
...
class ClaudeSkillsRuntimeSandbox:
    ...
    async def execute_skill_safely(
        self, skill_func, *args, **kwargs
        ...
        try:
            ...
            result = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(
                    None, lambda: skill_func(*args, **kwargs)
                ),
    ...
    async def recommend_skills(self, user_input: str) -> list[tuple[str, float, str]]:
        ...
        for skill_name in available_skills:
            ...
            if skill and hasattr(skill, "metadata"):
                # 计算相似度
                skill_keywords = set(
                    skill.metadata.description.lower().split() + skill.metadata.tags
                )
                similarity = self._jaccard_similarity(user_keywords, skill_keywords)

                if similarity > 0.1:  # 阈值
                    recommendations.append(
                        (skill_name, similarity, skill.metadata.description)
                    )
...
class EnhancedClaudeSkillsManager:
    ...
    async def load_skills_from_github(self, repo_url: str) -> list[str]:
        ...
        if downloaded_skills:
            self.skill_manager.load_claude_skills_from_directory("./claude_skills")
    ...
    async def execute_skill_with_context_handling(
        ...
        return SkillOutput(
            ...
            else (result.result if hasattr(result, "result") else str(result)),
            confidence=0.85,
            execution_time=0.1,
...
def integrate_with_intent_recognizer(
    recognizer, skill_manager: SkillManager, model_provider=None
):
