...
class EnhancedModelManager:
    ...
    def _set_default_model(self):
        ...
        preferred_models = [
            ...
            and "instruction_following" in model.capabilities
        ]
    ...
    def select_best_model_for_task(
        self, role_type: str = "general", task_type: str = "general"
    ) -> ModelInfo:
        """为特定任务选择最佳模型"""
        if not self._available_models:
            return self._default_model
        ...
        for model in self._available_models:
            ...
            if (
                role_type
                and "researcher" in role_type.lower()
                and "reasoning" in model.capabilities
            ):
                score += 2
            elif (
                role_type
                and "writer" in role_type.lower()
                and "instruction_following" in model.capabilities
            ):
                score += 2
            elif (
                role_type
                and "analyst" in role_type.lower()
                and "reasoning" in model.capabilities
            ):
                score += 2
            elif (
                role_type
                and "expert" in role_type.lower()
                and "high_capacity" in model.capabilities
            ):
                score += 2
            else:
                score += 1  # 通用评分
            ...
            if task_type == "analysis" and "reasoning" in model.capabilities:
                ...
                task_type == "writing" and "instruction_following" in model.capabilities
            ):
                score += 1
    ...
    def get_model_for_debate_role(self, role_name: str) -> ModelInfo:
        ...
        # 基于角色名称确定模型需求
        if (
            "pro" in role_name.lower()
            or "affirmative" in role_name.lower()
            or "支持" in role_name
        ):
            task_type = "argumentation"
        elif (
            "con" in role_name.lower()
            or "negative" in role_name.lower()
            or "反对" in role_name
        ):
    ...
    def get_model_performance_report(self) -> str:
        ...
        report_parts = [
            "模型性能报告:",
            f"最后扫描时间: {self._last_scan_time}",
            "",
            "可用模型:",
        ]

        for i, model in enumerate(self._available_models, 1):
            stats = self._model_usage_stats.get(
                model.name, {"usage_count": 0, "success_count": 0}
            )
            success_rate = (
        ...
        if self._default_model:
            report_parts.append("", f"当前默认模型: {self._default_model.name}", "")

        return "\n".join(report_parts)


# 便捷函数：创建增强模型管理器并集成到现有系统
def create_enhanced_model_manager():
    """创建增强模型管理器实例"""
