...
from daip_live.skills.base import SkillInput, SkillOutput, SkillMetadata
...
class ClaudeSkillsIntegrationService:
    ...
    def __init__(self, skill_manager: SkillManager, model_provider: LiteLLMProvider = None):
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 技能已注册，从技能管理器中获取
            self.claude_skills["text_analysis"] = self.skill_manager.get_skill("text_analysis")
        
        # 尝试加载来自GitHub的外部技能
        await self._load_external_skills()
        
        self._initialized = True
        print("✅ Claude Skills 集成服务已初始化")
    ...
    async def _load_skill_from_manifest(self, manifest_path: Path):
        ...
        try:
            ...
            skill_name = manifest_data.get("name", f"external_skill_{manifest_path.parent.name}")
            skill_description = manifest_data.get("description", "External Claude-compatible skill")
            ...
            print(f"✅ 已加载 Claude 技能: {skill_name}")
    ...
    async def find_appropriate_skill(self, user_input: str) -> Optional[str]:
        ...
        skill_matchers = {
            "text_analysis": [
                r"分析.*文本", r"分析.*内容", r"文本.*分析", r"内容.*分析",
                r"帮我.*分析", r"分析.*", r"解读.*", r"理解.*文本",
                r"理解.*内容", r"评价.*文本", r"评价.*内容", r"总结.*文本", r"总结.*内容"
            ],
            "search_papers": [
                r"搜索.*", r"查找.*", r"查询.*", r"找.*", r"搜.*",
                r"检索.*", r"寻找.*", r"论文.*", r"文章.*", r"资料.*"
            ],
            "personal_assistant": [
                r"助手", r"助理", r"帮我", r"请.*", r"我想.*",
                r"可以.*", r"能够.*", r"怎么办", r"如何.*", r"怎么.*"
            ],
            "create_wiki": [
                r"创建.*wiki", r"写.*wiki", r"新建.*wiki", r"创建.*维基",
                r"写.*维基", r"新建.*维基", r"创建.*百科", r"写.*百科", r"新建.*百科"
            ]
        }
        ...
        if available_skills:
            ...
            for skill_name in available_skills:
                ...
                if skill_meta:
                    meta_keywords = set(skill_meta.description.lower().split() + skill_meta.tags)
    ...
    async def execute_skill_with_context(self, skill_name: str, input_text: str, context: Dict[str, Any] = None) -> Optional[SkillOutput]:
        """在上下文中执行技能"""
        if not self._initialized:
            await self.initialize()
        ...
        if skill:
            try:
                skill_input = SkillInput(
                    data=input_text,
                    context=context or {},
                    metadata={"source": "claude_integration", "timestamp": datetime.now().isoformat()}
                )
    ...
    async def get_skill_recommendations(self, user_input: str) -> List[Dict[str, str]]:
        ...
        for skill_name in available_skills:
            ...
            if skill and hasattr(skill, 'metadata'):
                # 计算与用户输入的相关性
                similarity = self._calculate_similarity(user_input.lower(), skill.metadata.description.lower())
...
class ClaudeSkillAdapter:
    ...
    def __init__(self, skill_name: str, manifest_data: Dict[str, Any], skill_manager: SkillManager):
        ...
        self.metadata = SkillMetadata(
            name=skill_name,
            description=manifest_data.get("description", f"External skill: {skill_name}"),
...
# 集成到现有的意图识别器
def integrate_with_intent_recognizer(recognizer, skill_manager: SkillManager, model_provider: LiteLLMProvider):
