...
class ClaudeSkillAdapter(BaseModel):
    ...
    async def execute_with_params(self, params: Dict[str, str]) -> str:
        ...
        try:
            ...
            import subprocess
            # 模拟执行，因为可能Ollama未运行
            return f"Claude Skill '{self.skill_name}' executed successfully with parameters: {list(params.keys())}"
...
class ClaudeSkillsManager:
                    ...
                    if skill_subdir.is_dir():
                        ...
                        if manifest_path.exists() and tools_path.exists():
                            try:
                                ...
                                skill_name = manifest_data.get("name", skill_subdir.name)
                                ...
                                adapter = ClaudeSkillAdapter(
                                    ...
                                    model_name=manifest_data.get("api", {}).get("model_name", "default"),
                                    description=manifest_data.get("description", ""),
                                    parameters_needed=[]
                                )

                                self._skills[skill_name] = adapter
                                print(f"✅ Loaded Claude Skill: {skill_name}")

                            except Exception as e:
                                print(f"⚠️  Failed to load Claude Skill from {skill_subdir}: {e}")
    ...
    def _discover_available_models(self):
        ...
        try:
            ...
            print(f"⚠️  Ollama未运行或未安装，跳过模型发现: {e}")
                        ...
                        if manifest_path.exists() and tools_path.exists():
                            try:
                                ...
                                adapter = ClaudeSkillAdapter(
                                    ...
                                    manifest_path=str(manifest_path),
                                    tools_path=str(tools_path),
                                    model_name=manifest_data.get("api", {}).get("model", "default"),
                                    description=manifest_data.get("description", ""),
                                    parameters_needed=[]
                                )
    ...
    def find_matching_skill(self, query: str) -> Optional[ClaudeSkillAdapter]:
        ...
        for skill_name, skill in self._skills.items():
            if query_lower in skill.description.lower():
                return skill
            if any(query_lower in tag.lower() for tag in ["text", "analyze", "analysis", "search", "find", "process", "handle"]):
                # 智能识别，如果描述中包含匹配关键词
                if any(keyword in skill.description.lower() for keyword in ["text", "analyze", "search", "process"]):
                    return skill
        
        return None
    
    def get_skill_for_natural_language(self, nl_input: str) -> Optional[Tuple[ClaudeSkillAdapter, Dict[str, str]]]:
        """根据自然语言输入获取技能和参数"""
        # 尝试解析参数
    ...
    def _extract_params_from_natural_language(self, text: str) -> Dict[str, str]:
        ...
        for pattern, param_name in patterns:
            ...
            if match:
                ...
                if extracted and len(extracted) > 2 and extracted not in ["这个", "那个", "这些", "那些"]:
    ...
    def _smart_skill_match(self, text: str) -> Optional[ClaudeSkillAdapter]:
        ...
        for skill_name, skill in self._skills.items():
            ...
            # 文本分析类型
            if any(keyword in text_lower for keyword in ["分析", "analyze", "text", "内容", "文档", "文章"]) and \
               any(keyword in skill_desc_lower for keyword in ["text", "content", "analyze", "analysis"]):
                return skill
            
            # 搜索类型
            elif any(keyword in text_lower for keyword in ["搜索", "查找", "search", "find", "资料", "信息", "论文"]) and \
                 any(keyword in skill_desc_lower for keyword in ["search", "find", "information"]):
                return skill
            
            # 处理类型
            elif any(keyword in text_lower for keyword in ["处理", "process", "转换", "translate", "总结"]) and \
                 any(keyword in skill_desc_lower for keyword in ["process", "convert", "translate", "summarize"]):
                return skill
...
class SkillIntegrationService:
    ...
    def get_matching_claude_skill(self, user_input: str) -> Optional[Tuple[ClaudeSkillAdapter, Dict[str, str]]]:
        """获取匹配的Claude技能"""
...
if __name__ == "__main__":
    print("Testing Claude Skills Integration System...")
    
    # 测试Claude Skills管理器
    manager = ClaudeSkillsManager()
    print(f"Found {len(manager._skills)} Claude Skills")
    ...
    for query in test_queries:
        ...
        if skill:
            print(f"✅ {query} → {skill.skill_name} with params: {params}")
        else:
            print(f"❌ {query} → No matching skill")