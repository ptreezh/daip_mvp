...
class TaskContextManager:
    ...
    @contextmanager
    def create_context(
        self, original_request: str, description: str = ""
    ) -> TaskContext:
        """创建任务上下文的上下文管理器"""
    ...
    async def get_task_progress(self, task_id: str) -> Optional[dict[str, Any]]:
        ...
        completed = len(
            [st for st in context.subtasks if st.status == TaskStatus.COMPLETED]
        )
        in_progress = len(
            [st for st in context.subtasks if st.status == TaskStatus.IN_PROGRESS]
        )
        failed = len([st for st in context.subtasks if st.status == TaskStatus.FAILED])
        pending = len(
            [st for st in context.subtasks if st.status == TaskStatus.PENDING]
        )
    ...
    def _trigger_subtask_event(
        self, context: TaskContext, subtask: SubTaskContext, old_status: TaskStatus
    ):
        """触发子任务事件"""
        logger.info(
            f"Subtask {subtask.id} in task {context.id} changed from {old_status.value} to {subtask.status.value}"  # noqa: E501
        )
...
class AdvancedTaskOrchestrator:
    ...
    async def execute_task_with_context(self, original_request: str) -> dict[str, Any]:
        ...
        await self.context_manager.update_task_status(
            context.id, TaskStatus.IN_PROGRESS
        )
        ...
        for subtask_id in execution_order:
            ...
            await self.context_manager.update_subtask_status(
                context.id, subtask_id, TaskStatus.IN_PROGRESS
            )
            ...
            try:
                ...
                # 更新子任务为完成
                await self.context_manager.update_subtask_status(
                    context.id, subtask_id, TaskStatus.COMPLETED, result=result
                )
                ...
                # 更新子任务为失败
                await self.context_manager.update_subtask_status(
                    context.id, subtask_id, TaskStatus.FAILED, error=str(e)
                )
...
{{
    ...
    def _calculate_execution_order(self, subtasks: list[SubTaskContext]) -> list[str]:
        ...
        for subtask in subtasks:
            # 只依赖不在当前任务列表中，则忽略
            valid_deps = [
                dep_id for dep_id in subtask.dependencies if dep_id in all_task_ids
            ]
    ...
    async def _execute_subtask(
        self, subtask: SubTaskContext, parent_context: TaskContext
    ) -> str:
        """执行子任务"""
        if self.model_provider:
            context_prompt = f"""
...
"""
    ...
    async def _synthesize_results(
        self, original_request: str, subtask_results: list[str]
    ) -> str:
        """合成子任务结果"""
        if self.model_provider:
            results_summary = "\\n".join(
...
"""
        ...
        return (
            ...
            + "\\n".join([f"- {result[:100]}..." for result in subtask_results[:3]])
        )
