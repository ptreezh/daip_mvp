...
class AgileTaskMemory:
    ...
    def save_sprint(self, sprint: Sprint) -> str:
        ...
        sprint_data = {
            ...
            "name": sprint.name,
            "goal": sprint.goal,
            "created_at": sprint.created_at.isoformat() if isinstance(sprint.created_at, datetime) else sprint.created_at,
    ...
    def save_intermediate_document(self, document_name: str, content: str) -> str:
        """保存中间文档"""
        doc_file = self.documents_path / f"{document_name.replace(' ', '_')}_{uuid.uuid4().hex[:8]}.md"
...
class TaskDecomposer:
    ...
    def __init__(self, model_provider=None):
        ...
        self.complexity_keywords = [
            # 动词类
            "分析", "研究", "调查", "评估", "比较", "设计", "构建", "开发", 
            "创建", "实现", "撰写", "制定", "规划", "探索", "审查", "验证",
            "测试", "优化", "改进", "建立", "组织", "管理", "执行", "解决",
            # 形容词类
            "复杂", "详细", "深入", "全面", "系统", "整体", "多方面", "多层次",
            "多维度", "综合性", "战略性", "创新性", "重要", "关键", "核心", "主要",
            "显著", "广泛", "长远", "持续", "多阶段", "多步骤", "多环节"
        ]
        
        # 复杂任务模式
        self.complexity_patterns = [
            r".*(分析|研究|调查).*方法", r".*(分析|研究|调查).*技术", r".*(分析|研究|调查).*原因", r".*(分析|研究|调查).*机制",
            r".*(设计|构建|开发).*系统", r".*(设计|构建|开发).*框架", r".*(开发|实现).*模块", r".*(制定|创建).*策略",
            r".*(评估|比较).*影响", r".*(评估|比较).*优劣", r".*(探索|探讨).*趋势", r".*(分析|研究).*前景",
            r".*(创建|实现).*解决方案", r".*(实现|开发).*功能", r".*(构建|建立).*平台", r".*(开发|构建).*架构",
            r".*(撰写|编写).*报告", r".*(编写|撰写).*方案", r".*(制定|创建).*计划", r".*(建立|构建).*机制"
        ]
    ...
    async def should_decompose_task(self, user_request: str) -> bool:
        ...
        # 检查复杂性关键词数量
        keyword_count = sum(1 for keyword in self.complexity_keywords if keyword in user_lower)
        
        # 检查长度和动作词数量
        word_count = len(user_request.split())
        action_words = ["分析", "研究", "设计", "开发", "实现", "制定", "评估", "比较", "撰写", "构建", "建立", "创建", "探索", "探讨"]
        action_count = sum(1 for word in action_words if word in user_lower)
        
        # 如果多个复杂关键词 或 多个动作词 或 句子长度较长且包含动作词，则为复杂任务
        return (keyword_count >= 2) or (action_count >= 2) or (word_count > 10 and action_count >= 1)
...
{{
        ...
        {{
            ...
            try:
                ...
                if not sprint.tasks:  # 如果AI分解失败，使用规则分解
                    sprint = self._decompose_by_rules(original_goal, sprint_id, sprint_name)
    ...
    def _decompose_by_rules(self, original_goal: str, sprint_id: str, sprint_name: str) -> Sprint:
        """基于规则的任务分解"""
        ...
        print(f"[DECOMPOSER] 规则分解得到 {len(sprint.tasks)} 个子任务")
        return sprint
...
class TaskExecutor:
    ...
    async def execute_task(self, task: AgileTask, original_goal: str, 
                          previous_results: List[Dict[str, Any]], 
                          task_memory: AgileTaskMemory) -> str:
        """执行单个任务"""
        if self.model_provider:
            # 构建任务上下文（包含前期结果）
            context_parts = []
...
class SprintExecutor:
    ...
    async def execute_sprint(self, sprint: Sprint) -> Dict[str, Any]:
        ...
        # 保存执行历史
        history_file = self.task_memory.save_execution_history(sprint.id, self.execution_history)
        ...
        return {
            ...
            "completed_tasks": len([t for t in sprint.tasks if t.status == TaskStatus.DONE.value]),
    ...
    async def _synthesize_final_result(self, sprint: Sprint, execution_results: List[Dict[str, Any]]) -> str:
        """合成最终结果"""
        ...
        if self.model_provider:
            ...
            for result in execution_results:
                ...
                result_preview = result['result'][:300] if result['result'] else "无结果"  # 截取前300字符
...
class AgileTaskSystemCore:
    ...
    async def process_request(self, user_request: str) -> AsyncGenerator[str, None]:
        ...
        # 检查是否已有相关会话
        existing_sprint = self.executor.task_memory.get_latest_sprint_for_goal(user_request)
        ...
        if existing_sprint and existing_sprint.status != "completed":
            ...
            completed_tasks = len([t for t in existing_sprint.tasks if t.status == TaskStatus.DONE.value])
            
            yield f"[BLUE]📋 当前进度: {completed_tasks}/{total_tasks} 任务完成[/BLUE]"
            
            # 继续执行未完成的任务
            incomplete_tasks = [task for task in existing_sprint.tasks if task.status != TaskStatus.DONE.value]
            ...
            if incomplete_tasks:
                ...
                for i, task in enumerate(incomplete_tasks, 1):
                    ...
                    yield f"[CYAN]🔄 执行任务 {i}/{len(incomplete_tasks)}: {task.title}[/CYAN]"
                ...
                final_result = await self.executor._synthesize_final_result(existing_sprint, completed_tasks_data)
                
                yield f"\\n[FINAL]🎉 任务完成!\\n{final_result}"
            else:
                # 所有任务已完成
                final_task = existing_sprint.tasks[-1] if existing_sprint.tasks else None
                yield f"[INFO] 所有相关任务已完成。最终结果:\\n{final_task.result if final_task and final_task.result else '无结果'}"
        else:
            # 创建新会话
    ...
    async def _execute_sprint_with_updates(self, sprint: Sprint) -> AsyncGenerator[str, None]:
        ...
        final_result = await self.executor._synthesize_final_result(sprint, execution_results)
        yield f"\\n[FINAL] 🎉 **最终结果**:\\n{final_result}"
    ...
    def _generate_task_list_display(self, tasks: List[AgileTask]) -> str:
        ...
        pending = len([t for t in tasks if t.status in [TaskStatus.BACKLOG.value, TaskStatus.READY.value]])
