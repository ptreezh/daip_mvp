...
class TaskListManager:
    ...
    async def update_task_status(self, task_id: str, status: TaskStatus, result: Optional[str] = None, error: Optional[str] = None):
        """更新任务状态"""
        async with self._lock:
    ...
    def generate_task_list_display(self) -> str:
        ...
        display += f"📈 **进度**: {stats['completed']}/{stats['total']} 任务完成 ({stats['completed']/stats['total']*100:.1f}%)" if stats['total'] > 0 else ""
...
class ComplexityDetector:
    ...
    def __init__(self, model_provider=None):
        ...
        self.complexity_keywords = [
            ...
            "实现", "比较", "总结", "撰写", "制定", "规划", "探索", "审查",
            "验证", "测试", "优化", "改进", "建立", "组织", "管理", 
            # 形容词/副词类  
            "复杂", "困难", "详细", "深入", "全面", "系统", "整体", "多方面",
            "多层次", "多维度", "综合性", "战略性", "创新性", "重要", "关键",
            "核心", "主要", "显著", "深入", "广泛", "长远", "持续"
        ]
    ...
    async def _detect_by_rules(self, user_request: str) -> bool:
        ...
        # 检查复杂性关键词
        complexity_count = sum(1 for keyword in self.complexity_keywords if keyword in user_request)
        ...
        # 检查句子复杂度
        action_words = ["分析", "设计", "实现", "开发", "研究", "评估", "验证", "比较", "总结", "制定", "构建"]
        action_count = sum(1 for word in action_words if word in user_request)
        
        # 如果有多于2个动作词，或有多个复杂性关键词，或匹配复杂任务模式，则认为是复杂任务
...
class TaskDecomposer:
    ...
    async def decompose_task(self, user_request: str) -> List[TaskItem]:
        """将复杂任务分解为多个子任务"""
        print(f"[TASK DECOMPOSER] 正在分解任务: {user_request[:50]}...")
...
5. 优先级按重要性排序"""
        ...
        try:
            ...
            print(f"[TASK DECOMPOSER] 智能分解得到 {len(tasks)} 个子任务")
            return tasks
            
        except Exception as e:
            print(f"[TASK DECOMPOSER] 智能分解失败: {e}，使用规则分解")
            return self._decompose_with_rules(user_request)
    ...
    def _decompose_with_rules(self, user_request: str) -> List[TaskItem]:
        ...
        # 根据任务类型生成通用的分解步骤
        if any(word in user_request.lower() for word in ["分析", "research", "study", "investigate"]):
            steps = [
        ...
        elif any(word in user_request.lower() for word in ["设计", "design", "develop", "create"]):
            steps = [
                ("需求分析", "明确设计或开发的具体需求", 5),
                ("方案设计", "设计实施方案或架构", 4),
                ("实现执行", "执行设计方案", 5),
        ...
        elif any(word in user_request.lower() for word in ["比较", "compare", "evaluate", "assess"]):
            steps = [
                ("标准制定", "确定比较或评估的标准", 4),
                ("信息收集", "收集各选项的相关信息", 4),
                ("对比分析", "进行详细的对比分析", 5),
        ...
        print(f"[TASK DECOMPOSER] 规则分解得到 {len(tasks)} 个子任务")
        return tasks
...
class TaskExecutionCoordinator:
    ...
    async def execute_task_list(self, task_list_manager: TaskListManager, original_request: str) -> AsyncGenerator[AgentEvent, None]:
        """执行任务清单中的所有任务"""
        print(f"[COORDINATOR] 开始执行任务清单，共 {len(task_list_manager.tasks)} 个任务")
        ...
        for i, task in enumerate(task_list_manager.tasks):
            ...
            yield ThoughtEvent(content=f"🔄 **正在执行任务 {i+1}/{len(task_list_manager.tasks)}: {task.title}**\n\n{updated_display}")
            
            print(f"[COORDINATOR] 执行任务 {i+1}: {task.title}")
            
            try:
                # 执行单个任务
                result = await self._execute_single_task(task, original_request, task_list_manager)
                
                # 更新任务状态为完成
                await task_list_manager.update_task_status(task.id, TaskStatus.COMPLETED, result=result)
                
                print(f"[COORDINATOR] 任务完成: {task.title}")
                
            except Exception as e:
                # 更新任务状态为失败
                error_msg = str(e)
                await task_list_manager.update_task_status(task.id, TaskStatus.FAILED, error=error_msg)
                
                print(f"[COORDINATOR] 任务失败: {task.title}, 错误: {error_msg}")
                
                # 发送错误信息
                yield ThoughtEvent(content=f"❌ **任务执行失败**: {task.title}\n错误: {error_msg}")
        ...
        final_summary = await self._generate_final_summary(task_list_manager, original_request)
        yield ThoughtEvent(content=final_summary)

    async def _execute_single_task(self, task: TaskItem, original_request: str, task_list_manager: TaskListManager) -> str:
        """执行单个任务"""
        if self.model_provider:
            # 构建上下文
...
请专注完成当前子任务，并提供具体的结果或答案。"""
    ...
    def _build_context_string(self, task_list_manager: TaskListManager) -> str:
        """构建任务执行上下文"""
        completed_tasks = [t for t in task_list_manager.tasks if t.status == TaskStatus.COMPLETED]
        ...
        for task in completed_tasks[-3:]:  # 只取最后3个已完成的任务
            result_preview = task.result[:100] if task.result and len(task.result) > 100 else (task.result or "无结果")
            context_parts.append(f"已完成: {task.title} -> {result_preview}")
        
        return "; ".join(context_parts)

    async def _generate_final_summary(self, task_list_manager: TaskListManager, original_request: str) -> str:
        """生成最终总结"""
        if self.model_provider:
            completed_results = []
            ...
            results_summary = "\n".join(completed_results) if completed_results else "无完成的任务结果"
...
class AutoTaskDecompositionEngine:
    ...
    async def process_with_task_decomposition(self, user_request: str) -> AsyncGenerator[AgentEvent, None]:
        """使用任务分解处理复杂请求"""
        print(f"[ENGINE] 开始处理复杂任务: {user_request[:50]}...")
        ...
        for task in tasks:
            await self.task_list_manager.add_task(task.title, task.description, task.priority)
        
        # 3. 执行任务清单
        async for event in self.task_execution_coordinator.execute_task_list(self.task_list_manager, user_request):
            yield event
    ...
    "tasks": [
        ...
        },
        {
            ...
            "priority": 3
                ...
                return "根据各项任务的执行结果，已完成了对原始请求的全面分析和处理。各项子任务均已按计划完成，达到了预期目标。"
            else:
        ...
        if should_decompose:
            ...
            async for event in engine.process_with_task_decomposition(request):
                ...
                if hasattr(event, 'content'):
                    if "📋 **任务清单**" in str(event.content) or "🔄 **正在执行任务" in str(event.content):
                        print(f"   显示任务状态更新")
