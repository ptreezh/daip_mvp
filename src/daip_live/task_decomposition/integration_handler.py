...
from daip_live.core.models import AgentEvent, ThoughtEvent, FinalResponseEvent
from daip_live.task_decomposition.automatic_task_decomposition_engine import AutoTaskDecompositionEngine, TaskListManager
...
class IntegratedTaskDecompositionHandler:
    ...
    def __init__(self, model_provider=None, memory_service=None):
        ...
        self.active_task_list_managers: Dict[str, TaskListManager] = {}  # 按会话跟踪任务清单
    
    async def should_handle_with_task_decomposition(self, user_request: str) -> bool:
        """判断是否应该使用任务分解处理此请求"""
        return await self.auto_decomposition_engine.should_process_with_task_decomposition(user_request)
    ...
    async def process_with_task_decomposition(
        ...
        与现有状态循环兼容，按顺序生成事件
        """
        print(f"[INTEGRATED_HANDLER] 开始处理复杂任务分解: {user_request[:50]}...")
        ...
        # 1. 分解任务
        tasks = await self.auto_decomposition_engine.task_decomposer.decompose_task(user_request)
        print(f"[INTEGRATED_HANDLER] 分解得到 {len(tasks)} 个子任务")
        
        # 添加任务到管理器
        for task in tasks:
            await task_list_manager.add_task(task.title, task.description, task.priority)
        ...
        # 3. 执行任务清单中的任务
        print(f"[INTEGRATED_HANDLER] 开始顺序执行 {len(task_list_manager.tasks)} 个任务")
...
**进度**: {i+1}/{len(task_list_manager.tasks)} 任务完成"""
        ...
        print(f"[INTEGRATED_HANDLER] 任务分解流程完成")
    ...
    async def _execute_single_task(self, task, original_request, task_list_manager):
        ...
        if self.model_provider:
            # 构建上下文
            context_parts = []
            completed_tasks = [t for t in task_list_manager.tasks if t.status == "completed" and t.result]
            for prev_task in completed_tasks[-2:]:  # 包含最近2个已完成任务的上下文
                context_parts.append(f"已完成: {prev_task.title} -> {prev_task.result[:100]}...")
...
def enhance_agent_executor_with_task_decomposition(agent_executor, model_provider=None):
    ...
    """
    # 创建任务分解处理器
    task_handler = IntegratedTaskDecompositionHandler(model_provider, agent_executor.memory_service)
    
    # 保存原始执行方法
    original_run = agent_executor.run
    
    # 创建增强的运行方法
    async def enhanced_run(goal: str, workflow_definition=None) -> AsyncGenerator[AgentEvent, None]:
        """增强的运行方法，支持任务分解"""
        
        # 首先检测是否是复杂任务
        should_decompose = await task_handler.should_handle_with_task_decomposition(goal)
        
        if should_decompose:
            print(f"[ENHANCED_RUNNER] 检测到复杂任务，使用任务分解处理: {goal[:50]}...")
            
            # 使用任务分解处理
            async for event in task_handler.process_with_task_decomposition(goal, getattr(agent_executor, 'session', {}).get('session_id', 'default')):
                yield event
        else:
            # 使用原始流程
            print(f"[ENHANCED_RUNNER] 使用原流程处理: {goal[:50]}...")
...
# 集成到意图识别器
def enhance_intent_recognizer_for_task_decomposition(intent_recognizer, model_provider=None):
    """
    增强意图识别器以支持任务分解
    """
    # 保存原始识别方法
    ...
    if model_provider:
        ...
        intent_recognizer.task_decomposition_engine = AutoTaskDecompositionEngine(model_provider)
