...
class TaskDecompositionEngine:
    ...
    async def should_decompose_task(self, user_request: str) -> bool:
        ...
        # 检查是否包含复杂任务关键词
        complex_keywords = [
            "系统", "平台", "框架", "架构", "方案", "模型", "流程", "机制", "体系", "结构",
            "多", "多种", "多个", "复合", "综合", "全方位", "智能化", "自动化", "高级", "深度"
        ]
...
回复格式：YES 或 NO"""
    ...
    async def decompose_task(self, user_request: str) -> List[DecomposedTask]:
        ...
        将复杂任务分解为多个可执行的任务
        """
        print(f"[TASK DECOMPOSER] 正在分解任务: {user_request}")
...
- 优先级从高到低排列关键任务"""
        ...
        try:
            ...
            print(f"[TASK DECOMPOSER] AI分解得到 {len(tasks)} 个子任务")
            return tasks
            
        except Exception as e:
            print(f"[TASK DECOMPOSER] AI分解失败，回退到规则分解: {e}")
            return self._decompose_with_rules(user_request)
    ...
    def _decompose_with_rules(self, user_request: str) -> List[DecomposedTask]:
        ...
        if any(word in request_lower for word in ["分析", "research", "study", "investigate"]):
            # 分析类任务的分解
            steps = [
        ...
        elif any(word in request_lower for word in ["设计", "design", "develop", "create"]):
            # 设计/开发类任务的分解
            steps = [
                ("需求分析", "明确设计或开发的具体需求"),
                ("方案设计", "设计实施方案或架构"),
        ...
        elif any(word in request_lower for word in ["比较", "compare", "evaluate", "assess"]):
            # 比较评估类任务的分解
            steps = [
                ("标准制定", "确定比较或评估的标准"),
                ("信息收集", "收集各选项的相关信息"),
        ...
        for title, description in steps:
            task = DecomposedTask(
                ...
                priority=TaskPriority.HIGH if title in ["执行过程", "实现执行"] else TaskPriority.MEDIUM
            )
            tasks.append(task)
...
class SequentialTaskExecutor:
    ...
    async def execute_decomposed_tasks(self, tasks: List[DecomposedTask], original_request: str) -> Dict[str, Any]:
        """执行分解后的任务列表"""
        self.tasks = tasks
        self.original_request = original_request

        print(f"[TASK EXECUTOR] 开始执行 {len(tasks)} 个分解任务")
        ...
        for i, task in enumerate(self.tasks):
            print(f"[TASK EXECUTOR] 执行任务 {i+1}/{len(tasks)}: {task.title}")
            ...
            if await self._check_dependencies(task):
                ...
                if task_result["success"]:
                    ...
                    # 对于失败的任务，可以选择继续或停止
                    print(f"[TASK EXECUTOR] 任务执行失败: {task.error}")
            else:
                print(f"[TASK EXECUTOR] 任务依赖未满足，跳过: {task.title}")
        ...
        print(f"[TASK EXECUTOR] 任务执行完成 - 成功: {results['completed_tasks']}, 失败: {results['failed_tasks']}")
        return results

    async def execute_decomposed_tasks_with_visualization(self, tasks: List[DecomposedTask], original_request: str, visualization_manager) -> Dict[str, Any]:
        """执行分解后的任务列表，并实时更新可视化界面"""
        self.tasks = tasks
        self.original_request = original_request

        print(f"[TASK EXECUTOR] 开始执行 {len(tasks)} 个分解任务 (带可视化)")
        ...
        for i, task in enumerate(self.tasks):
            ...
            if await self._check_dependencies(task):
                ...
                if task_result["success"]:
                    ...
                    # 更新可视化：标记为已完成
                    visualization_manager.update_task_status(task.id, TaskStatus.COMPLETED, task_result["result"])
                    visualization_manager.update_and_display(tasks, original_request)
                    ...
                    # 更新可视化：标记为失败
                    visualization_manager.update_task_status(task.id, TaskStatus.FAILED, task_result.get("error", "Unknown error"))
                    visualization_manager.update_and_display(tasks, original_request)
                ...
                # 更新可视化：标记为跳过
                visualization_manager.update_task_status(task.id, TaskStatus.SKIPPED, "Dependencies not met")
                visualization_manager.update_and_display(tasks, original_request)
    ...
    async def _execute_single_task(self, task: DecomposedTask, original_request: str) -> Dict[str, Any]:
        """执行单个任务"""
        try:
    ...
    async def _synthesize_final_result(self, execution_results: Dict[str, Any]) -> str:
        ...
        if self.model_provider:
            ...
            task_summaries = "\\n".join([
                ...
                for tr in task_results if tr['status'] == 'completed'
            ])
...
请提供一个完整、连贯的回答，整合所有子任务的结果。"""
            ...
            # 简单合成结果
            return f"任务分解执行完成。总共执行了 {execution_results['total_tasks']} 个子任务。"
...
6. 部署上线: 将系统部署到生产环境"""
                ...
                return f"执行结果: {prompt.split('子任务标题:')[1].split()[0] if '子任务标题:' in prompt else '模拟结果'}"
            else:
    ...
    for request in test_requests:
        ...
        if should_decompose:
            ...
            results = await executor.execute_decomposed_tasks(tasks, request)
            
            print(f"   执行完成 - 成功: {results['completed_tasks']}, 失败: {results['failed_tasks']}")
            print(f"   最终结果预览: {results['final_result'][:100]}...")
