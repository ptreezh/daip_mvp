...
from daip_live.task_decomposition.advanced_context_manager import (
    SubTaskContext, TaskContext as EnhancedTaskContext, TaskStatus,
    get_context_manager, AdvancedTaskOrchestrator
)
from daip_live.task_decomposition.task_decomposition_engine import (
    TaskDecompositionEngine, SequentialTaskExecutor, DecomposedTask
)
...
class TaskManager:
    ...
    def __init__(self, model_provider=None):
        ...
        self.orchestrator = AdvancedTaskOrchestrator(self.context_manager, model_provider)
    ...
    async def handle_complex_task(self, user_request: str) -> Dict[str, Any]:
        ...
        :return: 执行结果
        """
        print(f"[TASK_MANAGER] 开始处理复杂任务: {user_request[:50]}...")
    ...
    async def _decompose_task(self, user_request: str) -> List[SubTaskContext]:
        """分解任务为子任务"""
        print(f"[TASK_MANAGER] 正在分解任务: {user_request[:30]}...")
        ...
        print(f"[TASK_MANAGER] 任务分解完成，生成 {len(subtasks)} 个子任务")
        return subtasks

    async def _create_task_context(self, user_request: str, subtasks: List[SubTaskContext]) -> EnhancedTaskContext:
        """创建任务上下文"""
        ...
        print(f"[TASK_MANAGER] 任务上下文创建完成，ID: {task_id}")
        return task_context

    async def _execute_task_list(self, task_context: EnhancedTaskContext) -> Dict[str, Any]:
        """执行任务列表"""
        print(f"[TASK_MANAGER] 开始执行任务列表，共 {len(task_context.subtasks)} 个子任务")
        ...
        for i, subtask in enumerate(task_context.subtasks):
            print(f"[TASK_MANAGER] 执行子任务 {i+1}/{len(task_context.subtasks)}: {subtask.title}")
        ...
        # 生成最终结果
        execution_results["summary"] = await self._synthesize_final_result(execution_results)

        print(f"[TASK_MANAGER] 任务列表执行完成 - 成功: {execution_results['completed_subtasks']}, 失败: {execution_results['failed_subtasks']}")

        return execution_results

    async def _execute_single_subtask(self, subtask: SubTaskContext, task_context: EnhancedTaskContext) -> Dict[str, Any]:
        """执行单个子任务"""
        try:
            print(f"[TASK_MANAGER] 执行子任务: {subtask.title}")
...
"""
            ...
            print(f"[TASK_MANAGER] 子任务执行失败: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    async def _synthesize_final_result(self, execution_results: Dict[str, Any]) -> str:
        ...
        if self.model_provider:
            ...
            task_summaries = "\\n".join([
                ...
                for sr in subtask_results if sr['success']
            ])
...
请提供一个完整、连贯的回答，整合所有子任务的结果。"""
            ...
            try:
                ...
                print(f"[TASK_MANAGER] AI合成最终结果失败: {e}")
                # 如果AI合成失败，返回任务结果摘要
            ...
            # 简单合成结果
            return f"任务分解执行完成。总共执行了 {execution_results['total_subtasks']} 个子任务，其中 {execution_results['completed_subtasks']} 个成功，{execution_results['failed_subtasks']} 个失败。"
        ...
        if task_context:
            ...
            for subtask in task_context.subtasks:
                if subtask.status in [TaskStatus.PENDING, TaskStatus.IN_PROGRESS]:
                    self.memory_service.update_subtask_status(subtask.subtask_id, TaskStatus.SKIPPED)
