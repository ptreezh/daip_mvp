    ...
    def get_tasks_by_status(self, status: TaskStatus) -> List[NestedTodoItem]:
        ...
        if context:
            if status == TaskStatus.COMPLETED:
                ...
                def find_tasks_with_status(tasks: List[NestedTodoItem], target_status: TaskStatus):
                    for task in tasks:
...
class HierarchicalTaskDecomposer:
    ...
    def decompose_task(task_description: str, level: int = 0) -> List[NestedTodoItem]:
        ...
        elif "写" in task_description or "撰写" in task_description or "create" in task_description.lower():
            # 写作类任务分解
            if level == 0:
...
if __name__ == "__main__":
    ...
    print(f"上下文: {hierarchy['name']}")
    print(f"总任务数: {hierarchy['total_tasks']}")
    print(f"活跃任务: {hierarchy['active_tasks']}")
    print(f"已完成: {hierarchy['completed_tasks']}")
    
    # 显示根任务
    for root_task in context.root_tasks:
        print(f"\\n根任务: {root_task.title}")
        print(f"  描述: {root_task.description}")
        print(f"  状态: {root_task.status.value}")
        print(f"  子任务数量: {len(root_task.subtasks)}")
        
        for subtask in root_task.subtasks[:2]:  # 只显示前两个
            print(f"    子任务: {subtask.title} (状态: {subtask.status.value})")
            print(f"      细节步骤: {len(subtask.details)} 个")
