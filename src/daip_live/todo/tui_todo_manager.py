...
class TUITodoManager(TodoManager):
    ...
    def _format_task_display(self, task: NestedTodoItem, depth: int = 0) -> str:
        ...
        if task.description:
            display += f"\\n{indent}   📝 {task.description[:100]}{'...' if len(task.description) > 100 else ''}"
        
        return display
    
    def _format_task_with_subtasks(self, task: NestedTodoItem, depth: int = 0, show_details: bool = True) -> str:
        """格式化任务及子任务显示"""
    ...
    def _format_task_list(self, tasks: List[NestedTodoItem], depth: int = 0, show_details: bool = True) -> str:
        """格式化任务列表显示"""
        result = ""
        for task in tasks:
  ...
  /todo help - 显示帮助"""
    ...
    def list_tasks(self, filter_type: str = "") -> None:
        ...
        if not context:
            self._update_tui_log("[bold yellow]> 没有激活的待办列表上下文[/bold yellow]")
            return
        ...
        for i, task in enumerate(tasks[:20]):  # 限制显示前20个任务
            try:
                ...
                self._update_tui_log(f"  📝 任务 {task.title[:50]}... (无法完全显示: {e})")
    ...
    def add_task(self, task_spec: str) -> None:
        ...
        if self.add_task_to_current_context(new_task):
            self._update_tui_log(f"[bold green]> ✅ 任务已添加: {new_task.title}[/bold green]")
    ...
    def complete_task(self, task_id: str) -> None:
        ...
        if task:
            task.mark_completed()
            self._update_tui_log(f"[bold green]> ✅ 任务已完成: {task.title}[/bold green]")
        else:
    ...
    def cancel_task(self, task_id: str) -> None:
        ...
        if task:
            task.mark_cancelled()
            self._update_tui_log(f"[bold yellow]> 🚫 任务已取消: {task.title}[/bold yellow]")
        else:
            self._update_tui_log(f"[bold red]> ❌ 未找到任务: {task_id}[/bold red]")
    ...
    def clear_completed_tasks(self) -> None:
        ...
        self._update_tui_log(f"[bold green]> 🗑️ 清除已完成任务: {cleared_count} 个[/bold green]")
    ...
    def manage_context(self, args: str) -> None:
        ...
        if not args_list:
            ...
            if self.current_context:
                context = self.get_current_context()
                if context:
                    self._update_tui_log(f"[bold blue]> 当前上下文: {context.name}[/bold blue]")
        ...
        if command == "list":
            ...
            if self.switch_context(target_ctx):
                self._update_tui_log(f"[bold green]> ✅ 上下文切换到: {target_ctx}[/bold green]")
            else:
                self._update_tui_log(f"[bold red]> ❌ 未找到上下文: {target_ctx}[/bold red]")
        elif command == "create" and len(args_list) > 1:
            name = " ".join(args_list[1:])
            new_ctx = self.create_context(name)
            self._update_tui_log(f"[bold green]> ✅ 创建新上下文: {name} (ID: {new_ctx.owner_id})[/bold green]")
        else:
            self._update_tui_log("[bold red]> 用法: /todo context [list|switch <id>|create <name>][/bold red]")
    ...
    def decompose_task(self, task_desc: str) -> None:
        ...
        if not subtasks:
            self._update_tui_log(f"[bold yellow]> ⚠️ 无法分解任务: {task_desc}[/bold yellow]")
            return
        ...
        if self.add_task_to_current_context(main_task):
            self._update_tui_log(f"[bold green]> ✅ 任务分解完成: {main_task.title}[/bold green]")
