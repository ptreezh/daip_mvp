  ...
  [green]/debate export json --include-memory[/green]
    ...
    def _handle_start_command(self, args: str) -> None:
        ...
        try:
            ...
            parser.add_argument('--optimization', type=str, choices=['basic', 'standard', 'full'],
                             help='优化级别', default='standard')
            parser.add_argument('--memory-system', type=str, choices=['basic', 'advanced'],
                             help='记忆系统类型', default='standard')
            ...
            if not topic.strip():
                self.tui._update_log_view("[bold red]> 错误: 必须提供辩论主题[/bold red]")
                return
            ...
            # 启动增强辩论
            self._start_enhanced_debate(topic, roles, rounds, {
                'models': models,
            ...
            self.tui._update_log_view("[dim]使用 /debate start --help 查看详细帮助[/dim]")
    ...
    def _handle_export_command(self, args: str) -> None:
        ...
        if format_type not in valid_formats:
            ...
            self.tui._update_log_view(f"[dim]支持的格式: {', '.join(valid_formats)}[/dim]")
            return

        self.tui._update_log_view(f"[cyan]📤 准备导出辩论结果为 {format_type} 格式...[/cyan]")
    ...
    def _handle_switch_model_command(self, args: str) -> None:
        """处理模型切换命令"""
        if not args.strip():
            self.tui._update_log_view("[red]❌ 请提供模型切换配置，例如: philosopher=ollama/llama3:instruct[/red]")
            return
        ...
        try:
            ...
            if hasattr(self.tui, '_debate_manager') and self.tui._debate_manager:
                if hasattr(self.tui._debate_manager, 'switch_role_model'):
                    ...
                    self.tui._update_log_view("[red]❌ 当前辩论管理器不支持动态模型切换[/red]")
            else:
                self.tui._update_log_view("[red]❌ 没有进行中的辩论[/red]")
    ...
    def _start_enhanced_debate(self, topic: str, roles: str, rounds: int, options: dict) -> None:
        """启动增强辩论"""
        try:
            # 检查TUI是否有辩论管理器
            ...
            if hasattr(self.tui, '_start_debate') and callable(self.tui._start_debate):
                ...
                if loop.is_running():
                    ...
                    loop.run_until_complete(self.tui._start_debate(topic, roles, rounds))
            else:
    ...
    def _display_debate_status(self, status: Dict[str, Any]) -> None:
        ...
        if 'optimization_level' in status:
            self.tui._update_log_view(f"[dim]优化级别: {status['optimization_level']}[/dim]")
...
class SearchCommands:
    ...
    def search_conversation_history(self, query: str) -> None:
        ...
        try:
            ...
            # 获取所有会话
            all_sessions = self.tui._session_manager.list_sessions() if hasattr(self.tui, '_session_manager') else []
            if not all_sessions:
            ...
            # 显示搜索结果
            if found_results:
                self.tui._update_log_view(f"[bold green]📋 找到 {len(found_results)} 个相关会话：[/bold green]")
                self.tui._update_log_view("")
                ...
                for i, result in enumerate(found_results[:10], 1):  # 最多显示前10个结果
                    ...
                    self.tui._update_log_view(f"[bold cyan]{i}. 会话 {session.id[:8]}...[/bold cyan] (相关性: {relevance})")
                    if session.goal:
                        self.tui._update_log_view(f"[dim]   目标: {session.goal}[/dim]")
                    if session.summary:
                        self.tui._update_log_view(f"[dim]   摘要: {session.summary}[/dim]")
                    self.tui._update_log_view("")
            else:
                self.tui._update_log_view("[yellow]📚 未找到相关的历史会话[/yellow]")

        except Exception as e:
            self.tui._update_log_view(f"[bold red]❌ 搜索历史对话时发生错误: {str(e)}[/bold red]")
