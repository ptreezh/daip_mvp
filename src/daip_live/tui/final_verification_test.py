...
class TestFinalVerification(unittest.TestCase):
    ...
    def test_all_core_functionality_exists(self):
        ...
        with patch(
            ...
            # 验证所有主要命令处理方法都存在
            self.assertTrue(
                hasattr(tui, "_handle_search_command"), "搜索命令处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_debate_command"), "辩论命令处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_model_command"), "模型命令处理方法存在"
            )
            self.assertTrue(hasattr(tui, "_handle_doc_command"), "文档命令处理方法存在")
            self.assertTrue(
                hasattr(tui, "_handle_wiki_command"), "Wiki命令处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_permission_command"), "权限命令处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_role_command"), "角色命令处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_knowledge_command"), "知识库命令处理方法存在"
            )
            ...
            # 验证论文功能存在
            self.assertTrue(
                hasattr(tui, "_handle_paper_search"), "论文搜索处理方法存在"
            )
            self.assertTrue(
                hasattr(tui, "_handle_paper_download"), "论文下载处理方法存在"
            )

            # 验证知识库功能存在
            self.assertTrue(
                hasattr(tui, "_handle_knowledge_search"), "知识库搜索处理方法存在"
            )
