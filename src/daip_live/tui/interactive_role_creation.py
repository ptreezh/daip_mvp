...
class RoleManagerAdapter:
    ...
    def save_role(self, role_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not (hasattr(self.role_manager, 'name') and 'mock' in str(type(self.role_manager)).lower()):
...
class InteractiveRoleCreationService:
    ...
    def start_creation(self, user_query: str) -> RoleCreationResponse:
        ...
        try:
            ...
            # 使用AI生成角色
            suggested_role = self._ai_generator.generate_role_from_description(user_query)
            ...
            return RoleCreationResponse(
                status="error",
                message=f"创建角色时发生错误: {e}"
            )
    
    def continue_creation(self, session_id: str, input_data: Dict[str, Any]) -> RoleCreationResponse:
        """继续角色创建流程"""
        try:
