...
def demo_interactive_role_creation():
    ...
    if response.status == "success":
        service.continue_creation(response.session_id, {"confirm": True})
    ...
    if response.status == "success":
        custom_role = {
            "name": "高级企业法律顾问",
            "persona": "资深法律专家，专精企业合规、合同审查和知识产权保护，具备丰富的商业法律实务经验",  # noqa: E501
            "tools": ["法规数据库", "合同分析工具", "风险评估系统", "案例检索"],
        }

        modify_response = service.continue_creation(
            response.session_id, {"updated_role": custom_role}
        )
