...
class TestClaudeSkillsBackendFunctionality(unittest.TestCase):
    ...
    def test_claude_skills_adapter_manager_initialization_without_container(self):
        ...
        with patch('daip_live.tui.simplified_main.get_container', side_effect=Exception("Container not available")):
            with patch('daip_live.skills.claude_skill_adapter.ClaudeSkillAdapterManager') as mock_adapter_class:
                # 设置模拟适配器
    ...
    def test_claude_skills_list_command_uses_real_system(self):
        ...
        with patch('daip_live.tui.simplified_main.get_container', return_value=mock_container):
            with patch('daip_live.skills.claude_skill_adapter.ClaudeSkillAdapterManager') as mock_adapter_class:
                # 模拟Claude技能适配器
                mock_adapter_instance = Mock()
                mock_adapter_instance.list_claude_skills.return_value = ["skill1", "skill2"]
                mock_adapter_class.return_value = mock_adapter_instance
