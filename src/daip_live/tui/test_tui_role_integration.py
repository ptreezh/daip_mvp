...
class TestTUIInteractiveRoleCreationIntegration(unittest.TestCase):
    ...
    def test_complete_role_creation_flow(self):
        ...
        # 2. 确认角色创建
        confirm_response = self.service.continue_creation(
    ...
    def test_role_creation_with_customization(self):
        ...
        modify_response = self.service.continue_creation(
            response.session_id, {"updated_role": updated_role}
        )
    ...
    def test_invalid_session_handling(self):
        """测试无效会话处理"""
        response = self.service.continue_creation(
            "invalid_session_id", {"confirm": True}
        )
