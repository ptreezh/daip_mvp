...
class TUIRoleCommandHandler:
    ...
    def __init__(
        self, tui_instance, role_creation_service: InteractiveRoleCreationService
    ):
    ...
    def _handle_role_interactive(self, user_query: str) -> None:
        ...
        if not user_query.strip():
            self.tui._update_log_view(
                "[bold cyan]🎭 开始交互式角色创建向导...[/bold cyan]"
            )
            self.tui._update_log_view("[dim]请输入您想要创建的角色描述：[/dim]")
            self.tui._update_log_view(
                "[dim]例如：'数据科学家'、'法律顾问'、'编程导师'等[/dim]"
            )
            # 设置TUI状态为角色创建模式
        ...
        # 直接启动角色创建流程
        self.tui._update_log_view(
            f"[bold cyan]🎭 正在分析您的角色需求: {user_query}[/bold cyan]"
        )
    ...
    async def _process_role_creation(self, user_query: str) -> None:
        ...
        try:
            ...
            if response.status == "success":
                self._display_role_suggestion(
                    response.suggested_role, response.session_id
                )
    ...
    def _display_role_suggestion(
        self, suggested_role: dict[str, Any], session_id: str
    ) -> None:
        """显示角色建议"""
        self.tui._update_log_view("[green]✅ AI生成的角色建议:[/green]")
        ...
        self.tui._update_log_view(
            f"[bold]推荐工具:[/bold] {', '.join(suggested_role['tools']) if suggested_role['tools'] else '无'}"  # noqa: E501
        )
        ...
        self.tui._update_log_view(
            f"[dim]  /role confirm {session_id} - 确认创建此角色[/dim]"
        )
        self.tui._update_log_view(
            f"[dim]  /role modify {session_id} - 修改角色配置[/dim]"
        )
    ...
    def _handle_role_switch(self, role_name: str) -> None:
        ...
        try:
            ...
            if role:
                ...
                self.tui._update_log_view(
                    f"[green]✅ 已切换到角色: {role.name}[/green]"
                )
    ...
    def _handle_role_show(self, role_name: str) -> None:
        ...
        try:
            ...
            if role:
                self.tui._update_log_view(
                    f"[bold cyan]👤 角色详情: {role.name}[/bold cyan]"
                )
                self.tui._update_log_view(f"[dim]📋 人设: {role.persona}[/dim]")
                if role.tools:
                    self.tui._update_log_view(
                        f"[dim]🛠️  工具: {', '.join(role.tools)}[/dim]"
                    )
            else:
                self.tui._update_log_view(f"[red]❌ 角色 '{role_name}' 不存在[/red]")
        except Exception as e:
    ...
    def handle_role_confirm(self, session_id: str) -> None:
        ...
        try:
            # 调用服务完成角色创建
            response = self.role_creation_service.continue_creation(
    ...
    def _show_role_help(self) -> None:
        ...
        self.tui._update_log_view(
            "[dim]  /role interactive <描述> - 交互式创建角色[/dim]"
        )
