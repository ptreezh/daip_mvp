...
class DebateProgressWidget(Widget):
    ...
    def compose(self):
        ...
        progress_container.mount(Label("准备中...", classes="progress-header", id="progress-label"))
        progress_container.mount(ProgressBar(show_eta=False, show_percentage=True, id="progress-bar"))
    ...
    def update_participant_status(self, participant: str, status: str, color: str = "yellow") -> None:
        """更新参与者状态"""
    ...
    def _refresh_display(self) -> None:
        ...
        if self.debate_state['total_rounds'] > 0:
            progress_percentage = (self.debate_state['current_round'] / self.debate_state['total_rounds']) * 100
        else:
            progress_percentage = 0
        ...
        if current_participant and current_participant in self.debate_state['participant_status']:
            participant_status = self.debate_state['participant_status'][current_participant]
            if participant_status.get('status') == '思考中...':
                ...
                status_text.append(f"[green]💭 {current_participant} 思考中...{thinking_text}[/green]")
