...
class TUICommandProcessor:
    ...
    def _register_commands(self):
        ...
        # Removed ppt and survey commands - they will be handled through natural language processing
                ...
                if command_name == 'skill':
                    ...
                    if command.args:
                        ...
                        if len(command.args) > 1:
                            # If we have a second argument, treat as URL for download or skill name for info
    ...
    def _process_natural_language(self, input_text: str) -> str:
        """Process natural language input and automatically discover appropriate skills."""
        # Check if the input suggests PPT creation
        ppt_keywords = ['ppt', 'powerpoint', '演示文稿', '幻灯片', 'slide', 'deck', 'presentation', '报告', '汇报']
        survey_keywords = ['调查', '问卷', 'poll', 'survey', 'question', '问题', '满意度', '反馈']
        ...
        for keyword in ppt_keywords:
            if keyword in input_lower:
                ...
                import re
                title_match = re.search(r'--title\s+["\']([^"\']+)["\']|标题\s*[:：]\s*([^\n\r]+)', input_text)
                title = title_match.group(1) if title_match and title_match.group(1) else title_match.group(2) if title_match else "演示文稿"
                ...
                ppt_skills = [skill for skill in self.skill_manager.list_skills()
                             if any(k in skill.lower() for k in ['ppt', 'powerpoint', 'presentation', 'slide', 'deck', 'document'])]
        ...
        for keyword in survey_keywords:
            if keyword in input_lower:
                ...
                survey_skills = [skill for skill in self.skill_manager.list_skills()
                                if any(k in skill.lower() for k in ['survey', 'question', 'quiz', 'poll', 'feedback', 'form'])]
        ...
        return f"💡 我理解您想表达: {input_text[:50]}...\n如需特定功能，请尝试 /skill download 获取技能，或使用 /help 查看可用命令。"
    
    def _handle_skill_command(self, params: Dict[str, Any]) -> str:
        """Handle skill commands."""
        return self.skill_handler.handle_skill_command(params)
    
    # Removed individual command handlers as they're now handled through natural language processing
    # The PPT and Survey functionality is still available through the PPTSurveyCommandHandler
