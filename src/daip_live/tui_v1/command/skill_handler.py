...
class SkillCommandHandler:
    ...
    def __init__(
        self,
        skill_manager,
        claude_integration_service: Optional[EnhancedClaudeSkillsManager] = None,
    ):
    ...
    def _download_skills(self, params: dict[str, Any]) -> str:
        ...
        if not repo_url:
            ...
            default_repos = [
                ...
                "https://github.com/anthropics/claude-computer-use-tools",  # Another official  # noqa: E501
        ...
        normalized_url = (
            self.claude_integration_service.github_downloader._prepare_repo_url(
                repo_url
            )
        )
        ...
        try:
            ...
            async def download_async():
                return await self.claude_integration_service.load_skills_from_github(
                    normalized_url
                )

            downloaded_skills = asyncio.run(download_async())
            if downloaded_skills:
                return f"✅ 成功从 {normalized_url} 下载并加载了 {len(downloaded_skills)} 个技能: {', '.join(downloaded_skills)}"  # noqa: E501
            else:
                return f"⚠️ 从 {normalized_url} 未能下载到任何技能（可能该仓库不包含有效的Claude Skills）"  # noqa: E501
    ...
    def _discover_and_download_skills(self, repo_urls: list) -> str:
        ...
        for repo_url in repo_urls:
            try:
                ...
                async def download_async():
                    return (
                        await self.claude_integration_service.load_skills_from_github(
                            repo_url
                        )
                    )
                ...
                downloaded_skills = asyncio.run(download_async())
                if downloaded_skills:
                    result = f"✅ 成功从 {repo_url} 下载并加载了 {len(downloaded_skills)} 个技能: {', '.join(downloaded_skills)}"  # noqa: E501
    ...
    def _reload_skills(self) -> str:
        ...
        try:
            ...
            loaded_count = self.skill_manager.load_claude_skills_from_directory(
                "./claude_skills"
            )
