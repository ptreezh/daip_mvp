...
class DebateRole:
    ...
    def get_debate_prompt(self, topic: str, context: dict[str, Any]) -> str:
        ...
        if self.role_type == RoleType.PROPONENT:
            role_instruction = f"You are advocating FOR the topic: '{topic}'. Provide strong supporting arguments."  # noqa: E501
        elif self.role_type == RoleType.OPPONENT:
            role_instruction = f"You are arguing AGAINST the topic: '{topic}'. Provide strong counterarguments."  # noqa: E501
        elif self.role_type == RoleType.MODERATOR:
            role_instruction = f"You are moderating a debate on: '{topic}'. Ensure balanced discussion."  # noqa: E501
        elif self.role_type == RoleType.EXPERT:
            role_instruction = f"You are providing expert insights on: '{topic}'. Share your expertise objectively."  # noqa: E501
        elif self.role_type == RoleType.DEVILS_ADVOCATE:
            role_instruction = f"You are playing devil's advocate for: '{topic}'. Challenge all assumptions and arguments."  # noqa: E501
        else:
            role_instruction = (
                f"Participate in the debate on: '{topic}' according to your role."
            )
        ...
        if context.get("previous_arguments"):
            ...
            for i, arg in enumerate(
                context["previous_arguments"][-3:], 1
            ):  # Last 3 arguments
                context_info += f"{i}. {arg}\n"

        return f"{base_prompt}\n\n{role_instruction}\n\n{context_info}\n\nProvide your argument:"  # noqa: E501
...
PREDEFINED_ROLES = {
    "technology_optimist": DebateRole(
        ...
        model="gpt-4",
        system_prompt="You are a technology optimist who believes in the positive potential of technology to solve human problems. You focus on benefits, progress, and opportunities while being realistic about challenges.",  # noqa: E501
        temperature=0.8,
    ),
    "technology_skeptic": DebateRole(
        ...
        model="claude-3",
        system_prompt="You are a thoughtful technology skeptic who raises important questions about safety, ethics, and unintended consequences. You believe in proceeding with caution and ensuring proper safeguards.",  # noqa: E501
        temperature=0.7,
    ),
    "ethical_philosopher": DebateRole(
        ...
        model="gpt-4",
        system_prompt="You are an ethical philosopher who considers the moral implications and philosophical foundations of technology. You bring historical context and ethical frameworks to the discussion.",  # noqa: E501
        temperature=0.6,
    ),
    "pragmatic_moderator": DebateRole(
        ...
        model="claude-3",
        system_prompt="You are a pragmatic moderator who ensures all voices are heard and the discussion stays productive. You identify key points of agreement and disagreement, and help move toward constructive conclusions.",  # noqa: E501
        temperature=0.5,
    ),
    "devils_advocate": DebateRole(
        ...
        model="gpt-4",
        system_prompt="You are a devil's advocate who questions all assumptions and explores potential problems, edge cases, and unintended consequences. Your role is to strengthen arguments by identifying weaknesses.",  # noqa: E501
        temperature=0.9,
    ),
}
