...
class ModelMetrics:
    ...
    def _calculate_performance_score(self) -> float:
        ...
        cost_factor = max(0, 100 - (self.total_cost / max(1, self.successful_requests)) * 10)
...
class ModelSelector:
    ...
    def _calculate_model_score(self, model: ModelInfo, task_analysis: TaskAnalysis) -> Dict[str, float]:
        ...
        total_score = sum(
            scores[key] * weight
            for key, weight in self.selection_criteria.items()
        )
        scores['total'] = total_score

        return scores

    def _calculate_capability_match(self, model: ModelInfo, task_analysis: TaskAnalysis) -> float:
...
class ModelHealthMonitor:
    ...
    async def check_model_health(self, model: ModelInfo) -> bool:
        ...
        if (model.metrics.last_used and
            datetime.now() - model.metrics.last_used > timedelta(hours=1)):
...
class ProductionModelSystem:
    ...
    def switch_to_model(self, model_name: str, force: bool = False) -> bool:
        ...
        with self._lock:
            ...
            if not force and model.status != ModelStatus.ACTIVE:
                logger.warning(f"Model {model_name} is not active (status: {model.status})")
                return False
    ...
    async def intelligent_switch(
        ...
        should_switch = (
            force_switch or
            not current_model_info or
            best_model.name != current_model_info.name
        )
        ...
        if should_switch:
            ...
            return True, current_model_info, {
                "task_analysis": asdict(task_analysis),
                "selection_scores": score_details,
    ...
    def get_available_models(self, status_filter: Optional[ModelStatus] = None) -> List[ModelInfo]:
    ...
    def get_model_recommendations(self, task_description: str, limit: int = 5) -> List[Tuple[ModelInfo, Dict[str, float]]]:
        """Get model recommendations for a task"""
        task_analysis = self._analyze_task(task_description)
        available_models = [m for m in self.models.values() if m.status == ModelStatus.ACTIVE]
        ...
        with self._lock:
            if model_name in self.models:
                ...
                if new_status != model.status:
                    ...
                    logger.info(f"Model {model_name} status changed: {old_status} -> {new_status}")
    ...
    def get_system_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if total_requests > 0:
                total_successful = sum(m.metrics.successful_requests for m in self.models.values())
                stats["average_success_rate"] = (total_successful / total_requests) * 100
            ...
            for model in self.models.values():
                stats["model_breakdown"][model.name] = {
                    ...
                    "success_rate": ((model.metrics.successful_requests / max(1, model.metrics.total_requests)) * 100),
    ...
    def _analyze_task(self, task_description: str) -> TaskAnalysis:
        ...
        if any(word in text for word in ['comprehensive', 'detailed', 'thorough', 'in-depth']):
            base_tokens += 200
    ...
    def _record_model_switch(self, old_model: Optional[str], new_model: str, forced: bool) -> None:
    ...
    def _save_configuration(self) -> None:
        ...
        try:
            ...
            def model_to_dict(model):
                ...
                return {
                    ...
                    "metrics": {
                        ...
                        "average_tokens_per_second": model.metrics.average_tokens_per_second,
                        "total_cost": model.metrics.total_cost,
                        "last_used": model.metrics.last_used.isoformat() if model.metrics.last_used else None,
            ...
            config = {
                "current_model": self.current_model,
                "models": {name: model_to_dict(model) for name, model in self.models.items()},
                "last_updated": datetime.now().isoformat()
            }
        ...
        try:
            ...
            for name, model_data in models_data.items():
                ...
                if "metrics" in model_data:
                    ...
                    if "last_used" in metrics_data and metrics_data["last_used"] is not None:
...
def create_production_model_system(config_path: Optional[str] = None) -> ProductionModelSystem:
