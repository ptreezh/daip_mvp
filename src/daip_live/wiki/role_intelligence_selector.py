...
class RoleIntelligenceSelector:
    ...
    def analyze_topic_for_roles(self, topic: str, max_roles: int = 4) -> List[str]:
        ...
        try:
            ...
            # 基于主题关键词分析选择角色
            selected_roles = self._select_roles_by_topic_analysis(topic, all_roles, max_roles)
            
            # 如果选择的角色数量不足，补充默认角色
            if len(selected_roles) < max_roles:
                for default_role in self.default_roles:
                    if default_role not in selected_roles and len(selected_roles) < max_roles:
    ...
    def _select_roles_by_topic_analysis(self, topic: str, all_roles: List[str], max_roles: int) -> List[str]:
        """
        基于主题分析选择角色
        ...
        # 按得分排序，返回得分最高的角色
        sorted_roles = sorted(role_scores.keys(), key=lambda x: role_scores[x], reverse=True)
    ...
    def _calculate_role_topic_score(self, role_name: str, topic_keywords: List[str]) -> int:
        """
        计算角色与主题的相关性得分
            ...
            if role_file_path.exists():
                with open(role_file_path, 'r', encoding='utf-8') as f:
                    ...
                    if isinstance(role_data, dict):
                        # 尝试获取角色描述字段
                        return role_data.get('persona', role_data.get('description', ''))
        except Exception:
            # 如果无法获取角色描述，返回None
            pass
