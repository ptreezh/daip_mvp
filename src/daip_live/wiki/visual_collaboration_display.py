...
class VisualCollaborationDisplay:
    ...
    def _display_event(self, event: VisualEvent):
        ...
        if event.event_type == "role_contribution":
            color = self.role_colors.get(event.role_name, "white")
            print(f"\n🔄 [{elapsed_time:.1f}s] {event.role_name} 为 '{event.section}' 生成内容:")
            print(f"📝 内容预览: {event.content[:100]}{'...' if len(event.content) > 100 else ''}")
        elif event.event_type == "content_merge":
            print(f"\n🔄 [{elapsed_time:.1f}s] 系统合并内容到 '{event.section}' 部分")
            print(f"📝 合并内容: {event.content[:100]}{'...' if len(event.content) > 100 else ''}")
        elif event.event_type == "analysis":
            print(f"\n🔍 [{elapsed_time:.1f}s] 分析结果: {event.content}")
        elif event.event_type == "progress":
            print(f"\n📊 [{elapsed_time:.1f}s] 进度更新: {event.content}")
        else:
            print(f"\n🔄 [{elapsed_time:.1f}s] {event.event_type.upper()}: {event.content}")
        ...
        for round_num in range(1, total_rounds + 1):
            ...
            for contribution in contributions:
                self.log_event(
                    ...
                    contribution.content[:200] + "..." if len(contribution.content) > 200 else contribution.content,
                    {
    ...
    def get_detailed_log(self) -> str:
        """获取详细的协作日志"""
        log_lines = [
            f"协作会话日志 (共 {len(self.events)} 个事件)",
            "=" * 50
        ]
        ...
        for event in self.events:
            ...
            log_lines.append(f"  内容: {event.content[:100]}{'...' if len(event.content) > 100 else ''}")
            log_lines.append("")
    ...
    def get_collaboration_summary(self) -> Dict[str, Any]:
        ...
        roles_involved = set(event.role_name for event in self.events if event.role_name)
        sections_edited = set(event.section for event in self.events if event.section)
        contribution_events = [e for e in self.events if e.event_type == "role_contribution"]
...
class EnhancedMultiRoleWikiCollaborator:
    ...
    def __init__(self, model_provider=None, visual_display: Optional[VisualCollaborationDisplay] = None):
        from .real_collaboration_engine import MultiRoleWikiCollaborator
        self.base_collaborator = MultiRoleWikiCollaborator(model_provider=model_provider)
    ...
    async def start_collaboration(self, title: str, participants: List[str], initial_content: str = ""):
        ...
        return await self.base_collaborator.start_collaboration(title, participants, initial_content)

    async def generate_content_with_role(self, role_name: str, section: str, current_content: str = ""):
        """使用指定角色生成内容"""
        self.visual_display.log_event(
            "progress",
            role_name,
        ...
        result = await self.base_collaborator.generate_content_with_role(role_name, section, current_content)
    ...
    async def run_collaborative_editing_round(self, sections_to_edit: List[str] = None):
        ...
        self.visual_display.log_event(
            ...
            "system",
            f"开始协作编辑轮次",
            {"sections": sections_to_edit or list(self.base_collaborator.content.keys())}
        )

        contributions = await self.base_collaborator.run_collaborative_editing_round(sections_to_edit)
