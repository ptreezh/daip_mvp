...
class IntelligentAssistant:
    ...
    def __init__(self):
        ...
        # 安全初始化BasicIntentAnalysisService，支持优雅降级
        try:
            # 注意：BasicIntentAnalysisService可能需要一个user_profile_service参数
            # 这里我们尝试不同的初始化方式
            from src.core_services.user_profile_service import UserProfileService
            # 尝试初始化UserProfileService，但可能需要更多参数
            # 我们先用一个简单的空实现
            self.intent_analyzer = BasicIntentAnalysisService(None)
        except Exception as e:
            logger.warning(f"BasicIntentAnalysisService初始化失败，使用空实现: {e}")
            self.intent_analyzer = None
            
        # 安全初始化UniversalContextService，支持优雅降级
        try:
            # 注意：UniversalContextService需要token_service和memory_service参数
            from src.core_services.token_management_service import TokenManagementService
            token_service = TokenManagementService()
            # memory_service should already be initialized or we can create a simple one
            memory_service = self.memory_agent if self.memory_agent else None
            if memory_service is not None:
                self.context_optimizer = UniversalContextService(token_service, memory_service)
            else:
                self.context_optimizer = None
        except Exception as e:
            logger.warning(f"UniversalContextService初始化失败，使用空实现: {e}")
            self.context_optimizer = None
            
        # Note: We'll need to properly instantiate a specific consensus algorithm
        # For now, we'll set this to None and handle it appropriately
        self.consensus_algorithms = None
        
        # 安全初始化SynthesisEngine，支持优雅降级
        try:
            # 注意：SynthesisEngine需要一个llm_interface参数
            # 我们先用一个简单的空实现
            self.synthesis_engine = SynthesisEngine(None)
        except Exception as e:
            logger.warning(f"SynthesisEngine初始化失败，使用空实现: {e}")
    ...
    def _display_system_info(self):
        ...
        for role in available_roles[:5]:  # 显示前5个角色
    ...
    async def _analyze_intent(self, user_input: str) -> dict[str, Any]:
        ...
        try:
            # 使用意图分析服务（如果可用）
            if self.intent_analyzer is not None:
    ...
    async def _call_llm_for_role(self, role, prompt: str) -> str:
        """为特定角色调用LLM"""
        try:
            # 使用IntegratedLLMManager调用真实的LLM服务
    ...
    async def _compute_consensus(self, debate_result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if self.consensus_algorithms is not None:
                # 注意：这里需要适配新的共识算法接口
