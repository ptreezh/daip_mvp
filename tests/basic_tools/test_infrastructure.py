        ...
        return [
            {
                ...
                "summary": "This paper explores various applications of machine learning in real-world scenarios...",  # noqa: E501
            ...
            },
            {
                ...
                "summary": "Recent advances in natural language processing have revolutionized text analysis...",  # noqa: E501
            ...
            },
            {
                ...
                "summary": "This paper presents novel computer vision techniques for object detection...",  # noqa: E501
...
class ToolTestHarness:
    ...
    def assert_tool_error(
        self, execution_result: dict[str, Any], expected_error_type: str = None
    ):
        """断言工具执行失败"""
        assert not execution_result["success"], (
            "Tool execution should have failed but succeeded"
        )
        assert execution_result["error"] is not None

        if expected_error_type:
            assert execution_result["error_type"] == expected_error_type, (
                f"Expected error type {expected_error_type}, got {execution_result['error_type']}"  # noqa: E501
            )

    def measure_performance(
        self, tool_name: str, args: dict[str, Any], iterations: int = 10
    ) -> dict[str, float]:
        """测量工具性能"""
        import time
        ...
        for _ in range(iterations):
            start_time = time.time()
            self.execute_tool_safely(tool_name, args)
            end_time = time.time()
...
def assert_file_content(
    file_path: Path, expected_content: str, encoding: str = "utf-8"
):
    """断言文件内容匹配"""
    assert_file_exists(file_path)
    actual_content = file_path.read_text(encoding=encoding)
    assert actual_content == expected_content, (
        f"File content mismatch. Expected: {expected_content!r}, Got: {actual_content!r}"  # noqa: E501
    )
        ...
        if isinstance(structure, dict):
            for name, content in structure.items():
                ...
                if isinstance(content, dict):
                    ...
                    assert actual_content == content, f"Content mismatch in {item_path}"
        ...
        def wrapper(*args, **kwargs):
            ...
            if max_execution_time:
                assert execution_time <= max_execution_time, (
                    f"Execution time {execution_time:.3f}s exceeds maximum {max_execution_time}s"  # noqa: E501
                )

            if max_memory_mb:
                assert memory_increase <= max_memory_mb, (
                    f"Memory increase {memory_increase:.2f}MB exceeds maximum {max_memory_mb}MB"  # noqa: E501
                )
