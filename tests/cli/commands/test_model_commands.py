...
class TestModelListCommand:
    ...
    def test_model_list_with_refresh_flag(self):
        ...
        with patch("daip_live.cli.commands.model.ModelManager") as mock_manager_class:
            ...
            mock_manager.get_available_models.assert_called_once_with(
                force_refresh=True
            )
    ...
    def test_model_list_empty_models(self):
        ...
        with patch("daip_live.cli.commands.model.ModelManager") as mock_manager_class:
            ...
            assert result.exit_code == 0
            assert (
                "No models found" in result.stdout
                or "models found" in result.stdout.lower()
            )
    ...
    def test_model_list_with_error_handling(self):
        ...
        with patch("daip_live.cli.commands.model.ModelManager") as mock_manager_class:
            mock_manager = Mock()
            mock_manager_class.return_value = mock_manager
            mock_manager.get_available_models.side_effect = Exception(
                "Connection failed"
            )
