...
class TestStartCommand:
    ...
    @patch("src.cli.debate_execution.run_debate_command", new_callable=AsyncMock)
    def test_start_command_basic_success(self, mock_run_debate, mock_asyncio_run, cli_runner):
        ...
        result = cli_runner.invoke(app, ["debate", "start", "Should AI be regulated?"])
    ...
    def test_start_command_with_roles(self, mock_run_debate, mock_asyncio_run, cli_runner):
        ...
        result = cli_runner.invoke(app, [
            "debate", "start", "Climate change solutions",
    ...
    def test_start_command_validation_short_topic(self, cli_runner):
        ...
        result = cli_runner.invoke(app, ["debate", "start", "AI"])
    ...
    def test_start_command_validation_long_topic(self, cli_runner):
        ...
        result = cli_runner.invoke(app, ["debate", "start", long_topic])
    ...
    def test_start_command_validation_invalid_rounds(self, cli_runner):
        ...
        # Test rounds < 1
        result = cli_runner.invoke(app, ["debate", "start", "Test topic", "--rounds", "0"])
        assert result.exit_code == 1
        assert "Number of rounds must be at least 1" in result.stdout

        # Test rounds > 20
        result = cli_runner.invoke(app, ["debate", "start", "Test topic", "--rounds", "25"])
        assert result.exit_code == 1
...
class TestStatusCommand:
    ...
    @patch("src.cli.main.check_system_health")
    ...
    @patch("src.cli.main.check_system_health")
    def test_status_command_with_missing_dependencies(self, mock_check_health, cli_runner):
        """Test status command when dependencies are missing."""
        mock_check_health.return_value = {
            "configuration": {"status": "✅ Loaded", "details": "Config loaded successfully"},
            "dependencies": {"status": "❌ Missing dependencies", "details": "2 packages missing: aiosqlite, chromadb"},
            "services": {"status": "⚠️  Skipped", "details": "Dependencies missing, skipping service init"},
            "api_connectivity": {"status": "⚠️  Skipped", "details": "Core services not initialized, skipping API check"}
        }
...
class TestRolesCommand:
    ...
    @patch("src.cli.commands.role_commands.RoleManager.__init__", return_value=None)
    @patch("src.cli.commands.role_commands.RoleManager.list_roles")
    def test_roles_command_success(self, mock_list_roles, mock_init, cli_runner):
        """Test roles command with available roles."""
        mock_list_roles.return_value = [
            Role(id="1", name="Environmental Scientist", description="Expert in environmental science and climate research", system_prompt="", capabilities=[], tags=["science", "environment", "climate"]),
            Role(id="2", name="Economist", description="Specialist in economic analysis and policy", system_prompt="", capabilities=[], tags=["economics", "policy", "analysis"])
        ]
        ...
        assert "Expert in" in result.stdout
        mock_list_roles.assert_called_once()

    @patch("src.cli.commands.role_commands.RoleManager.__init__", return_value=None)
    @patch("src.cli.commands.role_commands.RoleManager.list_roles")
    def test_roles_command_no_roles(self, mock_list_roles, mock_init, cli_runner):
    ...
    def test_roles_command_import_error(self, mock_role_manager, cli_runner):
        """Test roles command handles import errors."""
        mock_role_manager.side_effect = ImportError("Cannot import role service")
    ...
    @patch("src.cli.commands.role_commands.RoleManager")
    def test_roles_command_general_error(self, mock_role_manager, cli_runner):
        """Test roles command handles general errors."""
        mock_role_manager.side_effect = Exception("Database connection failed")
