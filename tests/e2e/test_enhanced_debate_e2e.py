...
class TestEnhancedDebateEndToEnd:
    ...
    def test_complete_debate_workflow_end_to_end(self):
        ...
        debate_events = [
            # Round 1
            DebateRoundStartEvent(round_number=1, total_rounds=3, session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Pro_Arguer", round_number=1, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Pro_Arguer", round_number=1, content_preview="Pro argument round 1", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Con_Arguer", round_number=1, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Con_Arguer", round_number=1, content_preview="Con argument round 1", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Moderator", round_number=1, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Moderator", round_number=1, content_preview="Moderator summary round 1", session_id="e2e_complete_001"),
            
            # Round 2
            DebateRoundStartEvent(round_number=2, total_rounds=3, session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Con_Arguer", round_number=2, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Con_Arguer", round_number=2, content_preview="Con argument round 2", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Pro_Arguer", round_number=2, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Pro_Arguer", round_number=2, content_preview="Pro argument round 2", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Moderator", round_number=2, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Moderator", round_number=2, content_preview="Moderator summary round 2", session_id="e2e_complete_001"),
            
            # Round 3
            DebateRoundStartEvent(round_number=3, total_rounds=3, session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Moderator", round_number=3, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Moderator", round_number=3, content_preview="Moderator final summary", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Pro_Arguer", round_number=3, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Pro_Arguer", round_number=3, content_preview="Pro final argument", session_id="e2e_complete_001"),
            DebateTurnStartEvent(participant="Con_Arguer", round_number=3, session_id="e2e_complete_001"),
            DebateTurnCompleteEvent(participant="Con_Arguer", round_number=3, content_preview="Con final argument", session_id="e2e_complete_001"),
        ]
        
        # Process all turn complete events through the tracker
        turn_events = [event for event in debate_events if isinstance(event, DebateTurnCompleteEvent)]
        ...
        complete_event = DebateCompleteEvent(
            session_id="e2e_complete_001",
            summary="Complete workflow E2E test debate concluded with thorough discussion"
        )
...
""")
        ...
        try:
            ...
            assert updated_history.turns[0].content == "Pro argument in container system"
            assert updated_history.turns[1].content == "Con argument in container system"
    ...
    def test_cli_tui_integration_e2e(self):
        ...
        round1_events = [
            DebateTurnCompleteEvent(participant="Advocate", round_number=1, content_preview="Advocate's position statement", session_id=session_id),
            DebateTurnCompleteEvent(participant="Skeptic", round_number=1, content_preview="Skeptic's counter-argument", session_id=session_id),
        ]
        ...
        round2_events = [
            DebateTurnCompleteEvent(participant="Skeptic", round_number=2, content_preview="Skeptic's second argument", session_id=session_id),
            DebateTurnCompleteEvent(participant="Advocate", round_number=2, content_preview="Advocate's rebuttal", session_id=session_id),
        ]
    ...
    def test_multi_model_debate_end_to_end(self):
        ...
        # Add turns from different "models" (participants with different characteristics)
        model_turns = [
            # Round 1
            DebateTurnCompleteEvent(participant="GPT_Expert", round_number=1, content_preview="Expert analysis from GPT model", session_id=session_id),
            DebateTurnCompleteEvent(participant="Claude_Analyst", round_number=1, content_preview="Thoughtful analysis from Claude", session_id=session_id),
            DebateTurnCompleteEvent(participant="Gemini_Researcher", round_number=1, content_preview="Research-backed response from Gemini", session_id=session_id),
            
            # Round 2
            DebateTurnCompleteEvent(participant="Claude_Analyst", round_number=2, content_preview="Second analysis from Claude", session_id=session_id),
            DebateTurnCompleteEvent(participant="Gemini_Researcher", round_number=2, content_preview="Updated research from Gemini", session_id=session_id),
            DebateTurnCompleteEvent(participant="GPT_Expert", round_number=2, content_preview="Expert synthesis from GPT", session_id=session_id),
        ]
        ...
        model_participants = set(turn.participant_name for turn in final_history.turns)
        assert model_participants == {"GPT_Expert", "Claude_Analyst", "Gemini_Researcher"}
        
        # Verify all turns have appropriate content
        for turn in final_history.turns:
            assert turn.content.startswith(("Expert", "Thoughtful", "Research", "Second", "Updated", "Expert synthesis"))
    ...
    def test_enhanced_visualization_end_to_end(self):
        ...
        participants = [
            ...
            DebateParticipantView(name="Expert_Analyst", color="#98FB98", symbol="👤", turn_order=2)
        ]
        ...
        enhanced_view.history.extend([
            DebateTurnView(participant_name="Pro_Arguer", content="Pro visualization argument", round_number=1, turn_in_round=1, color="#87CEEB"),
            DebateTurnView(participant_name="Con_Arguer", content="Con visualization argument", round_number=1, turn_in_round=2, color="#FFB6C1"),
            DebateTurnView(participant_name="Expert_Analyst", content="Expert visualization analysis", round_number=1, turn_in_round=3, color="#98FB98"),
            DebateTurnView(participant_name="Pro_Arguer", content="Pro second round argument", round_number=2, turn_in_round=1, color="#87CEEB"),
            DebateTurnView(participant_name="Con_Arguer", content="Con second round argument", round_number=2, turn_in_round=2, color="#FFB6C1"),
        ])
        ...
        for turn in enhanced_view.history:
            assert turn.participant_name in ["Pro_Arguer", "Con_Arguer", "Expert_Analyst"]
    ...
    def test_error_recovery_end_to_end(self):
        ...
        remaining_turns = [
            DebateTurnCompleteEvent(participant="Participant_A", round_number=2, content_preview="Second round argument", session_id=session_id),
            DebateTurnCompleteEvent(participant="Participant_B", round_number=2, content_preview="Second round response", session_id=session_id),
        ]
