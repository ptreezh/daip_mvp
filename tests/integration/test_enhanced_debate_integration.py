...
class TestEnhancedDebateIntegration:
    ...
    def test_history_tracker_concurrent_operations(self):
        ...
        async def simulate_concurrent_operations():
            ...
            for i, updated_history in enumerate(updated_histories):
                assert len(updated_history.turns) == 1
                assert (
                    updated_history.turns[0].content == f"Turn content for debate {i}"
                )
    ...
    def test_large_debate_scenario(self):
        ...
        assert (
            len(final_history.turns) == turn_count
        )  # Should be 50 (5 rounds * 10 roles)
    ...
    def test_debate_completion_integration(self):
        ...
        turns = [
            DebateTurnCompleteEvent(
                participant="Pro",
                round_number=1,
                content_preview="Round 1 Pro",
                session_id="completion_int_006",
            ),
            DebateTurnCompleteEvent(
                participant="Con",
                round_number=1,
                content_preview="Round 1 Con",
                session_id="completion_int_006",
            ),
            DebateTurnCompleteEvent(
                participant="Pro",
                round_number=2,
                content_preview="Round 2 Pro",
                session_id="completion_int_006",
            ),
            DebateTurnCompleteEvent(
                participant="Con",
                round_number=2,
                content_preview="Round 2 Con",
                session_id="completion_int_006",
            ),
        ]
