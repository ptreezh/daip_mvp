...
class TestKnowledgeManager:
    ...
    def test_scan_and_detect_changes(self, mocker, tmp_path: Path):
        ...
        unchanged_source = KnowledgeSource(
            file_path=str(knowledge_dir / "unchanged.txt"),
            file_hash=_get_file_hash(knowledge_dir / "unchanged.txt"),
            status="indexed",
        )
        updated_source_from_db = KnowledgeSource(
            file_path=str(knowledge_dir / "updated.txt"),
            file_hash="old_hash_value",
            status="indexed",
        )
        deleted_source = KnowledgeSource(
            file_path=str(knowledge_dir / "deleted.txt"),
            file_hash="some_hash",
            status="indexed",
        )
        mock_db_manager = mocker.Mock()
        mock_db_manager.get_all_knowledge_sources.return_value = [
            unchanged_source,
            updated_source_from_db,
            deleted_source,
        ]
        manager = KnowledgeManager(
            db_manager=mock_db_manager,
            model_provider=mocker.Mock(),
            config=KnowledgeBaseConfig(directory=str(knowledge_dir)),
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_knowledge_base_handles_added_files(
        ...
        mocker.patch.object(
            manager, "_scan_and_detect_changes", return_value=mock_changes
        )
    ...
    @pytest.mark.asyncio
    async def test_sync_knowledge_base_handles_deleted_files(
        self, mocker, tmp_path: Path
    ):
        deleted_source = KnowledgeSource(
            id=42, file_path="/path/to/deleted.txt", file_hash="abc", status="indexed"
        )
        mock_db_manager = mocker.Mock()
        mocker.patch("daip_live.knowledge.manager.faiss.write_index")
        manager = KnowledgeManager(
            db_manager=mock_db_manager,
            model_provider=mocker.AsyncMock(),
            config=KnowledgeBaseConfig(directory=str(tmp_path)),
        )
        ...
        result = await manager.sync_knowledge_base()
        mock_db_manager.delete_knowledge_source.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    async def test_sync_knowledge_base_handles_updated_files(
        self, mocker, tmp_path: Path
    ):
        ...
        old_source = KnowledgeSource(
            id=101, file_path=str(updated_file), file_hash="old_hash", status="indexed"
        )
        ...
        assert np.array_equal(
            manager.faiss_index.remove_ids.call_args[0][0], np.array([101])
        )
    ...
    async def test_search_e2e(self, mocker, tmp_path: Path):
        ...
        mock_sources = [
            KnowledgeSource(
                id=101, file_path="/path/one.txt", file_hash="abc", status="indexed"
            ),
            KnowledgeSource(
                id=102, file_path="/path/two.txt", file_hash="def", status="indexed"
            ),
        ]
        ...
        assert np.array_equal(
            call_args[0], np.array([[0.1, 0.2, 0.3]], dtype=np.float32)
        )
