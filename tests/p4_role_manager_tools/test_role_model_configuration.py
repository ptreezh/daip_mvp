...
class TestRoleModelConfig:
    ...
    def test_role_model_config_creation(self):
        ...
        assert config.is_primary  # Default value
    ...
    def test_enhanced_role_primary_model_selection(self):
        ...
        configs = [
            ...
            RoleModelConfig(
                model_name=get_safe_test_model(), provider="local", is_primary=True
            ),
            RoleModelConfig(
                model_name="claude-3", provider="anthropic", is_primary=False
            ),
        ]
    ...
    def test_enhanced_role_fallback_to_first_model(self):
        ...
        configs = [
            RoleModelConfig(model_name="mock-llm", provider="local", is_primary=False),
            RoleModelConfig(
                model_name=get_safe_test_model(), provider="local", is_primary=False
            ),
        ]
    ...
    def test_role_model_mapping_creation(self):
        ...
        role = EnhancedRole(name="test_role", persona="Test persona", tools=["search"])

        mapping = RoleModelMapping.from_role(role)
        assert mapping.role_name == "test_role"
        assert (
            mapping.role_model_config.model_name == "gpt-3.5-turbo"
        )  # Default from EnhancedRole
...
"""
    ...
    async def test_model_provider_caching(self):
        ...
        self.role_model_manager.get_role_by_name("test_role")
        mapping = self.role_model_manager.get_role_model_mapping("test_role")

        provider1 = self.debate_manager._get_model_provider_for_config(
            mapping.role_model_config
        )
        provider2 = self.debate_manager._get_model_provider_for_config(
            mapping.role_model_config
        )
