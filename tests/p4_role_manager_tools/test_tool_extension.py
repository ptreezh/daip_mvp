...
class TestToolExtension:
    ...
    def test_tool_extension_interface_compliance(self):
        ...
        # 验证读工具符合规范
        session_context = SessionContext()
        read_result = manager.execute_tool("read_file_custom", {"file_path": "test.txt"}, session_context)
        assert "Custom read content from test.txt" in read_result
        
        # 验证写工具符合规范
        # 先添加到已读资源中以满足写后读约束
        session_context.recently_read_resources.add("test.txt")
        write_result = manager.execute_tool("write_file_custom", {"file_path": "test.txt", "content": "hello"}, session_context)
        ...
        assert read_file_custom.is_write == False
        ...
        assert write_file_custom.is_write == True