...
from daip_live.permission.permission_manager import PermissionManager, SimplePermissionManager
from daip_live.permission.user_response_collector import UserResponseCollector, ResponseProcessor
...
class TestPermissionInteractionRefactor:
    ...
    def test_permission_interaction_simplified_state_management(self, mock_dependencies):
        """测试简化后的权限交互状态管理"""
    ...
    @pytest.mark.asyncio
    async def test_permission_interaction_user_response_handling(self, mock_dependencies):
        """测试权限交互用户响应处理"""
        permission_manager = PermissionManager(**mock_dependencies)
        ...
        # 模拟用户响应收集器
        with patch('daip_live.permission.user_response_collector.UserResponseCollector.collect_response', 
    ...
    def test_user_response_collector_simplified_design(self, mock_dependencies):
        ...
        # 验证只支持基本响应类型
        expected_responses = {'y', 'n', 'a', 'v', 'c', 'yes', 'no', 'always', 'never', 'cancel'}
