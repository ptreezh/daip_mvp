...
class TestPermissionInteractionRobustness:
    ...
    def test_permission_interaction_expiration_detection(self):
        ...
        # Then: 验证过期
        assert interaction.is_expired() == True
        ...
        # When: 检查过期（指定时间）
        # Then: 应该过期
        assert interaction.is_expired(current_time) == True
    ...
    def test_permission_interaction_stale_detection(self):
        ...
        # When: 检查是否陈旧（默认5分钟阈值）
        # Then: 应该不陈旧
        assert interaction.is_stale() == False
        
        # When: 检查指定阈值
        # Then: 应该不陈旧（时间太短）
        assert interaction.is_stale(stale_threshold=0.001) == False
    ...
    def test_permission_interaction_retry_logic(self):
        ...
        # Then: 初始状态可以重试
        assert interaction.can_retry() == True
        ...
        # Then: 不可以再重试
        assert interaction.can_retry() == False
    ...
    def test_permission_interaction_duplicate_handling(self):
        ...
        # Then: 验证重复状态
        assert interaction.is_duplicate == True
    ...
    def test_permission_interaction_invalid_state_transition(self):
        ...
        with pytest.raises(ValueError, match="Cannot update response for non-pending interaction"):
            interaction.update_response(PermissionResponse.DENY)
        
        # When/Then: 验证无效记住操作
        with pytest.raises(ValueError, match="Can only mark as remembered if response is ALWAYS or NEVER"):
    ...
    def test_permission_interaction_circuit_breaker(self):
        ...
        # Then: 验证断路器状态
        assert interaction.circuit_breaker_open == True
...
class TestPermissionResult:
    ...
    def test_permission_result_creation(self):
        ...
        assert result.remembered == False
    ...
    def test_permission_result_error_cases(self):
        ...
        error_cases = [
            ...
            {"needs_manual_review": True, "reason": "system_error", "error_message": "Unexpected error"},
        ]
...
class TestPermissionInteractionRobustness:
    ...
    def test_concurrent_state_modification_safety(self):
        ...
        # Then: 状态应该一致（注意：Pydantic会将枚举转换为字符串）
        assert interaction.state == PermissionState.GRANTED.value
        assert interaction.response == PermissionResponse.GRANT.value
    ...
    def test_resource_cleanup_markers(self):
        ...
        assert interaction.is_duplicate == True
        assert interaction.circuit_breaker_open == True
        assert interaction.is_stale() == False  # 仍然新鲜
