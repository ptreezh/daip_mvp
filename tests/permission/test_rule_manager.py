...
class TestPermissionRuleManagerInterface(unittest.TestCase):
    ...
    def test_interface_methods_exist(self):
        ...
        self.assertTrue(
            hasattr(PermissionRuleManagerInterface, "reset_tool_permission")
        )
        self.assertTrue(
            hasattr(PermissionRuleManagerInterface, "set_default_permission")
        )
        self.assertTrue(
            hasattr(PermissionRuleManagerInterface, "list_permission_rules")
        )
...
class TestPermissionRuleManager(unittest.TestCase):
    ...
    def test_reset_tool_permission(self):
        ...
        self.assertEqual(
            self.permission_manager.get_tool_permission("test_tool"), "deny"
        )

        # 重置工具权限后应返回默认值
        self.permission_manager.reset_tool_permission("test_tool")
        self.assertEqual(
            self.permission_manager.get_tool_permission("test_tool"), "ask"
        )
...
class TestPermissionRuleManagerWithExistingConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        test_config = {
            "permission_rules": {
                "default": "deny",
                "tools": {"read_file": "allow", "write_file": "ask"},
            }
        }
...
def run_tests():
    ...
    # 添加所有测试类
    test_suite.addTests(
        test_loader.loadTestsFromTestCase(TestPermissionRuleManagerInterface)
    )
    test_suite.addTests(test_loader.loadTestsFromTestCase(TestPermissionRuleManager))
    test_suite.addTests(
        test_loader.loadTestsFromTestCase(TestPermissionRuleManagerFactory)
    )
    test_suite.addTests(
        test_loader.loadTestsFromTestCase(TestPermissionRuleManagerWithExistingConfig)
    )
