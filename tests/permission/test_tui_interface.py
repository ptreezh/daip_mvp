...
class TestPermissionTUIInterface:
    ...
    def test_user_input_parsing_valid(self):
        """验证有效用户输入解析 - 绿"""
        from daip_live.permission.tui_interface import PermissionTUIInterface, PermissionResponse
...
class TestPermissionUIConfiguration:
    ...
    def test_permission_ui_display_defaults(self):
        ...
        assert display.show_risk_level == True
        assert display.show_arguments == True
        assert display.show_description == True
        assert display.show_countdown == True
        assert display.show_confirmation == True
