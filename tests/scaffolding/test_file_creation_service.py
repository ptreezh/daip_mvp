...
class TestFileOperationStatus:
    ...
    def test_file_operation_status_from_string(self):
        ...
        assert (
            FileOperationStatus.from_string("unknown") == FileOperationStatus.SUCCESS
        )  # 默认值
...
class TestFileConflictResolution:
    ...
    def test_conflict_resolution_from_string(self):
        ...
        assert (
            FileConflictResolution.from_string("overwrite")
            == FileConflictResolution.OVERWRITE
        )
        assert (
            FileConflictResolution.from_string("unknown") == FileConflictResolution.FAIL
        )  # 默认值
...
class TestValidationRule:
    ...
    def test_validation_rule_with_condition(self):
        ...
        assert rule.should_validate(py_file)

        # 测试条件不匹配
        txt_file = ProjectFile(path="test.txt", content="")
        assert not rule.should_validate(txt_file)

    def test_validation_rule_validation(self):
        """测试验证规则验证"""
        # TC-2.4.7: 规则验证测试
        # 有效规则
        valid_rule = ValidationRule(name="test_rule", validator=lambda file: True)
        errors = valid_rule.validate()
        assert len(errors) == 0

        # 无效规则 - 无名称
        invalid_rule = ValidationRule(name="", validator=lambda file: True)
...
class TestFileCreationConfig:
    ...
    def test_file_creation_config_with_rules(self):
        ...
        # TC-2.4.9: 验证规则配置测试
        rule = ValidationRule(
            name="max_size", validator=lambda file: len(file.content) <= 100
        )

        config = FileCreationConfig(validation_rules=[rule], validation_enabled=True)
    ...
    def test_file_creation_config_defaults(self):
        ...
        assert config.create_directories
        assert not config.preserve_permissions
        assert config.validation_enabled
...
class TestDirectoryStructure:
    ...
    def test_directory_structure_creation(self):
        """测试目录结构创建"""
        # TC-2.4.11: 目录结构创建测试
        structure = DirectoryStructure(base_path="/test/project", mode=0o755)
...
class TestFileCreationResult:
    ...
    def test_file_creation_result_failure(self):
        ...
        result = FileCreationResult(
            file=file, status=FileOperationStatus.FAILED, error="Permission denied"
        )
...
class TestFileCreationService:
    ...
    def test_service_with_custom_config(self):
        ...
        assert service.config.conflict_resolution == FileConflictResolution.OVERWRITE
        assert not service.config.create_directories
    ...
    async def test_create_file_backup_existing(self):
        ...
        backup_service = FileCreationService(
            config=FileCreationConfig(conflict_resolution=FileConflictResolution.BACKUP)
        )
        ...
        # 验证备份文件内容
        with open(result.existing_path) as f:
    ...
    async def test_create_project_structure_with_validation(self):
        ...
        py_rule = ValidationRule(
            name="python_syntax",
            validator=lambda file: not file.path.endswith(".py")
            or "pass" in file.content,
            condition=lambda file: file.path.endswith(".py"),
        )

        validation_service = FileCreationService(
            config=FileCreationConfig(
                validation_rules=[py_rule], validation_enabled=True
            )
        )
        ...
        results = await validation_service.create_project_structure(
            structure, self.temp_dir
        )

        assert len(results) == 2
        assert results[0].success  # main.py通过验证
        assert not results[1].success  # invalid.py验证失败
    ...
    async def test_create_project_structure_partial_failure(self):
        ...
        allowed_dirs_service = FileCreationService(
            config=FileCreationConfig(allowed_directories=["src", "tests"])
        )
        ...
        results = await allowed_dirs_service.create_project_structure(
            structure, self.temp_dir
        )

        assert len(results) == 3
        assert results[0].success  # src/main.py
        assert results[1].success  # tests/test.py
        assert not results[2].success  # temp/cache.py
    ...
    async def test_dry_run_mode(self):
        """测试干运行模式"""
        # TC-2.4.31: 干运行模式测试
        dry_run_service = FileCreationService(config=FileCreationConfig(dry_run=True))

        file = ProjectFile(
            path=os.path.join(self.temp_dir, "dry_run.txt"), content="test content"
        )
    ...
    async def test_file_permission_handling(self):
        ...
        permission_service = FileCreationService(
            config=FileCreationConfig(
                preserve_permissions=True, default_file_mode=0o644
            )
        )

        file = ProjectFile(
            path=os.path.join(self.temp_dir, "chmod_test.py"), content="print('test')"
        )
    ...
    async def test_error_handling_missing_directory(self):
        ...
        file = ProjectFile(
            path=os.path.join(self.temp_dir, "nonexistent", "file.py"), content="test"
        )
