...
from daip_live.scaffolding.error_handler import ErrorHandler, ErrorSeverity
...
class TestGenerationRequest:
    ...
    def test_generation_request_creation(self):
        ...
        assert request.auto_confirm == False
    ...
    def test_generation_request_with_options(self):
        ...
        assert request.options["include_tests"] == True
    ...
    def test_generation_request_from_command(self):
        ...
        command = ScaffoldCommand(
            input_type=InputType.TEXT,
            description="创建一个API服务",
            auto_confirm=True
        )

        request = GenerationRequest.from_command(
            command,
            output_directory="./api_project"
        )

        assert request.description == "创建一个API服务"
        assert request.input_type == InputType.TEXT
        assert request.auto_confirm == True
...
class TestGenerationContext:
    ...
    def test_generation_context_warning_handling(self):
        ...
        assert context.has_warnings() == True
...
class TestGenerationResult:
    ...
    def test_generation_result_success(self):
        ...
        project_structure = ProjectStructure(
            files=[ProjectFile("test.py", "content")],
            description="测试项目"
        )
    ...
    def test_generation_result_to_scaffold_result(self):
        ...
        assert scaffold_result.is_success == True
...
class TestScaffoldEngine:
    ...
    async def test_generate_project_success(self):
        ...
        # TC-2.1.20: 成功生成测试
        request = GenerationRequest(
            description="创建一个简单的Python应用",
            output_directory="./test_output"
        )

        # Mock dependencies
        with patch.object(self.engine, '_analyze_content') as mock_analyze, \
    ...
    async def test_generate_project_with_validation_error(self):
        """测试生成项目时的验证错误"""
        # TC-2.1.21: 验证错误测试
        request = GenerationRequest(description="", input_type=InputType.TEXT)  # 空描述，无文件路径

        result = await self.engine.generate(request)

        assert result.success == False
        assert len(result.errors) > 0
        assert any("必须提供项目描述或描述文件" in str(error) for error in result.errors)
    ...
    async def test_engine_cancellation(self):
        ...
        result = GenerationResult.failure([asyncio.CancelledError("Test cancellation")], 0.1)
    ...
    async def test_engine_with_retry_mechanism(self):
        ...
        with patch.object(self.engine, '_generate_structure', side_effect=mock_generate_with_failure):
            result = await self.engine.generate(request)
    ...
    async def test_engine_error_recovery(self):
        ...
        # 设置可恢复的错误策略
        self.engine.configure_recovery_strategy(
            "ConnectionError",
            max_retries=2
        )
    ...
    async def test_engine_performance_monitoring(self):
        ...
        # Mock快速操作
        with patch.object(self.engine, '_analyze_content'), \
