...
class TestCommandCleanup:
    ...
    def test_help_output_no_redundant_commands(self):
        ...
        # Check that redundant shortcut commands are not in help output as standalone commands  # noqa: E501
        # Use word boundaries to avoid matching within other words (e.g., "pa" in "past")  # noqa: E501
        import re

        # Look for commands in the commands section (between "Commands" and the end)
        commands_section_match = re.search(
            r"Commands.*?$(.*)", result.output, re.MULTILINE | re.DOTALL
        )
        if commands_section_match:
            ...
            for command in redundant_commands:
                # Look for command as a word boundary followed by spaces (typical command format)  # noqa: E501
                pattern = r"\b" + re.escape(command) + r"\s+"
                assert not re.search(pattern, commands_section), (
                    f"Command '{command}' should not appear as standalone command in help output"  # noqa: E501
                )
        ...
        for command in essential_commands:
            assert command in result.output, (
                f"Essential command '{command}' should be present in help output"
            )
