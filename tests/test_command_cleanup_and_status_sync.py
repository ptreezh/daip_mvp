...
class TestCommandCleanup:
    ...
    def test_autocomplete_respects_white_list(self, mock_tui):
        ...
        for suggestion in suggestions:
            ...
            assert command in white_list_commands, (
                f"非白名单命令出现在自动完成中: {command}"
            )
...
class TestStatusBarSync:
    ...
    def test_model_metrics_tracking(self, mock_tui_with_status):
        ...
        # 模拟模型指标事件
        metrics_event = ModelMetricsEvent(latency=1.5, request_count=5)
...
class TestModelSpecificTokenLimits:
    ...
    def mock_model_manager(self):
        ...
        manager = Mock()
        manager.get_model_config = Mock(
            return_value={"max_tokens": 4096, "context_window": 8192}
        )
        return manager
    ...
    def test_dynamic_token_limit_adjustment(
        self, mock_tui_with_status, mock_model_manager
    ):
