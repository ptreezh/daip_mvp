...
from daip_live.core.models import ModelMetricsEvent, TokenUsageEvent
...
class TestCommandCleanup:
    ...
    def test_redundant_cli_commands_should_be_removed(self):
        ...
        for cmd in redundant_commands:
            result = subprocess.run(
                ["python", "-m", "daip", cmd, "test"], capture_output=True, text=True
            )
    ...
    def test_standard_commands_remain_functional(self):
        ...
        for cmd in standard_commands:
            result = subprocess.run(
                ["python", "-m", "daip", cmd, "--help"], capture_output=True, text=True
            )
...
class TestModelSwitchingStatusSync:
    ...
    def test_token_limits_updated_on_model_switch(self):
        ...
        for model, expected_limit in test_cases:
            ...
            assert current_total == expected_limit, (
                f"模型 {model} 的Token限制应为 {expected_limit}"
            )
...
class TestCommandRegistry:
    ...
    def test_command_registry_completeness(self):
        ...
        # 红阶段：当前可能存在缺失，测试将失败
        actual_commands = set()
        for category in COMMAND_REGISTRY.values():  # noqa: F821
...
class TestIntegrationScenarios:
    ...
    def test_concurrent_status_updates(self):
        ...
        # When: 模拟并发状态更新事件
        events = [
            ModelSwitchedEvent("llama3", "llama3:8b"),  # noqa: F821
            TokenUsageEvent(
