...
async def test_command_discovery(tui_app):
    ...
    tui = tui_app["app"]
    expected_commands = ['/pa', '/role', '/knowledge', '/session', '/help', '/quit', '/debate', '/model', '/init']
...
async def test_session_commands(tui_app):
    ...
    test_session = session_manager.create_session(goal="test session", session_type="chat", participant_ids=["user", "agent"])
...
async def test_help_dialog_content(tui_app):
    ...
    async with tui.run_test() as pilot:
        ...
        all_text = "\n".join(line.plain if hasattr(line, 'plain') else str(line) for line in rich_log.lines)
