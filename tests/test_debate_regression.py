...
class TestDebateRegression:
    ...
    def mock_dependencies(self):
        ...
        role_config = RoleModelConfig(model_name="test_model", provider="test_provider")
        mapping = Mock()
    ...
    def debate_manager(self, mock_dependencies):
        ...
        async def patched_generate_response(*args, **kwargs):
            response_content, token_info = await original_generate_response(
                *args, **kwargs
            )
        ...
        async def patched_generate_summary(*args, **kwargs):
            response_content, token_info = await original_generate_summary(
                *args, **kwargs
            )
            # 返回内容但不返回token_info
            return response_content, None
    ...
    def test_debate_turn_events(self, debate_manager):
        ...
        turn_complete_events = [
            e for e in events if isinstance(e, DebateTurnCompleteEvent)
        ]
    ...
    def test_role_retrieval(self, debate_manager, mock_dependencies):
        ...
        assert (
            mock_dependencies["role_manager"].get_role_by_name.call_count
            == expected_calls
        )
    ...
    def test_model_provider_calls(self, debate_manager, mock_dependencies):
        ...
        assert (
            mock_dependencies["model_provider"].generate_calls["count"]
            == expected_calls
        )
    ...
    def test_multiple_roles(self, mock_dependencies):
        ...
        mapping1.role_model_config = RoleModelConfig(
            model_name="model1", provider="provider1"
        )
        ...
        mapping2.role_model_config = RoleModelConfig(
            model_name="model2", provider="provider2"
        )
        ...
        mock_dependencies[
            "role_model_manager"
        ].get_debate_model_mappings.return_value = [mapping1, mapping2]
        ...
        mock_dependencies[
            "role_manager"
        ].get_role_by_name.side_effect = get_role_by_name
    ...
    def test_empty_roles_handling(self, mock_dependencies):
        ...
        mock_dependencies[
            "role_model_manager"
        ].get_debate_model_mappings.return_value = []

        # 源码权威: 默认 use_optimized_architecture=True 走优化路径（enhanced_debate_manager.py:72），  # noqa: E501
        # 该 ValueError 只在 legacy 路径抛出（L225），须显式指定 legacy 模式
        debate_manager = EnhancedDebateManager(
            **mock_dependencies, use_optimized_architecture=False
        )
        ...
        # 应该抛出ValueError
        with pytest.raises(
            ValueError, match="One or more specified roles could not be loaded"
        ):
            events = []
...
class TestDebateCompatibility:
    ...
    def test_event_structure_compatibility(self):
        ...
        start_event = DebateStartEvent(
            topic="Test", roles=["role1"], rounds=2, session_id="test_session"
        )
        ...
        turn_event = DebateTurnStartEvent(
            session_id="test_session", round_number=1, participant="role1"
        )
