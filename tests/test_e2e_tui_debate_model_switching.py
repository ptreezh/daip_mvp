...
def container():
    ...
    container.role_model_manager.override(providers.Singleton(RoleModelManager, roles_dir_path="roles"))
    container.knowledge_manager.override(providers.Singleton(KnowledgeManager, db_manager=container.db_manager, model_provider=container.embed_provider, config=container.knowledge_base_config))
    container.model_provider.override(providers.Singleton(ModelProvider, config=container.config.llm_provider))

    # Create enhanced debate manager after role_model_manager override to ensure it gets the right instance
    role_model_manager_instance = container.role_model_manager()
    container.enhanced_debate_manager.override(providers.Singleton(
...
async def test_debate_model_switching_e2e(container: Container, capsys):
    ...
    async with app.run_test() as pilot:
        ...
        input_widget.value = '/debate start "AI Ethics" --roles "tech_analyst,pro_arguer" --rounds 1'
