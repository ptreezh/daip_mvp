...
class TestEndToEndWikiCollaboration:
    ...
    def setup_method(self):
        ...
        async def mock_generate(prompt, **kwargs):
            ...
            if "领域专家" in prompt:
                return "领域专家贡献：核心知识点和关键技术", {
                    "usage": {"prompt_tokens": 10, "completion_tokens": 20}
                }
            elif "研究员" in prompt:
                return "研究员贡献：研究依据和数据支撑", {
                    "usage": {"prompt_tokens": 10, "completion_tokens": 20}
                }
            elif "编辑" in prompt:
                return "编辑贡献：结构和表述优化", {
    ...
    async def test_full_collaboration_workflow_with_mock(self):
        ...
        assert set(collaborator.default_roles) >= set(
            expected_roles[:2]
        )  # 至少包含前两个角色
    ...
    async def test_basic_wiki_page_creation(self):
        """测试基本Wiki页面创建功能（备选方案）"""
        # 验证即使没有协作器也能够创建基本页面
        basic_manager = EnhancedWikiManager(wiki_root=Path("./test_basic_wiki"))
