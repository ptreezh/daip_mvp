...
from src.daip_live.debate_module.core import DebateConfig, DebateCore
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with tempfile.TemporaryDirectory() as db_dir:
                ...
                knowledge_config = KnowledgeBaseConfig(
                    directory=str(temp_path / "knowledge"), embedding_dimension=384
                )

                KnowledgeManager(db_manager, mock_model_provider, knowledge_config)
                db_manager.engine.dispose()  # 释放文件锁，避免 TemporaryDirectory 清理失败（WinError 32/267）  # noqa: E501
    ...
    async def test_e2e_debate_to_wiki_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            debate_config = DebateConfig(
                topic="AI对社会的影响", roles=["支持者", "反对者"], rounds=2
            )
            ...
            assert (
                len(
                    [e for e in debate_result.turns if e.get("type") == "turn_complete"]
                )
                == 4
            )  # 2轮 * 2角色
    ...
    async def test_e2e_skill_to_knowledge_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with tempfile.TemporaryDirectory() as db_dir:
                ...
                knowledge_config = KnowledgeBaseConfig(
                    directory=str(knowledge_dir), embedding_dimension=384
                )

                knowledge_manager = KnowledgeManager(
                    db_manager, mock_model_provider, knowledge_config
                )
...
近年来，大语言模型在NLP任务中取得了显著进展。
                ...
                # 3. 测试搜索功能
                await knowledge_manager.search("人工智能", top_k=5)
    ...
    async def test_e2e_intent_driven_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with tempfile.TemporaryDirectory() as db_dir:
                ...
                # 2. 识别意图
                intent_recognizer.recognize_intent(user_input)
                ...
                "tools": [
                    {
                        ...
                        "parameters": {
                            "type": "object",
                            "properties": {
                                "query": {"type": "string", "description": "分析查询"}
                            },
                            "required": ["query"],
                        },
                    }
    ...
    async def test_e2e_complex_multi_component_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            wiki_manager = WikiManager(temp_path / "wiki")
            skill_manager = SkillManager()
            ClaudeSkillAdapterManager(skill_manager)
            ...
            with tempfile.TemporaryDirectory() as db_dir:
                ...
                for event in debate_result.turns:
                    if event.get("type") == "turn_complete":
                        debate_content += (
                            f"### {event['role']}观点 (第{event['round']}轮)\n"
                        )
                        debate_content += f"{event['content']}\n\n"
                ...
                # 5. 搜索知识
                await knowledge_manager.search("远程工作", top_k=5)

                # 6. 识别新意图
                followup_intent = intent_recognizer.recognize_intent(
                    "分析远程工作的技术影响"
                )
...
class TestIntegrationEdgeCases:
    ...
    async def test_concurrent_access_to_shared_resources(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建多个任务
            tasks = [create_page_task(f"页面{i}", f"内容{i}") for i in range(5)]
    ...
    def test_error_propagation_between_components(self):
        ...
        try:
            # 正常初始化应该成功
            WikiManager(Path("./valid_temp_dir"))
            success = True
        except Exception:
            # 路径不存在时可能会创建
            import tempfile
    ...
    async def test_long_running_conversation_context(self):
        ...
        for i, user_input in enumerate(conversation_steps):
            result = intent_recognizer.recognize_intent(
                user_input, session_id="long_convo"
            )
