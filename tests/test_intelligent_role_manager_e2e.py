...
from src.daip_live.p4_role_manager_tools.intelligent_role_manager import (
...
class TestIntelligentRoleManagerE2E:
    ...
    def setup_method(self):
        ...
        self.role_manager = IntelligentRoleManager(
            roles_dir=self.roles_path, model_provider=self.model_provider
        )
    ...
    def test_end_to_end_role_creation_and_persistence(self):
        ...
        created_role = self.role_manager.create_role_from_topic(
            topic, role_position="supporting"
        )
        ...
        assert (
            "healthcare" in created_role.persona.lower()
        )  # Check persona contains topic
    ...
    def test_create_and_save_role_for_topic(self):
        ...
        assert (
            "autonomous" in created_role.name.lower()
            or "challenger" in created_role.name.lower()
        )
    ...
    def test_topic_analysis_and_suggestion(self):
        ...
        assert (
            "environment" in analysis["domains"] or "technology" in analysis["domains"]
        )
        ...
        suggested_roles = self.role_manager.suggest_roles_for_topic(
            topic, test_roles, num_suggestions=2
        )
