...
class TestKnowledgeSource:
    ...
    def test_knowledge_source_creation(self):
        ...
        source = KnowledgeSource(
            file_path="test.txt", file_hash="abc123", status="indexed"
        )
    ...
    def test_knowledge_source_with_id(self):
        """测试带ID的知识源"""
        source = KnowledgeSource(
            file_path="test_with_id.txt", file_hash="def456", status="pending", id=1
        )
...
class TestKnowledgeBaseChanges:
    ...
    def test_knowledge_base_changes_with_data(self):
        ...
        source1 = KnowledgeSource(
            file_path="file1.txt", file_hash="hash1", status="indexed"
        )
        source2 = KnowledgeSource(
            file_path="file2.txt", file_hash="hash2", status="indexed"
        )
...
class TestKnowledgeBaseConfig:
    ...
    def test_knowledge_base_config_creation(self):
        ...
        config = KnowledgeBaseConfig(
            directory="test_knowledge_dir", embedding_dimension=384
        )
...
class TestKnowledgeManager:
    ...
    def test_knowledge_manager_initialization(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试知识库管理器初始化"""
        config = KnowledgeBaseConfig(
            directory=str(temp_knowledge_dir), embedding_dimension=384
        )
    ...
    def test_knowledge_manager_initialization_with_existing_index(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试初始化带已有索引的知识库管理器"""
        # 创建一个FAISS索引文件
    ...
    def test_scan_and_detect_changes_no_changes(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试扫描未变化的文件"""
        # 创建一个文件
        ...
        existing_source = KnowledgeSource(
            file_path=str(test_file), file_hash=file_hash, status="indexed", id=1
        )
    ...
    def test_scan_and_detect_changes_added_file(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试扫描新增文件"""
        # 创建一个新文件
    ...
    def test_scan_and_detect_changes_updated_file(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试扫描更新文件"""
        # 创建一个文件
        ...
        old_source = KnowledgeSource(
            file_path=str(test_file), file_hash=old_hash, status="indexed", id=1
        )
    ...
    def test_scan_and_detect_changes_deleted_file(
        self, temp_knowledge_dir, mock_db_manager, mock_model_provider
    ):
        """测试扫描删除文件"""
        # 创建一个文件
    ...
    @patch(
        "src.daip_live.persistence.database.DatabaseManager.get_all_knowledge_sources"
    )
    async def test_sync_knowledge_base_add_file(
        self, mock_get_all, mock_upsert, temp_knowledge_dir, mock_model_provider
    ):
        """测试同步知识库-添加文件"""
        # 创建一个新文件
        ...
        mock_new_source = KnowledgeSource(
            file_path=str(new_file), file_hash="hash", status="indexed", id=100
        )
    ...
    async def test_sync_knowledge_base_delete_file(
        self, mock_get_all, mock_delete, temp_knowledge_dir, mock_model_provider
    ):
        """测试同步知识库-删除文件"""
        # 创建一个文件
        ...
        # 设置数据库返回该文件
        existing_source = KnowledgeSource(
            file_path=str(test_file), file_hash=file_hash, status="indexed", id=200
        )
    ...
    async def test_search_functionality(
        ...
        mock_source = KnowledgeSource(
            file_path="search_test.txt", file_hash="hash", status="indexed", id=300
        )
...
class TestKnowledgeIntegration:
    ...
    async def test_complete_knowledge_workflow(self, temp_knowledge_dir):
        ...
        with tempfile.TemporaryDirectory() as db_dir:
            ...
            # 5. 搜索
            await manager.search("知识", top_k=10)
...
async def run_comprehensive_tests():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager_test.test_knowledge_manager_initialization(
            temp_path, mock_db_manager, mock_model_provider
        )
