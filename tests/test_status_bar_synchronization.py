...
class TestStatusBarSynchronization:
    ...
    @pytest.mark.parametrize("token_usage,expected_color", [
        (2048, "green"),      # 25% - green
        (4096, "yellow"),     # 50% - yellow  
        (6144, "orange"),     # 75% - orange
        (7680, "red"),        # 94% - red
    ])
    def test_status_bar_color_changes_with_token_usage(self, token_usage, expected_color):
        """
...
class TestStatusBarIntegration:
    ...
    async def test_model_switch_triggers_all_updates(self):
        ...
        with patch.object(tui_app, 'model_provider') as mock_provider, \
