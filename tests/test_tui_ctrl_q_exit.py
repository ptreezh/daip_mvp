...
class TestTUIExitFunctionality(unittest.TestCase):
    ...
    def test_ctrl_q_binding_exists(self):
        ...
        try:
            ...
            self.assertEqual(
                ctrl_q_binding.action,
                "_handle_ctrl_q_exit",
                "Ctrl+Q应该绑定到_handle_ctrl_q_exit",
            )
    ...
    def test_ctrl_q_attributes_initialization(self):
        ...
        try:
            ...
            with (
                ...
                # 检查属性是否存在并正确初始化
                self.assertTrue(
                    hasattr(tui, "_last_ctrl_q_time"), "应该有_last_ctrl_q_time属性"
                )
                self.assertTrue(
                    hasattr(tui, "_ctrl_q_press_count"), "应该有_ctrl_q_press_count属性"
                )
                self.assertEqual(
                    tui._last_ctrl_q_time, 0, "_last_ctrl_q_time应该初始化为0"
                )
                self.assertEqual(
                    tui._ctrl_q_press_count, 0, "_ctrl_q_press_count应该初始化为0"
                )
        except ImportError as e:
            self.fail(f"DAIP_TUI类导入失败: {e}")
        ...
        try:
            ...
            with (
                ...
                tui._update_log_view.assert_called_with(
                    "[yellow]会话已终止。再次按 CTRL+Q 退出应用。[/yellow]"
                )
                tui._update_status_bar.assert_called_with(
                    "会话已终止 - 再次按 CTRL+Q 退出应用"
                )
