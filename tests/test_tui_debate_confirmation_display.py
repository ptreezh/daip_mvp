    ...
    def test_debate_confirmation_message_display(self):
        ...
        try:
            ...
            with patch('daip_live.memory.session_manager.SessionManager', return_value=self.mock_session_manager), \
                ...
                # 直接调用显示确认信息的代码
                tui._update_log_view(f"[bold blue]> Starting debate on topic: {topic}[/bold blue]")
                ...
                topic_displayed = any("Starting debate on topic" in str(call) for call in calls)
                roles_displayed = any("Roles:" in str(call) and "pro_arguer" in str(call) for call in calls)
    ...
    def test_debate_start_event_display(self):
        ...
        try:
            ...
            with patch('daip_live.memory.session_manager.SessionManager', return_value=self.mock_session_manager), \
                ...
                # 检查是否显示了辩论开始信息
                calls = tui._update_log_view.call_args_list
                debate_started_displayed = any("Debate started" in str(call) for call in calls)
                participants_displayed = any("Participants:" in str(call) for call in calls)
