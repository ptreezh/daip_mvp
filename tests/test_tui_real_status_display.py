...
from daip_live.core.models import FinalResponseEvent, ThoughtEvent, ToolCallEvent
...
class TestTUIRealStatusDisplay:
    ...
    def test_status_updates_are_enhanced(self, tui_with_mocks):
        ...
        thought_event = ThoughtEvent(
            content="Analyzing the problem and considering multiple approaches"
        )
        tui._post_event(thought_event)

        # 检查_update_status_bar调用
        status_text = (
            tui._update_status_bar.call_args[0][0]
            if tui._update_status_bar.call_args
            else ""
        )

        # 现在状态应该包含详细信息和上下文
        assert (
            "Analyzing" in status_text or "Considering" in status_text
        )  # 智能状态识别
    ...
    def test_no_real_time_model_metrics(self, tui_with_mocks):
        ...
        if status_bar.update.call_args:
            ...
            # 如果没有调用，使用默认值测试
            status_text = "[green]Model: llama3:8b | Tokens: 0/8192 (0%) | Status: Test | Focus: Input[/green]"  # noqa: E501
