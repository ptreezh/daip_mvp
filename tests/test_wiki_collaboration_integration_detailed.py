...
class TestWikiCollaborationIntegration:
    ...
    def test_intent_recognizer_identifies_create_wiki(self):
        ...
        for test_input in test_inputs:
            ...
            assert result.name == "create_wiki", f"对 {test_input} 识别为 {result.name}，期望 create_wiki"
            assert "人工智能" in test_input or "机器学习" in test_input or "深度学习" in test_input or "量子计算" in test_input
    ...
    def test_enhanced_wiki_manager_has_collaborator(self):
        ...
        # 验证协作器的属性 - 检查实际方法名
        assert hasattr(self.wiki_manager.collaborator, 'create_collaborative_wiki'), "协作器应有create_collaborative_wiki方法"
        assert hasattr(self.wiki_manager.collaborator, 'default_roles'), "协作器应有default_roles属性"

    @pytest.mark.asyncio
    async def test_collaborative_wiki_creation_method_exists(self):
        """测试协作创建方法存在且可调用"""
        # 验证增强管理器有协作创建方法
        assert hasattr(self.wiki_manager, 'create_collaborative_wiki'), "增强Wiki管理器应有协作创建方法"
        
        # 模拟调用协作创建方法（由于缺少完整的依赖，我们验证方法定义）
        import inspect
        method = getattr(self.wiki_manager, 'create_collaborative_wiki')
        assert inspect.iscoroutinefunction(method), "create_collaborative_wiki应为异步方法"
