...
from src.daip_live.wiki.manager import WikiManager, WikiPage, WikiStatistics, WikiError, PageNotFoundError, PageAlreadyExistsError, InvalidTitleError
from src.daip_live.wiki.models import WikiPage as WikiPageModel
...
class TestWikiManager:
    ...
    def test_create_page_with_empty_title_raises_error(self, temp_wiki_dir):
        ...
        # 源码权威: 空标题+空内容才抛错；非空内容会从内容首行提取标题（manager.py:144-156）
    ...
    def test_update_page(self, temp_wiki_dir):
        ...
        # 创建页面
        original_page = manager.create_page(
            title="更新测试",
            content="# 更新测试\n原始内容",
            tags=["原始"]
        )
...
class TestWikiMultiRoleCollaboration:
    ...
    async def test_create_collaborative_page(self, temp_wiki_dir_with_mock_services):
        """测试创建协作页面"""
        temp_wiki_dir, mock_role_manager, mock_model_provider = temp_wiki_dir_with_mock_services
        ...
        assert "域专家" in page.content or "expert" in page.content  # 检查是否有AI贡献内容
    ...
    async def test_add_content_by_role_to_nonexistent_page_raises_error(self, temp_wiki_dir_with_mock_services):
    ...
    async def test_add_content_by_role_without_ai_services_raises_error(self, temp_wiki_dir):
        """测试在没有AI服务配置时添加角色内容抛出错误"""
        manager = WikiManager(wiki_root=temp_wiki_dir)
        
        with pytest.raises(WikiError, match="WikiManager is not configured for AI content generation"):
            await manager.add_content_by_role(
                page_title="测试",
                role_name="测试角色",
                instruction="测试指令"
...
class TestWikiAdvancedFeatures:
    ...
    def test_batch_create_pages_with_duplicate_raises_error(self, temp_wiki_dir):
        ...
        pages_data = [
            {"title": "正常页面", "content": "这是一段正常的页面内容", "tags": ["测试"]},
            {"title": "重复页面", "content": "这是重复页面的第一段内容", "tags": ["重复"]},
            {"title": "重复页面", "content": "这是重复页面的第二段内容", "tags": ["重复"]}  # 重复标题
        ]
    ...
    def test_search_advanced(self, temp_wiki_dir):
        ...
        # 测试按标签过滤
        tag_results = manager.search_advanced("AI", search_type="content", tags=["医疗"])
...
class TestWikiErrorHandling:
    ...
    def test_create_page_already_exists(self, temp_wiki_dir):
        ...
        with pytest.raises(ValueError, match="Page '测试页面' already exists and contains content"):
    ...
    def test_create_page_already_exists_with_empty_content(self, temp_wiki_dir):
        ...
        # 创建一个空页面
        empty_page = manager.create_page("空页面", "", ["空"])
    ...
    def test_is_empty_document(self, temp_wiki_dir):
        ...
        assert manager._is_empty_document("# AI伦理\n\n开始协同创建关于AI伦理的维基页面...") is True
        assert manager._is_empty_document("#  \n\n") is True
        assert manager._is_empty_document("# 标题\n\n") is True
...
async def run_all_async_tests():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch('src.daip_live.p4_role_manager_tools.role_model_manager.RoleModelManager') as mock_role_manager, \
            ...
            await collaboration_test.test_create_collaborative_page((temp_path, mock_role_manager, mock_model_provider))
            await collaboration_test.test_add_content_by_role((temp_path, mock_role_manager, mock_model_provider))
            
            # 测试错误处理
            await collaboration_test.test_add_content_by_role_to_nonexistent_page_raises_error((temp_path, mock_role_manager, mock_model_provider))
            
            # 集成测试（独立目录，避免前序测试创建的页面影响 total_pages 断言）
            integration_test = TestWikiIntegration()
            integration_dir = Path(tempfile.mkdtemp())
            await integration_test.test_complete_wiki_workflow((integration_dir, mock_role_manager, mock_model_provider))
...
def test_sync_runner():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        manager_test.test_create_page_with_whitespace_only_title_raises_error(fresh_dir())
