...
class TestTUIStateManagerSpecification:
    ...
    def test_multiple_subscribers(self):
        ...
        manager.subscribe("test_key", callback_1)
        manager.subscribe("test_key", callback_2)
    ...
    def test_state_history_maintenance(self):
        ...
        assert (
            any("step" in state and state["step"] == 3 for state in history)
            or current_state.get("step") == 3
        )
    ...
    def test_performance_requirement_update_latency(self):
        ...
        assert latency_ms < 50, (
            f"State update latency {latency_ms:.2f}ms exceeds 50ms requirement"
        )
