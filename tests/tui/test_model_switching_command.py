...
class TestModelSwitchingCommand(unittest.TestCase):
    ...
    def test_global_model_switch_command_updates_provider(self, mock_LiteLLMProvider):
        """
        [VERIFY] Test that `/model switch` correctly updates the TUI's main model provider.
        """
        # --- Arrange ---
        ...
        self.mock_model_manager.switch_model.assert_called_once_with(target_model_name, "ollama")

        # 2. Assert that a new LiteLLMProvider was instantiated with the correct config
        # The TUI creates a new ProviderConfig and passes it to a new LiteLLMProvider instance.
