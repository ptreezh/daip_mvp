...
class TestDebateContextMemory:
    ...
    def test_role_debate_session_tracks_personal_history(self):
        ...
        model_config = RoleModelConfig(
            model_name="test-model", provider="ollama", temperature=0.7, max_tokens=1000
        )
        ...
        role_session.add_personal_history(
            1, "AI regulation is necessary for safety", "Opponent argues for innovation"
        )
        role_session.add_personal_history(
            2,
            "Ethical frameworks must guide AI development",
            "Opponent emphasizes economic benefits",
        )
    ...
    def test_layered_memory_system_manages_shared_facts(self):
        ...
        memory_system.add_shared_fact(
            1, "AI development is accelerating rapidly", "system", 0.9
        )
        memory_system.add_shared_fact(
            2, "Regulatory frameworks are being developed", "proponent", 0.8
        )
    ...
    def test_layered_memory_system_updates_role_memory(self):
        ...
        memory_system.update_role_memory(
            "proponent", "Regulation ensures safety", 1, "arguments"
        )
        memory_system.update_role_memory(
            "opponent", "Innovation drives progress", 1, "arguments"
        )
    ...
    def test_layered_memory_system_provides_role_context(self):
        ...
        memory_system.update_role_memory(
            "proponent", "Ethical frameworks are essential", 2, "arguments"
        )

        # Add round summaries
        memory_system.add_round_summary(
            1,
            "First round focused on safety vs innovation",
            ["safety", "innovation"],
            0.3,
        )
        memory_system.add_round_summary(
            2,
            "Second round discussed ethical frameworks",
            ["ethics", "regulation"],
            0.5,
        )
    ...
    def test_layered_memory_system_provides_compressed_context(self):
        ...
        memory_system.update_role_memory(
            "proponent", "Public safety is paramount", 3, "arguments"
        )

        # Execute
        compressed_context = memory_system.get_compressed_context(
            "proponent", 4, max_rounds=2
        )
    ...
    def test_enhanced_debate_manager_integrates_context_and_memory(self):
        ...
        with (
            patch(
                "src.daip_live.p8_debate_system.enhanced_debate_manager.RoleManager"
            ...
            mock_model_provider_instance.generate = AsyncMock(
