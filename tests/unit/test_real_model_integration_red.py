...
class TestRealModelIntegrationRED:
    ...
    def test_require_real_model_provider_not_mock(self, temp_wiki_dir):
        ...
        try:
            ...
            real_model_provider = LiteLLMProvider(provider_config)
            print("✅ 真实模型提供者创建成功")
        ...
        print(f"✅ 正确拒绝了模拟模型提供者: {exc_info.value}")
    ...
    def test_require_real_role_model_manager(self, temp_wiki_dir):
        ...
        # 检查角色管理器配置
        role_manager_config = config_manager.get_config().model_dump().get('role_manager', {})
        ...
        print(f"✅ 找到角色配置文件: {[f.name for f in role_files]}")

        # 尝试创建真实的角色管理器
        try:
            from daip_live.p4_role_manager_tools.role_model_manager import RoleModelManager
            real_role_manager = RoleModelManager()
            print("✅ 真实角色管理器创建成功")
