...
class TestSkillManager:
    ...
    def test_find_skills_by_tag(self, skill_manager):
        ...
        class SkillA(Skill):
            def __init__(self):
                metadata = SkillMetadata("skill_a", "test", "1.0", "author", ["tag1", "common"])
                super().__init__(metadata)
        ...
        class SkillB(Skill):
            def __init__(self):
                metadata = SkillMetadata("skill_b", "test", "1.0", "author", ["tag2", "common"])
                super().__init__(metadata)
    ...
    def test_skill_enable_disable(self, skill_manager, mock_skill):
        ...
        assert mock_skill.is_enabled == True
        
        # Disable the skill
        mock_skill.disable()
        assert mock_skill.is_enabled == False
