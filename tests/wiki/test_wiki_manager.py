...
class TestWikiManager:
    ...
    def test_wiki_manager_create_page_duplicate_title(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建第一个页面
            manager.create_page(
                title="Test Page", content="First page content", tags=["test"]
            )

            # Act & Assert - 尝试创建重复标题
            # 源码权威: manager.py:172 raise "Page 'Test Page' already exists and contains content..."  # noqa: E501
    ...
    def test_wiki_manager_search_pages_by_tag(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建不同标签的页面
            manager.create_page(
                "Python Guide", "Python content", ["python", "programming"]
            )
    ...
    def test_wiki_manager_search_pages_by_content(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建包含不同内容的页面
            manager.create_page(
                "Algorithm Guide", "Content about sorting algorithms", ["algorithms"]
            )
            manager.create_page(
                "Data Structures", "Content about data structures", ["data"]
            )
    ...
    def test_wiki_manager_update_existing_page(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # 创建页面
            page = manager.create_page(
                title="Update Test", content="Original content", tags=["original"]
            )
    ...
    def test_wiki_manager_persistence_saves_and_loads(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            manager1.create_page(
                "Persistent Page", "Persistent content", ["persistent"]
            )
    ...
    def test_wiki_manager_file_naming_sanitization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Act - 创建包含特殊字符的标题
            page = manager.create_page(
                title="Test Page!@#$%^&*()", content="Content", tags=["test"]
            )
