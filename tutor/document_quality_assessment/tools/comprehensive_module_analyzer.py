...
class ComprehensiveModuleAnalyzer:
    ...
    def analyze_module(self, module_id: str, module_path: Path) -> ModuleInfo:
        """分析单个模块"""
        pattern_info = self.module_patterns.get(
            module_id, {"name": module_id, "type": "unknown"}
        )
        ...
        # 确定状态
        status = self.determine_module_status(
            module_id, pattern_info["type"], module_path
        )
    ...
    def determine_module_status(
        self, module_id: str, module_type: str, module_path: Path
    ) -> str:
        """确定模块状态"""
        # archived类型
    ...
    def analyze_module_relationships(self, modules: dict[str, ModuleInfo]) -> dict:
        ...
        relationships["module_hierarchy"] = {
            ...
            "new_p_modules": {
                k: {
                    "name": v.module_name,
                    "replaces": k.replace("new", "").lower()
                    if k.replace("new", "").lower() in core_modules
                    else None,
                    "status": v.status,
                    "doc_count": len(v.documents),
                }
            ...
            "support_modules": {
                ...
                if v.module_type in ["support", "project"]
            },
        }
        ...
        for module_id, module_info in modules.items():
            for dep in module_info.dependencies:
                if dep in relationships["dependency_graph"]:
                    relationships["dependency_graph"][dep]["dependents"].append(
                        module_id
                    )
    ...
    def analyze_evolution_reason(
        self, old_id: str, new_id: str, modules: dict[str, ModuleInfo]
    ) -> str:
        """分析演化原因"""
        ...
        # 基于文档类型分析
        old_docs = {doc["file_type"] for doc in old_module.documents}
        new_docs = {doc["file_type"] for doc in new_module.documents}
    ...
    def generate_teaching_structure(
        self, modules: dict[str, ModuleInfo], relationships: dict
    ) -> dict:
        """生成教学结构"""
        teaching_structure = {
            "learning_tracks": {},
        ...
        for module_id, module_info in modules.items():
            if (
                module_info.module_type in ["original", "new"]
                and not module_info.status == "archived"
            ):
                teaching_structure["module_labs"][module_id] = self.create_module_lab(
                    module_info
                )
        ...
        learning_sequence = [
            {
                "phase": 1,
                "title": "理论学习",
                "documents": doc_groups.get("specification", [])
                + doc_groups.get("requirement", []),
            ...
            },
            {
                "phase": 5,
                "title": "综合实践",
                "documents": doc_groups.get("checklist", [])
                + doc_groups.get("guide", []),
...
"""
        ...
        p_modules = {
            k: v
            for k, v in modules.items()
            if v.module_type == "original" and re.match(r"^p[0-9]$", k)
        }
...
def main():
    ...
    report = analyzer.generate_analysis_report(
        modules, relationships, teaching_structure
    )
